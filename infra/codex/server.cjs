...
const tlsServer = process.env.DEVFEED_CODEX_TLS_CERT ? createTransport({
...
const bridge = net.createServer(client => {
  if (sockets.size >= 64) { client.destroy(); return; }
  const upstream = net.connect(4500, '127.0.0.1');
  sockets.add(client);
  const close = () => { client.destroy(); upstream.destroy(); sockets.delete(client); };
...
bridge.on('error', () => { console.error('Codex socket bridge failed'); stop(); });
bridge.listen(socketPath, () => fs.chmodSync(socketPath, 0o600));
for (const signal of ['SIGINT', 'SIGTERM']) process.on(signal, () => stop(signal));
child.on('error', () => { console.error('Codex failed to start'); process.exit(1); });
child.on('exit', code => {
  tlsServer?.stop();
  bridge.close();
  for (const socket of sockets) socket.destroy();
