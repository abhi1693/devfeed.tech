...
def test_redirect_cannot_reach_metadata_service(monkeypatch):
...
def test_truncated_gzip_is_not_imported(monkeypatch):
    use_pool(
        monkeypatch,
        [
            httpcore.Response(
...
@pytest.mark.parametrize("fetch", [fetch_feed, fetch_page])
