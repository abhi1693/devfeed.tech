...
DEFAULTS = {"show_badge": True, "sound": False}
...
def test_preferences_persist_and_do_not_change_other_account_settings(user_data, database):
    ...
    saved = {"show_badge": False, "sound": True}
    ...
    with database() as session:
        ...
        assert session.get(UserAccount, second).notification_settings == {}
    current.user_id, current.subject = str(second), "user-b"
    assert client.get(PATH).json() == DEFAULTS


@pytest.mark.parametrize("field", ["subject", "issuer", "organization_id"])
def test_notification_settings_require_matching_identity(user_data, field):
...
def test_notification_settings_reset(user_data):
    client = user_data[0]
    client.put(PATH, json={"show_badge": False, "sound": True})
