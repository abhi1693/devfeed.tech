...
export function UserSearch({ filters }: { filters?: FeedFilters }) {
  ...
  const navigate = useCallback((value: string) => {
    ...
    pendingSearchFocus = field && document.activeElement === field ? {
      draft: field.value, start: field.selectionStart ?? field.value.length,
      end: field.selectionEnd ?? field.value.length, submitted: value.trim(), path: new URL(href, window.location.origin).pathname,
    } : null;
    startTransition(() => router.replace(href, { scroll: false }));
  }, [currentQuery, filters, router]);
  const schedule = useCallback((value: string) => {
    if (timer.current) clearTimeout(timer.current);
    timer.current = setTimeout(() => { timer.current = null; navigate(value); }, 350);
  }, [navigate]);
  ...
  useEffect(() => {
    ...
    if (window.location.pathname !== initialFocus.path || new URLSearchParams(window.location.search).get("q") !== (initialFocus.submitted || null)) return;
  ...
  useEffect(() => {
    ...
    if (input.current && document.activeElement !== input.current) input.current.value = currentQuery;
  }, [currentQuery]);
  useEffect(() => {
