...
function NotificationContent() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const config = await userRequest<InboxConfig>("notifications/config", {
        signal: AbortSignal.any([controller.signal, AbortSignal.timeout(15000)]),
      });
      if (controller.signal.aborted) return;
      if (!config.enabled || !config.environment || !config.subscriber_id) {
        setState({ enabled: true, disabled: true });
        return;
      }
      ...
      const enabled =
        preferences.find(
          (item) => item.channel === "in_app" && item.category === topicNotificationCategory,
        )?.enabled ?? true;
      if (!controller.signal.aborted) setState({ client, enabled });
    })().catch(() => {
      if (!controller.signal.aborted) setState({ enabled: true, failed: true });
    });
    return () => {
      controller.abort();
      client?.close();
  ...
  if (unavailable)
    return (
      <section className="profile-load-error" role="status">
        <h2>Couldn’t load notification settings</h2>
        <p>Your saved preferences are unchanged.</p>
...
function NotificationForm({
  ...
  inbox: InboxState;
  retry: () => void;
  onInboxSaved: (enabled: boolean) => void;
}) {
  ...
  if (snapshot !== baseline && !busy) {
    ...
    setValue({
      display:
        JSON.stringify(value.display) === JSON.stringify(previous.display)
          ? initial.display
          : value.display,
      enabled: value.enabled === previous.enabled ? initial.enabled : value.enabled,
    });
  ...
  return (
    <section className="profile-panel" aria-label="Notification preferences">
      <form
        ...
        onSubmit={async (event) => {
          ...
          try {
            if (inbox.client) {
              await inbox.client.setPreferences([
                { category: topicNotificationCategory, channel: "in_app", enabled: value.enabled },
              ]);
              inboxSaved = true;
            ...
            setMessage(
              inboxSaved
                ? "Article notification choices saved, but badge and sound settings could not be saved. Retry Save changes."
                : "Couldn’t save notification settings. Your changes are still here; please try again.",
            );
          } finally {
      ...
      >
        <fieldset disabled={busy}>
          ...
          <div className="notification-event-settings">
            <h2>Inbox events</h2>
            <Toggle
              label="New articles from followed topics"
              description="Notify me when a new article is published in a topic I follow."
              checked={value.enabled}
              disabled={!inbox.client}
              onChange={(enabled) => change({ ...value, enabled })}
            />
            {inbox.client ? (
              <p className="settings-help">
                Muting hides new and existing article notifications. Turning this back on restores
                the history. Your followed topics stay unchanged.
              </p>
            ) : (
              <div className="notification-connection" role="status">
                <p>
                  {inbox.disabled
                    ...
                    : "Could not load article notification preferences. Your badge and sound preferences can still be saved."}
                </p>
                <button
                  type="button"
        ...
        <div className="profile-form-actions">
          <button
            ...
            onClick={() =>
              change({
                display: notificationDefaults,
                enabled: inbox.client ? true : value.enabled,
              })
            }
          >
            Reset to defaults
