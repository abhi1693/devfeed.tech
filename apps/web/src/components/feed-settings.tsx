...
function FeedSettingsForm() {
  ...
  const { view, content_types, loading, busy, unavailable, error, save, refresh } = useFeedPreferences();
  ...
  return <section className="profile-panel" aria-label="Feed preferences">
    ...
    <form className="profile-form" onSubmit={async event => {
      ...
      setMessage("");
      if (await save({ view: selected, content_types: types })) { setMessage("Feed settings saved."); router.refresh(); }
    }}>
      ...
      <fieldset className="feed-content-preferences" disabled={busy}>
        ...
        <p className="profile-description" id="content-types-help">Choose the kinds of posts you want in your feeds.</p>
        <div className="feed-content-options" aria-describedby="content-types-help">
          {contentTypes.map(kind => <label className="feed-content-option" key={kind}>
            <input type="checkbox" name="content_types" value={kind} aria-label={contentOptions[kind][0]} aria-describedby={`content-type-${kind}-description`} checked={types.includes(kind)} onChange={() => {
            setTypes(contentTypes.filter(type => type === kind ? !types.includes(kind) : types.includes(type)));
            setMessage("");
            }} />
            <span><span className="feed-content-label">{contentOptions[kind][0]}</span><span className="feed-content-description" id={`content-type-${kind}-description`}>{contentOptions[kind][1]}</span></span>
          </label>)}
        </div>
        <p className="feed-content-help">You can still browse any type from the feed tabs.</p>
        {!types.length && <p className="profile-feedback error" role="alert">Select at least one content type.</p>}
      </fieldset>
      </section>
      ...
      <div className="profile-form-actions">
        <button className="settings-button settings-button-ghost" type="button" disabled={busy} onClick={() => { choose("cards"); setTypes([...contentTypes]); }}>Reset to defaults</button>
        <button className="settings-button" type="submit" disabled={busy || !dirty || !types.length}>{busy ? "Saving…" : "Save changes"}</button>
      </div>
