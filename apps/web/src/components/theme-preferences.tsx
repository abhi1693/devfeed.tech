...
const Context = createContext({
  appearance: defaults, loading: false, busy: false, unavailable: false, error: "", refresh: () => {},
  update: async (patch: Partial<Appearance>) => { if (patch.theme) setBrowserTheme(patch.theme); return true; },
  save: async (theme: ThemeMode) => { setBrowserTheme(theme); return true; },
});
export function useDateTimePreferences() { return useContext(Context).appearance; }
...
export function ThemePreferencesProvider({ children }: { children: React.ReactNode }) {
  ...
  const [state, setState] = useState<{ owner: string; unavailable: boolean; appearance: Appearance } | null>(null);
  ...
  useEffect(() => {
    ...
    userRequest<Appearance>("settings/appearance", { signal: AbortSignal.any([controller.signal, AbortSignal.timeout(15000)]) })
      .then(value => {
        ...
        if (!controller.signal.aborted) setState({ owner, unavailable: true, appearance: defaults });
      });
  ...
  async function update(patch: Partial<Appearance>) {
    ...
    if (!user) { if (patch.theme) setBrowserTheme(patch.theme); return true; }
    ...
    try {
      const value = await userRequest<Appearance>("settings/appearance", {
        ...
        body: JSON.stringify({ ...current?.appearance, ...patch }), signal: controller.signal,
      });
      ...
      if (!controller.signal.aborted) setFailure({ owner, message: "Couldn’t save your appearance settings. Please try again." });
      return false;
    } finally { setSaving(null); }
  }
  return <Context.Provider value={{ appearance: current?.appearance ?? defaults, loading: pending, busy: saving === owner, unavailable: !!user && !!current?.unavailable, error: failure?.owner === owner ? failure.message : "", update, save: (theme: ThemeMode) => update({ theme }), refresh: () => setRevision(value => value + 1) }}>{children}</Context.Provider>;
}
