...
export function useInfinitePages<T extends Page>(initialPage: T, fetchPage: (cursor: string, signal: AbortSignal) => Promise<T>) {
  ...
  const loadMore = useCallback(async () => {
    ...
    setError(null);
    try {
      const page = await fetchPage(cursor, AbortSignal.any([controller.signal, AbortSignal.timeout(15000)]));
      if (controller.signal.aborted) return;
      loaded.current.add(cursor);
      setPages(current => [...current, page]);
      setCursor(page.next_cursor !== null && !loaded.current.has(page.next_cursor) ? page.next_cursor : null);
      return page;
    } catch (cause) {
      if (!controller.signal.aborted) setError(cause instanceof Error ? cause : new Error("Could not load page"));
    } finally {
