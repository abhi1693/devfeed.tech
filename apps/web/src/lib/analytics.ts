...
export function trackEvent<K extends keyof AnalyticsEvents>(name: K, params: AnalyticsEvents[K]) {
  ...
  try {
    window.dispatchEvent(new CustomEvent(analyticsEventName, { detail: { name, params } }));
  } catch {
    /* Analytics must never break the action. */
  }
}

export function trackUserMutation(
  path: string,
  ...
  if (method === "PUT" && like && typeof value.liked === "boolean") {
    ...
    method === "PUT" &&
    path === "settings/feed" &&
    (value.view === "cards" || value.view === "compact") &&
    Array.isArray(value.content_types)
  ) {
    trackEvent("feed_settings_saved", {
      feed_view: value.view,
      selected_count: value.content_types.length,
    });
  } else if (
    method === "PUT" &&
    ...
    try {
      ...
      if (sourceType === "publisher" || sourceType === "aggregator")
        trackEvent("source_suggested", { source_type: sourceType });
    } catch {
      /* Never send raw request bodies or user text. */
    }
  }
}
