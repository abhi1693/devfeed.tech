...
vi.mock("@/components/source-follow", () => ({ SourceFollow: ({ sourceId }: { sourceId: string }) => <button>Follow {sourceId}</button> }));
let intersect: () => void;
const fetcher = vi.fn();
beforeEach(() => {
  fetcher.mockReset();
...
it.each(["topics", "sources"] as const)("appends %s without duplicate cards, preserves routes, and stops at the end", async kind => {
  const item = kind === "topics" ? topic : source;
  const first = Array.from({ length: 60 }, (_, i) => ({ ...item, id: String(i), name: `Item ${i}` }));
  ...
  expect(screen.getByRole("link", { name: `More ${kind}` }).getAttribute("href")).toBe(`/${kind}?offset=120`);
  await act(async () => { intersect(); intersect(); });
  expect(fetcher).toHaveBeenCalledTimes(1);
  expect(fetcher.mock.calls[0][0]).toBe(`/api/v1/${kind}?offset=120`);
  await act(async () => resolve(Response.json({ items: [first[0], extra, extra], next_cursor: null })));
  ...
  if (kind === "sources") expect(screen.getByRole("button", { name: "Follow extra" })).toBeDefined();
});

it("retains topics on error, retries manually, and cancels pagination in a background tab", async () => {
  const items = Array.from({ length: 60 }, (_, i) => ({ ...topic, id: String(i), name: `Topic ${i}` }));
  fetcher.mockResolvedValueOnce(Response.json({}, { status: 503 })).mockImplementationOnce(() => new Promise(() => {})).mockResolvedValue(Response.json({ items: [], next_cursor: null }));
...
it("reveals preference choices incrementally and resets the visible batch when search changes", async () => {
  ...
  const choices = (visible: string[]) => <div>{visible.map(item => <button key={item}>{item}</button>)}</div>;
  const view = render(<InfiniteChoices key="all" items={items} label="topics">{choices}</InfiniteChoices>);
  expect(screen.queryByRole("button", { name: "Topic 60" })).toBeNull();
  await act(async () => intersect());
  expect(screen.getByRole("button", { name: "Topic 119" })).toBeDefined();
  view.rerender(<InfiniteChoices key="129" items={[items[129]]} label="topics">{choices}</InfiniteChoices>);
