...
it("uses the shared inbox, marks seen and read with CSRF, opens an article modal and closes on sign out", async () => {
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input, init) => {
      ...
      if (path.endsWith("config"))
        return Response.json({ enabled: true, environment: "users", subscriber_id: "user_a" });
  ...
  expect(
    requests.some((r) => r.path.endsWith("seen-all") && r.headers.get("x-csrf-token") === "csrf"),
  ).toBe(true);
  ...
  expect(
    requests.some(
      (r) =>
        r.path.endsWith("/read") && r.method === "POST" && r.headers.get("x-csrf-token") === "csrf",
    ),
  ).toBe(true);
  expect(
    requests.filter((r) => r.path.includes("chimely")).every((r) => r.path.startsWith(inboxPath)),
  ).toBe(true);
...
it("only opens local article URLs", () => {
  ...
  expect(notificationArticle("/articles/docker-image-guide-42")).toBe(
    "/articles/docker-image-guide-42",
  );
  for (const url of [
    "//evil.test",
    "javascript:alert(1)",
