...
afterEach(() => { cleanup(); vi.clearAllMocks(); vi.unstubAllGlobals(); session.user = null; });
...
it("shows only saved content types that have data on the current topic", async () => {
  session.user = { user_id: "reader", csrf_token: "csrf", name: "Reader", email: null, expires_at: 9999999999 };
  const fetcher = vi.fn().mockResolvedValue(Response.json({ view: "cards", content_types: ["tutorial", "news"] }));
  vi.stubGlobal("fetch", fetcher);
  render(<FeedPreferencesProvider><FeedFiltersBar filters={parseFilters({ topic: "python" })} topicPage sources={[]} availableTypes={["article", "tutorial", "opinion"]} /></FeedPreferencesProvider>);
  const tabs = within(screen.getByRole("navigation", { name: "Article type" }));
  expect(tabs.queryByRole("link", { name: "Articles" })).toBeNull();
  expect((await tabs.findByRole("link", { name: "Tutorials" })).getAttribute("href")).toBe("/topics/python/tutorials");
...
it("keeps all available content types for guests without fetching preferences", async () => {
  const fetcher = vi.fn();
  vi.stubGlobal("fetch", fetcher);
  render(<FeedPreferencesProvider><FeedFiltersBar filters={parseFilters({})} sources={[]} availableTypes={["article", "opinion"]} /></FeedPreferencesProvider>);
  const tabs = within(screen.getByRole("navigation", { name: "Article type" }));
  await tabs.findByRole("link", { name: "Articles" });
  expect(tabs.getAllByRole("link").map(link => link.textContent)).toEqual(["All", "Articles", "Opinions"]);
