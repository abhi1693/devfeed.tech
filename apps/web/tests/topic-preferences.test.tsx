...
it("toggles topics with the keyboard and saves the selection without checkboxes", async () => {
  const fetcher = vi.fn((url: string, init?: RequestInit) => Promise.resolve(Response.json(
    url.endsWith("auth/me") ? { user_id: "user-a", csrf_token: "csrf" }
      : init?.method === "PUT" ? { topic_ids: [topic.id] } : { topic_ids: [] },
  )));
  vi.stubGlobal("fetch", fetcher);
  const user = userEvent.setup();
  render(<UserProvider><TopicPreferences topics={[topic]} /></UserProvider>);
  const tile = await screen.findByRole("button", { name: topic.name });
  expect(screen.queryByRole("checkbox")).toBeNull();
  expect(tile.getAttribute("aria-pressed")).toBe("false");
  tile.focus(); await user.keyboard(" ");
  ...
  expect(JSON.parse(String(fetcher.mock.calls.find(([, init]) => init?.method === "PUT")?.[1]?.body))).toEqual({ topic_ids: [topic.id] });
});
