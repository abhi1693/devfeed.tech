...
let display = { show_badge: true, sound: false },
  enabled = true,
  failDisplay = false,
  failInbox = false;
let writes: { path: string; body: unknown; csrf: string | null }[];
beforeEach(() => {
  display = { show_badge: true, sound: false };
  enabled = true;
  failDisplay = false;
  failInbox = false;
  writes = [];
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input, init) => {
      ...
      if (path.endsWith("auth/me"))
        return Response.json({ user_id: "alice", name: "Alice", csrf_token: "csrf" });
      ...
      if (path.endsWith("config"))
        return Response.json({ enabled: true, environment: "users", subscriber_id: "user_alice" });
      if (path.endsWith("preferences")) {
        if (failInbox) return Response.json({}, { status: 503 });
        if (write) enabled = body.preferences[0].enabled;
        return Response.json({
          preferences: [{ category: "feed.topic.new", channel: "in_app", enabled }],
        });
      }
...
it("persists display and delivery choices with CSRF, updates the bell and reloads saved values", async () => {
  ...
  expect(display).toEqual({ show_badge: false, sound: true });
  expect(enabled).toBe(false);
  expect(writes).toHaveLength(2);
  expect(writes.every((write) => write.csrf === "csrf")).toBe(true);
  expect(screen.getByRole("button", { name: "Notifications" })).toBeTruthy();
  expect(screen.queryByText("You have unsaved changes.")).toBeNull();
  view.unmount();
  mount();
  const choice = (await screen.findByRole("checkbox", {
    name: "New articles from followed topics",
  })) as HTMLInputElement;
  expect(choice.checked).toBe(false);
  expect(
    (screen.getByRole("checkbox", { name: "Notification sound" }) as HTMLInputElement).checked,
  ).toBe(true);
});
...
it("resets to defaults only when changes are saved", async () => {
  display = { show_badge: false, sound: true };
  enabled = false;
  const user = userEvent.setup();
  mount();
  ...
  await user.click(screen.getByRole("button", { name: "Save changes" }));
  await screen.findByText("Notification settings saved.");
  expect(display).toEqual({ show_badge: true, sound: false });
