...
it("debounces typing, retains filters, and resets pagination without requiring Enter", () => {
  vi.useFakeTimers();
  render(<UserSearch filters={parseFilters({ topic: "python", content_type: "tutorial", cursor: "old-page" })} />);
  const search = screen.getByRole("searchbox");
  fireEvent.change(search, { target: { value: "rea" } });vi.advanceTimersByTime(200);
  fireEvent.change(search, { target: { value: "react" } });vi.advanceTimersByTime(349);
...
it("clearing search updates results and unmounting cancels queued work", () => {
  vi.useFakeTimers();const view = render(<UserSearch filters={parseFilters({ q: "react" })} />);
  fireEvent.change(screen.getByRole("searchbox"), { target: { value: "" } });vi.advanceTimersByTime(350);
  expect(router.replace).toHaveBeenCalledWith("/", { scroll: false });
  fireEvent.change(screen.getByRole("searchbox"), { target: { value: "later" } });view.unmount();vi.advanceTimersByTime(350);
  expect(router.replace).toHaveBeenCalledOnce();
});

it("waits for composition to finish before searching", () => {
  vi.useFakeTimers();render(<UserSearch />);const search = screen.getByRole("searchbox");
  fireEvent.compositionStart(search);fireEvent.change(search, { target: { value: "日本語" } });vi.advanceTimersByTime(500);
  expect(router.replace).not.toHaveBeenCalled();
  fireEvent.compositionEnd(search);vi.advanceTimersByTime(350);expect(router.replace).toHaveBeenCalledOnce();
});
