...
function setup(signedIn = true) {
  ...
  const fetcher = vi.fn(async (url: string, init?: RequestInit) => {
    if (url.endsWith("auth/me"))
      return Response.json(signedIn ? { user_id: "one", csrf_token: "csrf" } : null);
...
const shell = (children: React.ReactNode) => (
  <UserProvider>
    <SourceFollowsProvider>{children}</SourceFollowsProvider>
  </UserProvider>
);
it("shares one preference read across source controls and synchronizes follow state", async () => {
  ...
  render(
    shell(
      <>
        ...
        <SourceFollow sourceId={source.id} returnTo="/articles/test" />
      </>,
    ),
  );
  await waitFor(() =>
    expect(
      (screen.getAllByRole("button", { name: "Follow source" })[0] as HTMLButtonElement).disabled,
    ).toBe(false),
  );
  fireEvent.click(screen.getAllByRole("button", { name: "Follow source" })[0]);
  await waitFor(() =>
    expect(screen.getAllByRole("button", { name: "Following source" })).toHaveLength(2),
  );
  expect(
    mock.fetcher.mock.calls.filter(
      ([url, init]) => url.endsWith("preferences/sources") && !init?.method,
    ),
  ).toHaveLength(1);
  ...
  mock.fail();
  fireEvent.click(screen.getAllByRole("button", { name: "Following source" })[0]);
...
it("keeps browsing anonymous and sends sign-in back to the source", async () => {
  ...
  render(shell(<SourceFollow sourceId={source.id} returnTo={`/sources/${source.id}`} />));
