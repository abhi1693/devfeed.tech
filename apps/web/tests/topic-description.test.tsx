...
vi.mock("@/lib/api", () => ({ getTopic: vi.fn(async () => ({
  ...topic, description: "A *literal* pattern with [brackets](example).",
})) }));
vi.mock("@/components/topic-follow", () => ({ TopicFollow: () => null }));
afterEach(cleanup);

it("renders topic prose without applying Markdown formatting", async () => {
  const { container } = render(await ArticleTopicBrief({
    topic: { ...topic, role: "primary" }, articleSlug: "example",
  }));
