...
it.each([
  ...
  [
    "settings/feed",
    "PUT",
    { view: "compact", content_types: ["article", "news"] },
    "feed_settings_saved",
    { feed_view: "compact", selected_count: 2 },
  ],
  [
    "settings/appearance",
    "PUT",
    { theme: "system", timezone: "Private/Location" },
    "appearance_settings_saved",
    { theme: "system" },
  ],
...
it("sends source type without the feed URL, submitted name or response identity", () => {
  ...
  trackUserMutation(
    "sources/suggestions",
    "POST",
    { name: "Personal name", id },
    JSON.stringify({
      source_type: "publisher",
      feed_url: "https://example.com/private?token=secret",
      name: "Personal name",
    }),
  );
  expect((spy.mock.calls[0][0] as CustomEvent).detail).toEqual({
    name: "source_suggested",
    params: { source_type: "publisher" },
  });
});
...
it("ignores reads, profile details, lookups, and duplicate backend open telemetry", () => {
  ...
  trackUserMutation("settings/profile", "PUT", {
    name: "Personal name",
...
it("emits no success event for failed writes, then emits once on success", async () => {
  ...
  try {
    const fetcher = vi
      .fn()
      .mockResolvedValueOnce(Response.json({ detail: "Failed" }, { status: 503 }))
      .mockResolvedValueOnce(Response.json({ liked: true }));
    ...
    expect(events.map((event) => event.detail)).toEqual([
      { name: "article_like", params: { article_id: id } },
    ]);
  } finally {
    window.removeEventListener(analyticsEventName, listener);
  }
});
