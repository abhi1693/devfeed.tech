...
beforeEach(() => {
  account.user = null;
  vi.mocked(userRequest).mockReset().mockResolvedValue({ article_id: "article", opens: 1, likes: 0, liked: false });
});
afterEach(cleanup);
...
function preview() {
  return render(<>
    ...
    <ArticleReadLink articleId="article" href="https://publisher.example/article?utm_source=devfeed" target="_blank" rel="noopener noreferrer">Read article</ArticleReadLink>
  </>);
}
...
it("records no view on preview mount and records an anonymous outbound click", async () => {
  ...
  await waitFor(() => expect(userRequest).toHaveBeenCalledWith("articles/article/open", {
...
it("includes signed-in CSRF and supports opening the original with the middle button", async () => {
  account.user = { user_id: "user", csrf_token: "csrf", name: null, email: null, expires_at: 4102444800 };
  preview();
  ...
  await waitFor(() => expect(userRequest).toHaveBeenCalledWith("articles/article/open", {
    method: "POST", keepalive: true, headers: { "X-CSRF-Token": "csrf" },
  }));
});
