"""Durable, lease-protected web research for pending topic proposals."""
...
def _analyze(identifier):
    ...
    logger.info("topic_analysis_started", extra={"proposal_id": proposal_id})
    try:
        # Hosted web search is allowed for this task; all other tools remain disabled.
        client = CodexClient(settings)
        output = client.complete(
            research_prompt(snapshot),
            TopicResearchResult.model_json_schema(),
            allow_web_search=True,
        )
        result = TopicResearchResult.model_validate(output)
        with factory.begin() as session:
            job = _locked_job(session, identifier)
