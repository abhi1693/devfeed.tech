...
import type { AdminIdentity, AdminOverview, AuthConfig } from "@/lib/api/generated/models";
...
export async function initialOverview(): Promise<AdminOverview> {
  const jar = await cookies();
  const session = jar.get("__Host-devfeed_admin_session") ?? jar.get("devfeed_admin_session");
  if (!session) redirect("/login");
  const response = await fetch(`${adminApiOrigin()}/v1/admin/overview`, {
