...
export const notify = {
  ...
  info: toast.info,
  warning: (message: string, options?: ExternalToast) => toast.warning(message, { duration: 8000, ...options }),
  error: (message: string, options?: ExternalToast) => toast.error(message, { duration: 8000, ...options }),
};
...
export function notifyFailure(error: unknown, title: string, id?: string) {
  ...
  notify.error(title, {
    id,
    description: error instanceof ApiError ? error.message : "Please try again. If the problem continues, check the admin service.",
  });
}
