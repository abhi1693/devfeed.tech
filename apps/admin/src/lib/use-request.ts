...
export function useRequest<T>(
  ...
  const refresh = useCallback(
    async (signal: AbortSignal, automatic = false) => {
      if (automatic && (unauthorized.current || (pending.current && !pending.current.aborted)))
        return;
      pending.current = signal;
      if (automatic)
        setState((previous) =>
          previous?.key === key ? { ...previous, refreshing: true } : previous,
        );
      try {
        ...
        if (pending.current === signal) {
          ...
          setState((previous) =>
            previous?.key === key && previous.refreshing
