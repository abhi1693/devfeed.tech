...
export type ResourceSpec = {
  ...
  columns: ColumnSpec[];
  fields: FieldSpec[];
  filter?: { key: string; label: string; choices: string[] };
  readonly?: boolean;
};
...
const job = (label: string, resource: "articles" | "sources"): ResourceSpec => ({
  ...
  filter: {
    key: "status",
    label: "Status",
    choices: ["queued", "running", "succeeded", "failed", "retried"],
  },
});
...
export const resources: Record<Resource, ResourceSpec> = {
  users: {
    label: "Users",
    singular: "User",
    description: "Inspect accounts, followed topics, likes, and personalized recommendations.",
    group: "Audience",
    title: "name",
    defaultSort: "-created_at",
    readonly: true,
    ...
    fields: [
      ...
      { key: "sign_in_name", label: "Sign-in name" },
      { key: "email", label: "Email" },
      { key: "avatar_url", label: "Avatar", type: "logo-url" },
    ],
  },
  articles: {
    label: "Articles",
    ...
    fields: [
      ...
      { key: "image_url", label: "Image URL", type: "image-url", max: 2048 },
      {
        ...
        type: "language",
        max: 35,
        help: "Select the article’s language, or leave it unspecified if unknown.",
      },
      {
        key: "content_type",
        label: "Content type",
  ...
  sources: {
    ...
    fields: [
      ...
      { key: "website_url", label: "Website", type: "url", max: 2048 },
      {
        ...
        type: "logo-url",
        max: 2048,
        help: "The source’s logo or declared website icon.",
      },
      {
        key: "image_url",
        label: "Image URL",
        type: "image-url",
        max: 2048,
        help: "A preview or cover image, separate from the logo.",
      },
      {
        key: "language",
        label: "Language",
  ...
  topics: {
    ...
    fields: [
      ...
      slug,
      {
        ...
        required: true,
        max: 50,
        help: "A descriptive kind, such as discipline, language, framework, or platform.",
      },
      {
        key: "status",
        label: "Status",
  ...
  "topic-relations": {
    ...
    singular: "Topic relationship",
    description: "Approved relationships and AI suggestions awaiting review.",
    group: "Taxonomy",
    title: "relation",
    ...
    fields: [
      ...
      },
      {
        ...
        label: "Relationship",
        type: "select",
        choices: relationKinds,
        required: true,
        createOnly: true,
        default: "related_to",
      },
      {
        key: "related_topic_id",
        label: "To topic",
        type: "reference",
        resource: "topics",
        required: true,
