...
export function useTableQuery(key: string) {
  ...
  const permitted = (name: string) =>
    allowed.has(name) && (name === "limit" || (name === "sort" ? sort : filters));
  const query = new URLSearchParams(
    raw ||
      Object.entries(remembered).filter(
        (entry): entry is [string, string] => permitted(entry[0]) && typeof entry[1] === "string",
      ),
  );
  if (!query.has("limit")) query.set("limit", String(pageSize));
  const next = JSON.stringify(
    Object.fromEntries([...new URLSearchParams(raw)].filter(([name]) => permitted(name))),
  );
  const previous = JSON.stringify(remembered);
  useEffect(() => {
    if (!persistent || !raw || next === previous) return;
    const timer = window.setTimeout(() => {
      void saveTable(key, { query: JSON.parse(next) }).catch((error) =>
        notifyFailure(error, "Could not save table preferences"),
      );
    }, 500);
