...
export function validNodeId(value: string | null) {
  return value && nodePattern.test(value) ? value : undefined;
}
export function graphQuery(search: URLSearchParams): AdminKnowledgeGraphParams {
  const layers = Array.from(
    new Set(
      search
        .getAll("layers")
        .filter((v): v is GraphLayer => ["article", "tag", "source", "user"].includes(v)),
    ),
  );
  ...
  return {
    focus: validNodeId(search.get("focus")),
    expand: Array.from(new Set(search.getAll("expand").filter((v) => validNodeId(v)))).slice(0, 20),
    layers,
    depth: search.get("depth") === "2" ? 2 : 1,
    limit: [100, 150, 300].includes(Number(search.get("limit")))
      ? Number(search.get("limit"))
      : 150,
    relation: graphRelations.includes(search.get("relation") ?? "")
      ? (search.get("relation") as AdminKnowledgeGraphParams["relation"])
      : undefined,
    include_pending: search.get("pending") === "1",
    published_only: search.get("published") === "1",
  };
}
...
export function edgeLabel(edge: GraphEdge) {
  return edge.kind === "classified_as"
    ? `${humanize(edge.role ?? "classified")} topic`
    : humanize(edge.kind);
}
...
export function updateGraphDetail(cy: Core) {
  const zoom = Math.max(0.15, cy.zoom());
  cy.nodes()
    .toggleClass("zoomed-out", cy.nodes().length > 80 && zoom < 0.65)
...
export function packGraphComponents(cy: Core) {
  ...
  const boxes = components
    .map((component) => ({ component, box: component.boundingBox() }))
    .sort((a, b) => b.box.h - a.box.h);
  const gap = 75;
  const area = boxes.reduce((area, { box }) => area + (box.w + gap) * (box.h + gap), 0);
  const width = Math.max(
    ...boxes.map(({ box }) => box.w),
    Math.sqrt((area * cy.width()) / Math.max(1, cy.height())),
  );
  let x = 0,
    y = 0,
    rowHeight = 0;
...
export function syncGraph(cy: Core, nodes: GraphNode[], edges: GraphEdge[]) {
  ...
  cy.batch(() => {
    ...
    for (const node of nodes) {
      const data = {
        ...node,
        label: node.label.length > 48 ? node.label.slice(0, 45) + "…" : node.label,
      };
  ...
  added.forEach((node, index) => {
    const neighbour = node
      .neighborhood()
      .nodes()
      .filter((other) => before.has(other.id()))
      .first();
    const center = neighbour.length ? (neighbour as NodeSingular).position() : { x: 0, y: 0 };
    const angle = index * 2.399963;
    node.position({
      x: center.x + Math.cos(angle) * (110 + (index % 5) * 20),
      y: center.y + Math.sin(angle) * (110 + (index % 5) * 20),
    });
  });
  return { before, added };
}
...
export function graphStyles(dark: boolean): StylesheetJson {
  ...
  return [
    ...
    ...nodeKinds.map((kind) => ({
      ...
      style: {
        "background-color": graphColors[kind],
        "border-color": graphColors[kind],
        shape: (
          {
            topic: "ellipse",
    ...
    })),
    {
      ...
      style: {
        width: 1.5,
        "line-color": dark ? "#687586" : "#91a0b5",
        "target-arrow-color": dark ? "#687586" : "#91a0b5",
    ...
    { selector: "edge[!directed]", style: { "target-arrow-shape": "none" } },
    {
      ...
      style: { "line-style": "dashed", "line-color": "#b87918", "target-arrow-color": "#b87918" },
    },
    { selector: "node.pinned", style: { "border-width": 4, "border-style": "double" } },
    { selector: ".dim", style: { opacity: 0.18 } },
    {
      selector: "node.active, node.highlight",
      style: {
    ...
    {
      selector: "edge.active, edge.highlight",
      style: { label: "data(label)", width: 2.5, "z-index": 10 },
    },
    { selector: "node.zoomed-out", style: { label: "" } },
    {
      selector: "node.zoomed-out.active, node.zoomed-out.highlight",
