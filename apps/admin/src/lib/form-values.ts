...
export function initialFacts(facts: TopicFact[] = []): EditableFact[] {
  return facts.map((fact) => ({
    ...fact,
    retrieved_at: localDate(fact.retrieved_at),
    originalRetrievedAt: fact.retrieved_at,
  }));
}
export function factsPayload(facts: EditableFact[]): TopicFact[] {
  return facts.map(({ originalRetrievedAt, ...fact }) => ({
    ...fact,
    retrieved_at:
      originalRetrievedAt && fact.retrieved_at === localDate(originalRetrievedAt)
        ? originalRetrievedAt
        : new Date(fact.retrieved_at).toISOString(),
  }));
}
...
export function formPayload(
  ...
  for (const field of resources[resource].fields) {
    ...
    if (field.type === "lines") value = lineValues(value);
      ...
      value = value
        ? record?.[field.key] && value === localDate(String(record[field.key]))
          ? record[field.key]
