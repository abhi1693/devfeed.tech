...
export function SettingsProvider({
  ...
  const save = useCallback(
    async <S extends SettingsSection>(section: S, value: Settings[S]) => {
      if (!persistent) {
      ...
      return schedule(async () => {
        ...
        const actions = {
          ...
          appearance: adminSettingsAppearance,
          defaults: adminSettingsDefaults,
        };
        const action = actions[section] as (
          body: Settings[S],
          options: { headers: Record<string, string> },
        ) => Promise<UserSettings>;
        const result = normalizeSettings(await action(value, options));
        if (mounted.current)
          setSettings((previous) => ({ ...previous, [section]: result[section] }));
      });
    },
    [persistent, admin.csrf_token, schedule],
  );
  const saveTable = useCallback(
    async (key: string, value: TableSettingsPatch) => {
      if (!persistent) return;
      return schedule(async () => {
        const result = normalizeSettings(
          await adminSettingsTable(key, value, { headers: { "X-CSRF-Token": admin.csrf_token } }),
        );
        if (mounted.current)
          setSettings((previous) => ({
            ...previous,
            tables: { ...previous.tables, [key]: result.tables[key] },
          }));
      });
    },
    [persistent, admin.csrf_token, schedule],
  ...
  const resetTables = useCallback(async () => {
    return schedule(async () => {
      if (persistent)
        await adminSettingsTablesReset({ headers: { "X-CSRF-Token": admin.csrf_token } });
  ...
  useEffect(() => {
    ...
    const refresh = async (signal: AbortSignal) => {
      ...
      try {
        ...
        /* Keep the last saved preferences through temporary outages. */
      }
    };
    const stop = persistent
      ? runWhenPageActive((signal, resumed) => {
    ...
    const apply = () =>
      root.classList.toggle(
        ...
        settings.appearance.theme === "dark" ||
          (settings.appearance.theme === "system" && !!media?.matches),
      );
    apply();
  ...
  return (
    <Context.Provider value={{ settings, persistent, save, saveTable, resetTables }}>
      {children}
    </Context.Provider>
  );
}
