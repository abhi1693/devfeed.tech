...
export function EntityPicker({
  ...
  const result = useRequest(
    `${scope}/${status}/${open}/${params.q}/${params.offset}/${revision}`,
    load,
  );
  ...
  return (
    <Combobox
      ...
      onChange={(value) => {
        setPicked(choices.find((choice) => choice.value === value));
      ...
      onOpenChange={(next) => {
        setOpen(next);
        setSearch("");
        setParams({ q: "", offset: 0 });
      }}
      loading={query !== params.q || result.loading}
      error={result.error ? "Could not load options. Try again." : undefined}
      onRetry={() => setRevision((value) => value + 1)}
      footer={
        <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
          <span role="status">
            {total.toLocaleString()} matching {total === 1 ? "object" : "objects"}
          </span>
          {(total > 25 || params.offset > 0) && (
            <div className="flex items-center gap-1">
              <Button
                ...
                onClick={() =>
                  setParams((previous) => ({
                    ...previous,
                    offset: Math.max(0, previous.offset - 25),
                  }))
                }
              >
                <ChevronLeftIcon />
              ...
              <span>
                {Math.floor(params.offset / 25) + 1} / {Math.max(1, Math.ceil(total / 25))}
              </span>
              <Button
                type="button"
                size="icon-xs"
                ...
                disabled={params.offset + 25 >= total}
                onClick={() =>
                  setParams((previous) => ({ ...previous, offset: previous.offset + 25 }))
                }
              >
                <ChevronRightIcon />
              </Button>
