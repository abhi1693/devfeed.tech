...
export function TopicAnalysisControl({
  proposal,
  compact = false,
  disabled = false,
  onUpdated,
  ...
  proposal: TopicProposalOut;
  compact?: boolean;
  disabled?: boolean;
  onUpdated: (proposal: TopicProposalOut) => void;
  ...
  const missing = ["description", "aliases", "keywords", "website_url", "logo_url", "facts"].some(
    (field) => {
  ...
  useEffect(() => {
    onBusyChange?.(starting || active);
  }, [starting, active, onBusyChange]);
  const poll = useCallback(
    async (signal: AbortSignal) => {
      try {
        const updated = await adminTopicProposalGet(proposal.id, { signal });
        if (signal.aborted) return;
        setJob(updated.analysis);
        setError(undefined);
        if (!analysisActive(updated.analysis)) onUpdated(updated);
      } catch (error) {
        if (!signal.aborted)
          setError(error instanceof Error ? error.message : "Could not check analysis status");
      }
    },
  ...
  async function run() {
    ...
    setError(undefined);
    try {
      const queued = await adminTopicProposalAnalyze(proposal.id, {
        headers: { "X-CSRF-Token": admin.csrf_token },
      });
      setJob({
        id: queued.id,
        status: queued.status as TopicAnalysisOut["status"],
  ...
  const tooltip = error
    ? `${error} ${active ? "Click to retry the status check." : "Click to try again."}`
    : status || (missing ? "Enrich missing information with AI" : "No missing information");
  const button = (
    <Button
      variant="outline"
    ...
    >
      {error ? <CircleAlert aria-hidden /> : <Sparkles aria-hidden />}
      {!compact && (retryStatus ? "Retry status check" : job ? "Run again" : "Run AI analysis")}
    </Button>
  );
  return (
    <div
      className={compact ? "flex shrink-0 items-center" : "space-y-3 rounded-lg border bg-card p-5"}
    >
      {!compact && (
        <>
          <h2 className="font-semibold">AI analysis</h2>
          <p className="text-sm text-muted-foreground">
            Fill missing information using web research. Check the sources before approving.
          </p>
        </>
      )}
      ...
      {!compact && disabled && !active && (
        <p className="text-xs text-muted-foreground">
          Finish or discard your edits before running AI analysis.
        </p>
      )}
      {!compact && error && (
        <div role="alert" className="max-w-64 whitespace-normal text-xs text-destructive">
          {error}
        </div>
      )}
    </div>
