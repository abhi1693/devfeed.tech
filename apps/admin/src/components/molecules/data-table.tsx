...
export function DataTable<T extends RowData>({
  ...
  const visible: Record<string, boolean> = {
    ...initialVisibility,
    ...Object.fromEntries(
      Object.entries(remembered ?? {}).filter(
        (entry): entry is [string, boolean] => typeof entry[1] === "boolean",
      ),
    ),
    selection: true,
    ...Object.fromEntries(
      columns.filter((column) => column.enableHiding === false).map((column) => [column.id!, true]),
    ),
  };
      ...
      setSelection((previous) => {
        const rows =
          typeof updater === "function"
            ? updater(previous.scope === scope ? previous.rows : {})
            : updater;
        return {
          scope,
          rows,
          allRows: Object.values(rows).some(Boolean) ? previous.allRows : undefined,
        };
      }),
    onColumnVisibilityChange: (updater) => {
      const next = typeof updater === "function" ? updater(columnVisibility) : updater;
      setVisibility({ saved, columns: next });
      if (columnChoices && persistent && settings.defaults.remember_columns)
        void saveTable(key, { columns: next }).catch((error) =>
          notifyFailure(error, "Could not save table columns"),
        );
    },
    onPaginationChange: (updater) => {
      const next = typeof updater === "function" ? updater(currentPage) : updater;
      pagination?.onChange({
        offset: String(next.pageIndex * next.pageSize),
  ...
  const selected = selection.allRows
    ? [
        ...new Map([...selection.allRows, ...data].map((row) => [getRowId(row), row])).values(),
    ...
    try {
      ...
      if (!controller.signal.aborted)
        setSelection({
          scope,
          allRows: rows,
          rows: Object.fromEntries(rows.map((row) => [getRowId(row), true])),
        });
    } catch (error) {
      if (!controller.signal.aborted) notifyFailure(error, "Could not select all matching records");
    } finally {
      if (!controller.signal.aborted) setSelectingAll(false);
    }
  }
  return (
    <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-x-2 gap-y-3">
      {(toolbar || columnChoices) && (
        <div
          className={cn(
            "flex min-w-0 flex-wrap items-center gap-3",
            !bulkSources.length && "col-span-full",
          )}
        >
          {toolbar}
          {columnChoices && (
              ...
              <PopoverContent align="end" aria-label="Table columns" className="w-56 p-2">
                ...
                <div className="max-h-72 overflow-y-auto">
                  {table
                    ...
                    .map((column) => (
                      <label
                        key={column.id}
                        className="flex cursor-pointer items-center gap-3 rounded px-2 py-2 text-sm hover:bg-muted has-disabled:cursor-default has-disabled:text-muted-foreground"
                      >
                        <Input
                          type="checkbox"
                        ...
                        />
                        {(column.columnDef.meta as ColumnStyle | undefined)?.label ??
                ...
                <div className="mt-1 border-t pt-1">
                  <Button
                    ...
                    onClick={() => table.setColumnVisibility(initialVisibility)}
                  >
                    Reset columns
                  </Button>
                </div>
      ...
      {(selectable || bulkSources.length > 0) && (
        <TableBulkActions
          key={scope}
          label={label}
          selected={selected}
          ...
          selectAllControl={
            loadAllRows &&
            !selection.allRows &&
            (pagination?.total ?? 0) > data.length &&
            table.getIsAllPageRowsSelected() && (
              <Button
                size="sm"
                variant="link"
                disabled={loading || !!error || bulkBusy}
                loading={selectingAll}
                loadingText="Loading records…"
                onClick={() => void selectAllMatching()}
              >
                Select all {pagination?.total.toLocaleString()} matching records
              </Button>
            )
          }
          getRowId={getRowId}
          getRowLabel={getRowLabel}
          disabled={loading || !!error || selectingAll}
      ...
      <div className="col-span-full overflow-hidden rounded-lg border bg-card">
        <Table aria-label={label} aria-busy={loading} className={className}>
                ...
                {group.headers.map((header) => {
                  ...
                  return (
                    <TableHead
                      ...
                      className={cn(
                        "px-3",
                        header.column.id === "actions" && "w-px whitespace-nowrap text-right",
          ...
          <TableBody inert={bulkBusy || undefined}>
            {loading || error ? (
              <TableRow>
                <TableCell
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <TableCell
                      ...
                      className={cn(
                        "px-3 py-2",
                        cell.column.id === "actions" && "w-px whitespace-nowrap text-right",
                        (cell.column.columnDef.meta as ColumnStyle | undefined)?.className,
                      )}
                    >
              ...
              <TableRow>
                <TableCell
                  colSpan={table.getVisibleLeafColumns().length}
                  className="h-36 whitespace-normal px-6 text-center text-sm"
                >
                  {empty}
                </TableCell>
              </TableRow>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t px-4 py-3 text-xs text-muted-foreground">
            <span>
              {loading
                ...
                : error
                  ...
                  : data.length
                    ? `${pagination.offset + 1}–${pagination.offset + data.length} of ${pagination.total}`
                    : pagination.total
                      ? `0 on this page · ${pagination.total} total`
                      : "0 records"}
            </span>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Select
                ...
                onChange={(value) => pagination.onChange({ limit: value, offset: "0" })}
              ...
              <Button
                variant="outline"
                size="sm"
                disabled={loading || error !== undefined || !table.getCanNextPage()}
