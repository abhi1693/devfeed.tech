...
export function TopicReplacementPicker({ topicId, ...props }: Omit<ComboboxProps, "label" | "options"> & { topicId: string }) {
  const source = useMemo<EntityPickerSource>(() => ({
    key: `topic-replacements/${topicId}`,
    list: async (params, signal) => {
      const page = await adminTopicReplacementsList({ q: params.q, offset: params.offset, limit: params.limit, exclude_topic_id: topicId }, { signal });
