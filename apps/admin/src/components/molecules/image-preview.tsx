...
export function ImagePreview({
  ...
  const imageRef = useCallback((image: HTMLImageElement | null) => {
    // Cached or server-rendered images can finish before React attaches handlers.
  ...
  return (
    <div
      className={cn(
        ...
        compact ? "size-5" : "h-48 max-h-[min(50vh,16rem)] w-full rounded bg-muted/40 sm:h-56",
        className,
      )}
    >
      {(!src || status !== "loaded") && (
        ...
        >
          <Icon
            ...
            className={cn(
              "size-4 shrink-0",
              src && status === "loading" && "animate-spin motion-reduce:animate-none",
            )}
          />
      ...
      {src && status !== "failed" && (
        <>
          ...
          <img
            ref={imageRef}
            src={src}
            alt={compact ? "Logo preview" : "Image preview"}
