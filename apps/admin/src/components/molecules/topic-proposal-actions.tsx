...
export function TopicProposalActions({
  proposal,
  onReviewed,
}: {
  ...
  async function review(next: TopicReviewDecision) {
    ...
    setError(undefined);
    try {
      await adminTopicProposalReview(
        proposal.id,
        {
          decision: next,
      ...
      notify.success(
        next === "approved"
          ? `${proposal.proposed.name} approved`
          : `${proposal.proposed.name} rejected`,
      );
      onReviewed();
    } catch (error) {
      setError({
        decision: next,
        message: error instanceof Error ? error.message : "Review failed. Try again.",
      });
  ...
  return (
    <div
      role="group"
      aria-label={`Actions for ${proposal.proposed.name}`}
      className="flex h-8 items-center justify-end gap-1"
    >
      <Tooltip>
        <TooltipTrigger asChild>
          <Button variant="outline" size="icon-sm" asChild disabled={!!decision}>
            <Link
              prefetch={false}
              href={`/taxonomy/topics/proposals/${proposal.id}`}
              aria-label={`${pending ? "Review" : "View"} ${proposal.proposed.name}`}
            >
              <Eye aria-hidden />
            </Link>
          </Button>
        </TooltipTrigger>
        <TooltipContent>{pending ? "Review and edit" : "View review"}</TooltipContent>
      </Tooltip>
      {pending && (
        <>
          <TopicAnalysisControl
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                size="icon-sm"
                aria-label={`Reject ${proposal.proposed.name}`}
                disabled={!!decision || analyzing}
            ...
            <TooltipContent>
              {error?.decision === "rejected"
                ? `${error.message} Click to try again.`
                : "Reject proposal"}
            </TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={`Approve ${proposal.proposed.name}`}
                disabled={!!decision || analyzing}
