...
export function FormField({ field, value, onChange, error, disabled, exclude }: { field: FieldSpec; value: unknown; onChange: (value: unknown) => void; error?: string; disabled?: boolean; exclude?: string }) {
  ...
  return <Field id={field.key} name={field.key} label={field.label} required={field.required} disabled={disabled} subtext={field.help} tooltip={field.tooltip} error={error}>
    {control => {
      ...
      if (field.type === "boolean") return <input {...control} type="checkbox" checked={!!value} onChange={event => onChange(event.target.checked)} className="size-4 accent-primary" />;
      ...
      if (field.type === "url") return <UrlInput {...control} value={text} onChange={event => onChange(event.target.value)} maxLength={field.max} />;
      return <Input {...control} value={text} onChange={event => onChange(event.target.value)} type={field.type === "datetime" ? "datetime-local" : field.type ?? "text"} maxLength={field.max} min={field.min} max={field.type === "number" ? field.max : undefined} />;
    }}
  </Field>;
}
