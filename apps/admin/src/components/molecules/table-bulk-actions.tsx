...
export function TableBulkActions<T>({
  ...
  onComplete,
  onBusyChange,
  selectionDescription = "on this page",
  selectAllControl,
  sources = [],
}: Props<T>) {
  ...
  async function prepare(source: BulkActionSource<T>, button: HTMLElement) {
    ...
    try {
      ...
      if (!eligible.length) {
        notify.info("No eligible records match these filters");
        return;
      }
      ...
      if (!controller.signal.aborted) notifyFailure(error, "Could not load matching records");
    } finally {
  ...
  async function execute() {
    if (
      ...
      lock.current ||
      result ||
      (pending.action.destructive && confirmation !== "DELETE")
    )
      return;
    lock.current = true;
    setBusy(true);
    ...
    async function worker() {
      while (index < pending!.rows.length) {
        ...
        try {
          ...
          successes.push(getRowId(row));
        } catch (error) {
          failures.push({
            name: getRowLabel(row),
            message: error instanceof ApiError ? error.message : "Request failed. Try again.",
          });
  ...
  return (
    <>
      {sources.length > 0 && (
        <div className="col-start-2 flex flex-wrap justify-end gap-2 self-center">
          {sources.map((source) => (
            <Button
              key={source.label}
              variant="outline"
              size="sm"
              disabled={disabled || busy || !!loadingSource}
              loading={loadingSource === source.label}
              loadingText="Loading records…"
              onClick={(event) => void prepare(source, event.currentTarget)}
            >
              {source.action.icon}
              {source.label}
            </Button>
      ...
      {selected.length > 0 && (
        <div
          ...
          className="col-span-full flex flex-wrap items-center gap-2 rounded-lg border bg-muted/40 px-3 py-2"
        >
          <span className="mr-2 text-sm font-medium" aria-live="polite">
            {selected.length} selected {selectionDescription}
          </span>
          {selectAllControl}
          {actions.map((action) => {
            ...
            return (
              <Button
                ...
                disabled={disabled || busy || !!loadingSource || !eligible.length}
      ...
      >
        <Dialog.Portal>
          ...
          <Dialog.Content
            className="fixed left-1/2 top-1/2 z-50 max-h-[85dvh] w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-lg border bg-background p-6 shadow-lg"
          ...
          >
            <Dialog.Title className="text-lg font-semibold">
              {pending?.action.label} {pending?.rows.length} selected{" "}
            ...
            {!!pending?.skipped && (
              <p className="mt-2 text-sm">
                {pending.skipped} ineligible {pending.skipped === 1 ? "row will" : "rows will"} be
                skipped.
              </p>
            )}
            <ul
              aria-label={`${label} to process`}
              className="my-4 max-h-40 space-y-1 overflow-y-auto rounded-md border p-3 text-sm"
            >
              {pending?.rows.map((row) => (
                <li className="break-words" key={getRowId(row)}>
                  {getRowLabel(row)}
                </li>
              ))}
            </ul>
            {pending?.action.destructive && !result && (
              <DeleteConfirmation value={confirmation} onChange={setConfirmation} disabled={busy} />
            )}
            {busy && (
              <p role="status" className="mt-3 text-sm">
                Processed {progress} of {pending?.rows.length}…
              </p>
            )}
            {result && (
              <div role="alert" className="mt-3 space-y-2 text-sm">
                <p>
                  {result.succeeded} completed. {result.failures.length} failed. Review the errors
            ...
            <div className="mt-5 flex flex-wrap justify-end gap-2">
              ...
              {!result && (
                <Button
                  ...
                  disabled={disabled || (pending?.action.destructive && confirmation !== "DELETE")}
