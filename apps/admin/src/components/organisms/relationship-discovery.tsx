...
export const relationshipTrail = [
  ...resourceTrail("topic-relations"),
  { label: "Topic relationships", href: "/taxonomy/relationships" },
];
...
export function RelationshipDiscovery() {
  ...
  const load = useCallback(
    (signal: AbortSignal) => (topic ? adminTopicGet(topic, { signal }) : Promise.resolve(null)),
    [topic],
  );
  ...
  async function submit() {
    ...
    setError(undefined);
    try {
      const job = await adminTopicRelationshipsAnalyze(
        topic,
        { related_topic_id: related || null },
        { headers: { "X-CSRF-Token": admin.csrf_token } },
      );
      notify.success("Relationship research queued");
      router.push(recordHref("analysis-jobs", job));
    } catch (error) {
      setError(error instanceof Error ? error : new Error("Could not start research"));
      notifyFailure(error, "Could not start relationship research");
    } finally {
      setBusy(false);
    }
  }
    ...
    <section className="min-w-0 space-y-6">
      ...
      >
        ...
        <Field
          ...
          error={
            current.data && current.data.status !== "active"
              ? "Select an active topic to research."
              : undefined
          }
        >
          {(props) => (
        ...
        <Field
          label="Related topic"
          disabled={busy}
          subtext="Optional. Leave empty to look for connections across all active topics."
        >
          {(props) => (
            <EntityPicker
              {...props}
        ...
        <div className="flex flex-wrap justify-end gap-2">
          <Button variant="outline" asChild>
            <Link href="/taxonomy/relationships">Cancel</Link>
          </Button>
          <Button
            type="submit"
            disabled={!topic || current.data?.status !== "active"}
            loading={busy}
