...
export function TopicEnrichment({ id }: { id: string }) {
  ...
  return <section className="space-y-6"><PageHeading title="Enrich topic keywords" browserTitle={adminRouteTitle({ view: "enrich", id }, result.data?.topic.name)} trail={[...resourceTrail("topics"), { label: "Topics", href: "/taxonomy/topics" }, { label: result.data?.topic.name ?? "Topic", href: `/taxonomy/topics/${id}` }]} description="Suggestions come from existing tags on at least two approved, published articles in this topic. Select useful terms, then send them for review." />
    ...
    {result.data && <><p className="text-sm text-muted-foreground">Examined {result.data.articles_examined} articles, up to the latest 200. Current keywords: {result.data.topic.keywords?.join(", ") || "None"}.</p>
