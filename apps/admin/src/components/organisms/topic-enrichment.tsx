...
export function TopicEnrichment({ id }: { id: string }) {
  ...
  const result = useRequest(`${id}/${revision}`, load, busy || selected.length ? 0 : refreshSeconds * 1000);
  ...
  return <section className="space-y-6"><PageHeading browserTitle={adminRouteTitle({ view: "enrich", id }, result.data?.topic.name)} title="Enrich topic keywords" trail={[...resourceTrail("topics"), { label: "Topics", href: "/taxonomy/topics" }, { label: result.data?.topic.name ?? "Topic", href: `/taxonomy/topics/${id}` }]} description="Suggestions come from existing tags on at least two approved, published articles in this topic. Select useful terms, then send them for review." />
    <RequestState loading={result.loading} error={result.error ?? error} retry={() => setRevision(value => value + 1)} />
    {result.data && <><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted-foreground">Examined {result.data.articles_examined} articles, up to the latest 200. Current keywords: {result.data.topic.keywords?.join(", ") || "None"}.</p><RefreshInterval value={refreshSeconds} onChange={setRefreshSeconds} loading={result.loading || result.refreshing} /></div>
