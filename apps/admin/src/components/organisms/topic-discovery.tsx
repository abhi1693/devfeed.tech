...
export function TopicDiscovery({ onComplete }: { onComplete: () => void }) {
  ...
  async function pull() {
    ...
    try {
      while (true) {
        setResume(cursor);
        const result = await adminTopicGithubPull(cursor, {
          headers: { "X-CSRF-Token": admin.csrf_token },
        });
        added += result.created;
        setCreated(added);
        setIssues((values) => [...values, ...result.issues]);
        setProgress(
          `Checked ${result.processed} of ${result.total} topics. ${added} new proposals.`,
        );
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button size="sm" aria-label="Discover topics">
          {busy ? (
            <LoaderCircle aria-hidden className="animate-spin motion-reduce:animate-none" />
          ) : (
            <Sparkles aria-hidden />
          )}
          {busy ? "Pulling topics…" : "Discover topics"}
      ...
      >
        <Button
          variant="ghost"
          aria-label={resume ? "Continue pulling" : "Pull from GitHub"}
          className="h-auto w-full justify-start px-3 py-3 text-left"
          disabled={busy}
          onClick={() => void pull()}
        >
          <GitFork aria-hidden className="size-5" />
          <span>
            <span className="block">{resume ? "Continue pulling" : "Pull from GitHub"}</span>
            <span className="mt-0.5 block text-xs font-normal text-muted-foreground">
              Curated topics from github/explore
            </span>
          </span>
        </Button>
        <p className="border-t px-3 pt-3 pb-1 text-xs leading-relaxed text-muted-foreground">
          New topics follow your research and approval settings. Existing topics are skipped. The
          first batch downloads the catalog and can take a minute.
        </p>
        {(progress || error || issues.length > 0) && (
          <div className="space-y-3 px-3 py-2 text-sm">
            ...
            {issues.length > 0 && (
              <details>
                <summary className="cursor-pointer text-xs">
                  {issues.length} topics need manual correction
                </summary>
            ...
            {created > 0 && !busy && (
              <Button variant="outline" size="sm" asChild>
                <Link href="/taxonomy/topics?view=proposals&status=pending">
                  Review imported topics
                </Link>
              </Button>
            )}
          </div>
