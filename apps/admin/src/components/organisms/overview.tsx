...
export function Overview({ initialData }: { initialData: AdminOverview }) {
  ...
  function openSection(value: OverviewSection) { document.getElementById({ sources: "source-health", personalization: "personalization", operations: "processing" }[value])?.scrollIntoView({ block: "start" }); }
  ...
  return <section className="space-y-6" aria-label="Application overview" aria-busy={loading}>
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><h1 className="text-2xl font-semibold tracking-tight">Overview</h1></div>
      <div className="flex rounded-lg border bg-muted/50 p-1" role="group" aria-label="Chart date range">
        {[7, 30, 90].map(days => <Button key={days} variant={data.days === days ? "default" : "ghost"} size="sm" aria-pressed={data.days === days} disabled={loading} className="h-8 rounded-md px-3 text-xs" onClick={() => { if (days !== data.days) void refresh(days); }}>{days} days</Button>)}
      </div>
    </div>
    {failed && <div role="alert" className="flex flex-wrap items-center gap-3 rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm"><CircleAlert aria-hidden className="size-4 shrink-0" /><span>Could not update the overview. Showing the last successful snapshot.</span><RetryButton onRetry={() => void refresh(requestedDays.current, true)} pending={loading} /></div>}
