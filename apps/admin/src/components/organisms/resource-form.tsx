...
function Editor({ resource, record }: { resource: Resource; record?: RecordData }) {
  ...
  const cancel = record ? `/${resource}/${encodeURIComponent(record.id)}` : `/${resource}`;
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      router.replace(`/${resource}/${encodeURIComponent(result.id)}`); router.refresh();
