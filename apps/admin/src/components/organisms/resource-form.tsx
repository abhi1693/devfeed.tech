...
export function ResourceForm({ resource, id }: { resource: Resource; id?: string }) {
  ...
  const load = useCallback(
    (signal: AbortSignal) => (id ? getRecord(resource, id, signal) : Promise.resolve(null)),
    [resource, id],
  );
  ...
  return (
    <>
      ...
      {!loading && !error && (
        <Editor
          key={`${resource}/${id}/${created}`}
          resource={resource}
          record={data ?? undefined}
          focusFirstField={!id && created > 0}
          onCreateAnother={() => setCreated((value) => value + 1)}
        />
      )}
    </>
  );
...
function Editor({
  ...
  const [facts, setFacts] = useState(() =>
    initialFacts(record?.facts as EditableFact[] | undefined),
  );
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      router.replace(recordHref(resource, result));
      router.refresh();
    } catch (error) {
      setError(error instanceof Error ? error : new Error("Could not save"));
      notifyFailure(error, `Could not save ${spec.singular.toLowerCase()}`);
    ...
    <section className="max-w-4xl space-y-6">
      <PageHeading
        ...
        browserTitle={adminRouteTitle(
          record ? { view: "edit", resource, id: record.id } : { view: "new", resource },
          record?.[spec.title],
        )}
        trail={[
          ...resourceTrail(resource),
          { label: spec.label, href: resourceHref(resource) },
          ...(record ? [{ label: String(record[spec.title]), href: cancel }] : []),
        ]}
        description={
          resource === "articles"
      ...
      >
            ...
            {spec.fields.map((field) => (
              ...
              >
                <FormField
                  ...
                  disabled={
                    (!!record && field.createOnly) ||
                    (resource === "tags" &&
                      field.key === "topic_id" &&
                      values.auto_link_topic === true)
                  }
                  error={error instanceof ApiError ? error.fields[field.key] : undefined}
                />
              </div>
            ))}
          </fieldset>
        ...
        <div className="flex flex-wrap justify-end gap-2 border-t pt-5">
          <Button variant="outline" asChild>
            <Link href={cancel} prefetch={false}>
              Cancel
            </Link>
          </Button>
          <Button
            ...
            loading={busy && !addingAnother}
            loadingText={resource === "sources" && !record ? "Validating feed…" : "Saving…"}
          >
            {record ? "Save changes" : `Create ${spec.singular.toLowerCase()}`}
          </Button>
          {!record && (
            <Button
              type="submit"
              ...
              disabled={busy || previewBusy}
              loading={busy && addingAnother}
              loadingText={resource === "sources" ? "Validating feed…" : "Creating…"}
            >
