...
export function OverviewCharts({ data }: { data: AdminOverview }) {
  ...
  return <section aria-label="Platform activity" className="min-w-0 overflow-hidden rounded-lg border bg-card"><div className="flex items-center gap-2 border-b px-5 py-4"><h2 className="text-base font-semibold">Activity</h2><InfoTooltip label="Activity">Daily totals for the selected {data.days} days, in UTC. Today is in progress.</InfoTooltip></div><div className="grid min-w-0 divide-y xl:grid-cols-2 xl:divide-x xl:divide-y-0">
    ...
    <section className="min-w-0 p-5"><div className="space-y-4"><div className="flex min-h-10 items-center gap-2"><h3 className="text-sm font-medium">Reader activity</h3><InfoTooltip label="Reader activity">Article preview opens, counted once per viewer, article, and hour. {activity.some(day => day.opens == null) && "Some open history is unavailable. Missing days are gaps, not zero activity."}</InfoTooltip></div><Legend items={[["Preview opens", colors[0]]]} /></div><div className="mt-4 -ml-3">{activity.some(day => day.opens) ? <ChartContainer label="Daily article preview opens" className="h-64"><LineChart data={activity} accessibilityLayer margin={{ top: 10, right: 15, bottom: 5, left: 0 }}>{axes}<Tooltip content={props => <ChartTooltip {...props} formatLabel={shortDate} />} /><Line type="linear" dataKey="opens" name="Preview opens" stroke={colors[0]} strokeWidth={2} dot={false} connectNulls={false} isAnimationActive={false} /></LineChart></ChartContainer> : <p className="flex h-64 items-center justify-center text-sm text-muted-foreground">No recorded preview opens in the available history.</p>}{activity.some(day => day.opens == null) && <span className="sr-only">Some open history is unavailable.</span>}</div></section>
  </div></section>;
}
