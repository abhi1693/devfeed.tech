...
const labels = {
  ...
  signed_out: "Connect AI",
  connected: "AI connected",
  limited: "AI usage limit",
...
export function AiConnection({ csrfToken }: { csrfToken: string }) {
  ...
  const message = request.error
    ? "Could not check the AI connection. The admin service may be unavailable."
  ...
  async function act(action: "login" | "cancel") {
    ...
    try {
      ...
      if (action === "cancel" && login)
        await adminAiLoginCancel({ login_id: login.login_id }, options);
      ...
      notifyFailure(
        error,
        action === "cancel" ? "Could not cancel sign-in" : "Could not connect ChatGPT",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    ...
    >
      <PopoverTrigger asChild>
        <Button variant="outline" size="sm" aria-label={`AI connection: ${labels[state]}`}>
          <Icon
            ...
            className={cn(
              "size-4",
              state === "connected" && "text-emerald-600",
              state === "checking" && "animate-spin motion-reduce:animate-none",
              ["unavailable", "error", "limited", "signed_out"].includes(state) && "text-amber-600",
            )}
          />
      ...
      <PopoverContent
        align="end"
        className="w-96 max-w-[calc(100vw-24px)] space-y-4 p-4"
      ...
      >
        ...
        {pending && login.user_code && login.verification_url ? (
          <div className="space-y-3 border-t pt-4">
            <label htmlFor="ai-device-code" className="block text-sm">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant="outline" loading={busy} onClick={() => void act("cancel")}>
                Cancel
              </Button>
            </div>
            <p role="status" className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              />
              Waiting for approval in ChatGPT…
            </p>
          </div>
        ) : (
          <>
            ...
            {login?.status === "completed" && connected && (
              <p role="status" className="text-sm text-emerald-700">
                ChatGPT connected.
              </p>
            )}
            {!request.error && ["signed_out", "error", "connected", "limited"].includes(state) && (
              <Button
                loading={busy}
                loadingText="Starting sign-in…"
                onClick={() => void act("login")}
              >
                <PlugZap aria-hidden />
                {connected ? "Reconnect ChatGPT" : "Connect ChatGPT"}
              </Button>
            )}
          </>
        )}
