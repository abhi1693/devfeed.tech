...
const actionChoices: Choice<"action">[] = [
  { value: "create", label: "New topic" },
  { value: "update", label: "Update topic" },
];
...
export const clearedProposalFilters = {
  q: "",
  batch_id: "",
  kind: "",
  source: "",
...
export function TopicProposalFilters({
  filters,
  q,
  batchId,
  scopeKey,
  ...
  const load = useCallback(
    (signal: AbortSignal) => adminTopicProposalFilterOptions({ signal }),
    [],
  );
  const options = useRequest(`proposal-filter-options/${revision}`, load);
  const extraCount = [filters.kind, filters.source, filters.action, filters.missing].filter(
    Boolean,
  ).length;
  const chips = [
    { key: "q", label: "Search", value: q },
    {
      key: "analysis",
      label: "AI",
      value: analysisChoices.find((choice) => choice.value === filters.analysis)?.label,
    },
    ...
    { key: "kind", label: "Kind", value: filters.kind && humanize(filters.kind) },
    {
      key: "action",
      label: "Change",
      value: actionChoices.find((choice) => choice.value === filters.action)?.label,
    },
    {
      key: "missing",
      label: "Missing",
      value: missingChoices.find((choice) => choice.value === filters.missing)?.label,
    },
    { key: "batch_id", label: "Batch", value: batchId && "Current import" },
  ].filter((chip) => chip.value);
  function select(key: FilterKey, value: string) {
    onChange({ [key]: value, offset: "0" });
  }

  return (
    <>
      <SearchField
        className="basis-full sm:basis-auto sm:flex-1 sm:max-w-sm"
        label="Search proposals"
        placeholder="Search topics, keywords, sources…"
        value={q}
        scopeKey={scopeKey ?? JSON.stringify([filters, batchId])}
        onSearch={(q) => onChange({ q, offset: "0" }, true)}
      />
      <Select
        label="AI analysis filter"
        className="w-44"
        value={filters.analysis ?? ""}
        options={analysisChoices}
        placeholder="All AI statuses"
        clearLabel="All AI statuses"
        onChange={(value) => select("analysis", value)}
      />
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            aria-label={extraCount ? `Filters (${extraCount} active)` : "Filters"}
          >
            <SlidersHorizontal aria-hidden />
            Filters
            {extraCount > 0 && (
              <span className="rounded bg-muted px-1.5 text-xs tabular-nums">{extraCount}</span>
            )}
          </Button>
        </PopoverTrigger>
        <PopoverContent
          aria-label="Proposal filters"
          className="w-80 max-w-[calc(100vw-1.5rem)] space-y-4 p-4"
        >
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold">Filter proposals</h2>
            <Button
              variant="ghost"
              size="icon-sm"
          ...
          <Field label="Kind">
            {(control) => (
              <Combobox
                ...
                label="Kind"
                value={filters.kind ?? ""}
                options={(options.data?.kinds ?? []).map((value) => ({
                  value,
          ...
          <Field label="Change">
            {(control) => (
              <Select
                ...
                clearLabel="All changes"
                onChange={(value) => select("action", value)}
              />
            )}
          </Field>
          <Field label="Missing information">
            {(control) => (
              <Select
                ...
                options={missingChoices}
                placeholder="All proposals"
                clearLabel="All proposals"
                onChange={(value) => select("missing", value)}
              />
            )}
          </Field>
        </PopoverContent>
      ...
      {chips.length > 0 && (
        <div
          aria-label="Active proposal filters"
          className="order-last flex w-full flex-wrap items-center gap-2"
        >
          {chips.map((chip) => (
            <Button
              key={chip.key}
              variant="outline"
              size="xs"
              className="max-w-full rounded-full bg-muted/40 font-normal"
          ...
          <Button variant="ghost" size="xs" onClick={() => onChange(clearedProposalFilters)}>
            Clear filters
          </Button>
        </div>
      )}
