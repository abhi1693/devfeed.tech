...
const previewColumns: DataTableColumn<TopicImportPreview["rows"][number]>[] = [
  { accessorKey: "row", header: "Row" },
  {
    ...
    cell: ({ row: { original } }) => (
      <>
        {original.topic?.name ?? "Invalid row"}
        <p className="text-xs text-muted-foreground">{original.topic?.slug}</p>
      </>
    ),
  },
  { accessorKey: "action", header: "Change", meta: { className: "capitalize" } },
  {
    id: "issues",
    header: "Review notes",
    meta: { className: "whitespace-normal min-w-48" },
    cell: ({ row: { original } }) =>
...
export function TopicImport() {
  ...
  const [body, setBody] = useState<ImportBody>({
    format: "json",
  ...
  function change(values: Partial<ImportBody>) {
    setBody((previous) => ({ ...previous, ...values }));
  ...
  async function fileSelected(file?: File) {
    ...
    try {
      ...
      setError(error instanceof Error ? error : new Error("Could not read file"));
    } finally {
      setBusy(false);
    }
  }
  async function inspect(event: React.FormEvent) {
    ...
    try {
      setPreview(
        await adminTopicImportPreview(body, { headers: { "X-CSRF-Token": admin.csrf_token } }),
      );
    } catch (error) {
      setError(error instanceof Error ? error : new Error("Could not preview import"));
      notifyFailure(error, "Could not preview import");
    } finally {
      setBusy(false);
    }
  }
  async function submit() {
    ...
    setError(undefined);
    try {
      const result = await adminTopicImportSubmit(
        { ...body, preview_token: preview.preview_token },
        { headers: { "X-CSRF-Token": admin.csrf_token } },
      );
      notify.success(`${result.length} topic proposals awaiting review`);
      router.push(
        `/taxonomy/topics?view=proposals&batch_id=${encodeURIComponent(result[0].batch_id)}`,
      );
    } catch (error) {
      setError(error instanceof Error ? error : new Error("Could not submit import"));
      notifyFailure(error, "Could not submit import");
      setPreview(undefined);
      setBusy(false);
    }
  }
      ...
      <form onSubmit={inspect} className="space-y-5 rounded-lg border bg-card p-6">
        <fieldset disabled={busy} className="space-y-5">
          <div className="grid gap-5 sm:grid-cols-2">
            <Field name="file" label="Choose a file" disabled={busy}>
              {(control) => (
                <Input
                  {...control}
                  type="file"
            ...
            <Field name="format" label="Format" required disabled={busy}>
              {(control) => (
                <Select
                  {...control}
                  label="Format"
                  value={body.format}
                  onChange={(value) => change({ format: value as ImportBody["format"] })}
                  options={[
          ...
          <FormField
            field={{ key: "source_name", label: "Source name", required: true, max: 200 }}
            value={body.source_name}
            onChange={(value) => change({ source_name: String(value) })}
            disabled={busy}
          />
          <Field
            name="content"
            ...
            required
            disabled={busy}
            subtext="Up to 100 rows and 256 KiB. Required fields: name, slug, and kind. Optional: description, aliases, keywords, website_url, logo_url, and facts. CSV aliases and keywords use | between terms; facts use a JSON array. Omitted fields preserve existing values; empty optional fields clear them."
          >
            {(control) => (
              <Textarea
                {...control}
          ...
          <div className="flex flex-wrap justify-between gap-3">
            <Button
              variant="ghost"
              onClick={() =>
                change({ format: "json", source_name: "example.json", content: example })
              }
            >
              Load example
            </Button>
            <Button type="submit" loading={busy} loadingText="Checking…">
              Preview import
            </Button>
          </div>
        </fieldset>
        ...
        <section className="space-y-4" aria-label="Import preview">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Existing slugs update their topics. Unchanged rows are skipped. Names, slugs, and
              aliases must identify a single topic.
            </p>
          </div>
          <DataTable
          ...
          {!preview.can_submit && (
            <p role="status" className="text-sm">
              Resolve the listed issues, or supply changed topics, then preview again.
            </p>
          )}
          <div className="flex justify-end gap-2">
            <Button variant="outline" asChild>
              <Link href="/taxonomy/topics">Cancel</Link>
            </Button>
            <Button
              onClick={submit}
              disabled={!preview.can_submit}
              loading={busy}
