...
export function SourcePublicationPolicy({ id, mode, revision, approved, fullAutomation = false }: { id: string; mode: PublicationPolicyUpdate["mode"]; revision: number; approved: boolean; fullAutomation?: boolean }) {
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      const result = await adminSourcePublicationPolicy(id, { mode: selected, expected_revision: current.revision }, { headers: { "X-CSRF-Token": admin.csrf_token } });
      setCurrent({ mode: result.publication_policy ?? "manual", revision: result.publication_policy_revision ?? 0 });
  ...
  return <InfoPanel title="Automatic publication">
    ...
    <div className="flex flex-wrap gap-2"><select id={`publication-policy-${id}`} value={selected} disabled={busy || !approved} onChange={event => setSelected(event.target.value as PublicationPolicyUpdate["mode"])} className="h-9 rounded-md border bg-background px-3 text-sm">
      <option value="manual">Manual review</option><option value="preview">Preview automatic decisions</option><option value="auto" disabled={current.mode === "manual"}>Publish eligible articles automatically</option>
    </select><Button disabled={busy || !approved || selected === current.mode} loading={busy} onClick={() => void save()}>Save publication mode</Button></div>
    <p className="mt-3 text-xs text-muted-foreground">{!approved ? "Approve this source to configure automation." : current.mode === "manual" ? "Start with preview before enabling automatic publication." : `Policy revision ${current.revision}. Existing articles can be evaluated from the overview.`}</p>
  </InfoPanel>;
}
