...
import {
  ...
  adminSourceReview,
  adminSourceFetch,
  adminTopicRelationshipsAnalyze,
...
import {
  relationshipTableColumns,
  relationshipProposal as proposalFor,
} from "@/components/molecules/relationship-table-columns";
export function RecordTable({
  ...
  loading?: boolean;
  error?: Error;
  loadAllRows?: (signal: AbortSignal) => Promise<RecordData[]>;
  loadFailedRows?: (signal: AbortSignal) => Promise<RecordData[]>;
}) {
  const admin = useAdmin();
  const spec = resources[resource];
  const options = { headers: { "X-CSRF-Token": admin.csrf_token } };
  const bulkActions: BulkAction<RecordData>[] = [];
  const retryAction: BulkAction<RecordData> = {
    id: "retry",
    label: "Retry",
    icon: <RotateCcw aria-hidden />,
    description:
      "Queue retries for the selected unresolved failures. Jobs that can no longer be retried will be listed with a reason.",
    eligible: (row) => row.status === "failed" && row.retryable === true,
    run: (row) => retryRecordJob(resource, row, admin.csrf_token),
  };
  ...
  if (resource === "articles" || resource === "sources") {
    for (const decision of ["approve", "reject"] as const)
      bulkActions.push({
        ...
        icon: decision === "approve" ? <Check aria-hidden /> : <X aria-hidden />,
        ...
        run: (row) =>
          resource === "articles"
            ? adminArticleReview(
                row.id,
                { action: decision, expected_revision: Number(row.editorial_revision) },
                options,
              )
            : adminSourceReview(
                row.id,
  ...
  if (resource === "sources")
    bulkActions.push({
      ...
      description: "Request a feed fetch for each selected approved source.",
      eligible: (row) => row.approval_status === "approved",
      run: (row) => adminSourceFetch(row.id, options),
    });
  if (resource === "topics")
    bulkActions.push({
      id: "relationships",
      ...
      description:
        "Research relationships for each selected active topic using the internet. Suggestions appear in the relationships table for approval.",
      eligible: (row) => row.status === "active",
      run: (row) => adminTopicRelationshipsAnalyze(row.id, {}, options),
    });
  if (resource === "topic-relations") {
    for (const decision of ["approved", "rejected"] as const)
      bulkActions.push({
        ...
        icon: decision === "approved" ? <Check aria-hidden /> : <X aria-hidden />,
        description: `${decision === "approved" ? "Approve" : "Reject"} the selected pending relationship suggestions.`,
        eligible: (row) =>
          row.status === "pending" &&
          !!proposalFor(row) &&
          (decision !== "approved" || !!proposalFor(row)?.can_approve),
        run: (row) => {
          const proposal = proposalFor(row)!;
          return adminRelationshipProposalReview(
            proposal.id,
  ...
  if (!spec.readonly)
    bulkActions.push({
      ...
      description:
        resource === "topics"
          ? "Permanently delete the selected topics, unpublish their linked articles, and remove article and tag topic links, relationships, relationship proposals and relationship research runs. Articles are kept and will need review before publication. To relink articles to another topic, use the individual topic’s delete form."
          : resource === "sources"
            ? "Permanently delete the selected sources and their links, follows, review history, and all source jobs, including queued and running jobs. Articles are kept; they remain visible in the public feed only if another approved source is linked. This cannot be undone."
            : `Permanently delete the selected ${spec.label.toLowerCase()}. Linked content or active jobs may prevent deletion. ${resource === "articles" ? "Completed jobs, evidence, classifications and review history will also be removed. Feeds may ingest these articles again. " : ""}This cannot be undone.`,
  ...
  return (
    <DataTable
      toolbar={toolbar}
      columnChoices
      preferenceKey={`${resource}${topicId ? "-related" : ""}`}
      label={spec.label}
      data={page.items}
      columns={columns}
      ...
      getRowLabel={(row) =>
        resource === "topic-relations" && row.topic_name && row.related_topic_name
          ? relationshipLabel(row as unknown as RelationshipProposalOut)
          : String(row[spec.title] || row.id)
      }
      ...
      onRetry={onRefresh}
      bulkSources={
        jobKinds[resource] && loadFailedRows
          ? [
              {
                ...
                action: {
                  ...retryAction,
                  description:
                    "Queue retries for unresolved failures matching the current search and filters across all pages. Previous failures with a newer run are excluded. Jobs that can no longer be retried will be listed with a reason.",
                },
                loadRows: loadFailedRows,
              },
            ]
          : []
      }
      sort={sort}
      onSortChange={(value) => onChange({ sort: value || spec.defaultSort, offset: "0" })}
      ...
      empty={
        <span className="text-muted-foreground">
          {resource === "analysis-jobs"
            ...
            : resource === "topic-relations"
              ? "No relationships or pending suggestions match these filters."
              : `No ${spec.label.toLowerCase()} match these filters.`}
        </span>
      }
    />
  );
