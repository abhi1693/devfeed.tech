...
export function ResourceDelete({ resource, id }: { resource: Resource; id: string }) {
  ...
  return <section className="max-w-2xl space-y-6">
    ...
    {result.data && <form onSubmit={remove} className="space-y-5 rounded-lg border border-destructive/30 bg-card p-6">
      ...
      <p className="text-sm text-muted-foreground">{resource === "topics" ? "This permanently deletes the topic. Linked articles are kept, unpublished, and require review before publishing again." : "This permanently deletes this record. It cannot be undone. Linked content or active jobs can prevent deletion. Disable or unpublish records when you want to retain their history."}</p>
