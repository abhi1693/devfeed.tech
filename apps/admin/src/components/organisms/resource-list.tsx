...
export function ResourceList({ resource, analysisType }: { resource: Resource; analysisType?: AnalysisType }) {
  ...
  if (resource === "topics" && search.get("status") === "proposed") {
    search.set("view", "proposals"); search.set("status", "pending");
    const sort = search.get("sort");
    if (sort === "name" || sort === "-name") search.set("sort", sort.replace("name", "slug"));
  }
  return resource === "topics" ? <TopicsScreen query={search.toString()} /> : <ResourceListTable resource={resource} analysisType={analysisType} query={search.toString()} />;
}
...
function TopicsScreen({ query }: { query: string }) {
  ...
  function href(next: string) {
    ...
    params.set("offset", "0");
    if (next === "catalog") {
      for (const key of ["view", "status", "sort", "batch_id", "kind", "source", "action", "analysis", "missing"]) params.delete(key);
    } else {
      params.set("view", "proposals"); params.set("status", next);
      if (!proposals) params.delete("sort");
    }
    return `/taxonomy/topics?${params}`;
  }
  return <section className="min-w-0 space-y-6">
    <PageHeading title="Topics" trail={resourceTrail("topics")} description="Manage the topic catalog and track proposals from research to decision.">
      <Button variant="outline" size="sm" asChild><Link href="/taxonomy/topics/import">Import</Link></Button>
      <TopicDiscovery onComplete={() => setRevision(value => value + 1)} />
      <Button size="sm" asChild><Link href="/taxonomy/topics/new"><Plus aria-hidden />Add topic</Link></Button>
    </PageHeading>
    <nav aria-label="Topic views" className="flex gap-6 overflow-x-auto border-b">
      {([ ["catalog", "Topics"], ["pending", "Proposed"], ["approved", "Approved"], ["rejected", "Rejected"] ] as const).map(([value, label]) => {
        const selected = value === "catalog" ? !proposals : proposals && status === value;
        return <Link key={value} href={href(value)} prefetch={false} scroll={false} aria-current={selected ? "page" : undefined}
          className={`shrink-0 border-b-2 px-1 pb-3 text-sm ${selected ? "border-primary font-medium" : "border-transparent text-muted-foreground hover:text-foreground"}`}>{label}</Link>;
      })}
    </nav>
    {proposals ? <TopicProposalsPanel query={query} refreshRevision={revision} /> : <ResourceListTable resource="topics" query={query} embedded refreshRevision={revision} />}
  </section>;
}

function ResourceListTable({ resource, analysisType, query, embedded = false, refreshRevision = 0 }: { resource: Resource; analysisType?: AnalysisType; query: string; embedded?: boolean; refreshRevision?: number }) {
  const refreshSeconds = useRefreshInterval();
  const spec = resources[resource];
  const router = useRouter(); const search = new URLSearchParams(query);
  const [revision, setRevision] = useState(0);
  ...
  const { data, error, loading } = useRequest(`${resource}/${analysisType ?? "all"}?${query}/${revision}/${refreshRevision}`, load, refreshSeconds * 1000);
  function change(values: Record<string, string>, replace = false) { const params = new URLSearchParams(query); for (const [key, value] of Object.entries(values)) { if (value) params.set(key, value); else params.delete(key); } const kind = Object.hasOwn(values, "analysis_type") ? values.analysis_type as AnalysisType || undefined : analysisType; params.delete("analysis_type"); if (resource === "topics" && params.get("status") === "proposed") { params.set("status", "pending"); params.set("view", "proposals"); params.delete("sort"); } router[replace ? "replace" : "push"](`${resourceHref(resource, kind)}?${params}`, { scroll: false }); }
  return <section className="min-w-0 space-y-6">
    {!embedded && <PageHeading trail={resourceTrail(resource)} title={spec.label} browserTitle={adminRouteTitle({ view: "list", resource, analysisType })} description={spec.description}>
      {resource === "topic-relations" ? <RelationshipAddMenu /> : !spec.readonly && <Button size="sm" asChild><Link href={`${resourceHref(resource)}/new`} prefetch={false}><Plus />Add {spec.singular.toLowerCase()}</Link></Button>}
    </PageHeading>}
