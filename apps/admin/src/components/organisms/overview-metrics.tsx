...
function comparison(metric: OverviewMetric, isDuration = false) {
  ...
  if (isDuration)
    return `${duration(Math.abs(difference))} ${difference > 0 ? "slower" : "faster"} than previous period`;
...
function Sparkline({
  ...
  return (
    <div className="h-10 w-28 shrink-0" role="group" aria-label={`${label} daily trend`}>
        ...
        <LineChart data={rows} accessibilityLayer margin={{ top: 4, bottom: 4, left: 4, right: 4 }}>
          <Tooltip
            ...
            content={({ active, payload }) => {
              ...
              return (
                <div className="relative z-50 w-56 rounded-md border bg-popover px-3 py-2 text-xs text-popover-foreground shadow-md">
  ...
  const metrics = [
    {
      ...
      metric: insight.publications!,
      values: days.map((day) => day.published),
      info: "Articles first published to the feed during this period.",
    },
    {
      label: "Original article clicks",
      metric: insight.opens!,
      values: days.map((day) => day.opens),
      info: "Clicks to the original article, counted once per viewer, article, and hour. Unavailable history is not counted as zero.",
    },
    {
      label: "New accounts",
      metric: insight.accounts!,
      values: days.map((day) => day.accounts),
      info: "Accounts created during this period.",
    },
    {
      label: "Median publication time",
      metric: insight.publication_seconds!,
      values: days.map((day) => day.median_publication_seconds),
      info: `Time from discovery to first publication. 90th percentile: ${duration(insight.publication_p90_seconds)}.`,
      duration: true,
    },
  ];
  return (
