...
const icons: Partial<Record<Resource, typeof FileText>> = {
  users: Users,
  articles: FileText,
  sources: Rss,
  topics: Shapes,
  tags: Tags,
  "topic-relations": GitBranch,
};
export function Sidebar() {
  const pathname = usePathname();
  const [open, setOpen] = useState(false);
  const linkClass = (active: boolean) =>
    `flex items-center gap-2.5 rounded-md px-3 py-2 text-sm ${active ? "bg-accent font-medium text-foreground" : "text-muted-foreground hover:bg-accent hover:text-foreground"}`;
  return (
    <>
          ...
          {["Content", "Taxonomy", "Audience", "Operations"].map((group) => (
            <section key={group} className="mt-6" aria-label={group}>
              ...
              <ul className="space-y-0.5">
                ...
                {resourceKeys
                  .filter((key) => resources[key].group === group)
                  .map((key) => {
                    const Icon = icons[key] ?? Activity;
                    const active =
                      pathname === resourceHref(key) ||
                      pathname.startsWith(`${resourceHref(key)}/`);
                    return (
                      <li key={key}>
                        <Link
