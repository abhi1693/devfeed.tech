...
function Related({ resource, record }: { resource: Resource; record: RecordData }) {
  ...
  return <div className="space-y-8">
    ...
    {resource === "topics" && <><Button variant="outline" asChild><Link href={`/taxonomy/relationships/proposals?topic_id=${encodeURIComponent(id)}`}>Review relationship proposals</Link></Button><RelatedRecords resource="analysis-jobs" filter={{ topic_id: id }} /><RelatedRecords resource="articles" filter={{ topic_id: id }} /><RelatedRecords resource="topic-relations" filter={{ topic_id: id }} /><RelatedRecords resource="tags" filter={{ topic_id: id }} /></>}
