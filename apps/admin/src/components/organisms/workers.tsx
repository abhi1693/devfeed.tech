...
const resourcesByKind: Record<string, Resource> = {
  ...
  images: "image-jobs",
  "source-enrichment": "source-jobs",
  analysis: "analysis-jobs",
  "topic-analysis": "analysis-jobs",
  "research-verification": "analysis-jobs",
...
const queueLinks: Record<string, { label: string; href: string }[]> = {
  ...
  relationships: [
    { label: "Research runs", href: "/jobs/analysis/topics" },
    { label: "Relationship proposals", href: "/taxonomy/relationships/proposals" },
  ],
...
function jobHref(job: WorkerJob, logs = false) {
  ...
  return resource && job.id
    ? recordHref(
        resource,
        { id: job.id, kind: job.kind === "research-verification" ? "topic-analysis" : job.kind },
        logs ? "logs" : "details",
      )
    : undefined;
}
  ...
  return (
    <div className="space-y-1">
      <Link href={href} className={linkClass}>
        {job.target_name ?? `Run ${job.id?.slice(0, 8)}`}
      </Link>
      <p className="text-xs text-muted-foreground">
        {humanize((job.kind ?? "Job").replaceAll("-", " "))} · {duration(job.elapsed_seconds)}
      </p>
    </div>
  );
}
...
function ReadError({ error, stale }: { error?: Error; stale: boolean }) {
  ...
  return (
    ...
    >
      ...
      {stale
        ? "Showing the last successful snapshot; it may be out of date."
...
function useSnapshot() {
  ...
  const load = useCallback(
    (signal: AbortSignal) => {
      void revision;
      return adminWorkersSnapshot({ signal });
    },
    [revision],
  );
  return {
    ...useRequest("workers", load, seconds * 1000),
    seconds,
    ...
    <div className="overflow-hidden rounded-lg border bg-card">
      <Table aria-label="Workers">
        <TableHeader className="bg-muted/40">
          <TableRow>
            {["Worker", "Status", "Current work", "Completed", "Failed"].map((label) => (
              <TableHead className="px-4" key={label}>
                {label}
              </TableHead>
            ))}
          </TableRow>
          ...
          {workers.map((worker) => (
            <TableRow key={worker.name}>
              <TableCell className="px-4 py-3">
                ...
                {worker.hostname && (
                  <p className="mt-1 max-w-80 whitespace-normal break-all text-xs text-muted-foreground">
                    {worker.hostname}
                  </p>
                )}
              </TableCell>
              ...
              <TableCell className="max-w-80 whitespace-normal px-4">
                {worker.current_job ? (
                  <JobLink job={worker.current_job} />
                ) : (
              ...
              <TableCell className="px-4 tabular-nums">
                {worker.completed_executions.toLocaleString("en")}
              </TableCell>
              <TableCell className="px-4 tabular-nums">
                {worker.failed_executions ? (
                  <span className="text-destructive">
                    {worker.failed_executions.toLocaleString("en")}
                  </span>
                ) : (
...
export function WorkersOverview() {
  ...
  const visible = workers.filter(
    (worker) =>
      (!queue || worker.queues.includes(queue)) &&
      (!state || worker.state === state) &&
      `${workerLabel(worker)} ${worker.name} ${worker.hostname ?? ""} ${worker.current_job?.target_name ?? ""}`
        .toLowerCase()
        .includes(query.toLowerCase()),
  );
  return (
    <section className="space-y-6" aria-busy={loading || refreshing}>
      <PageHeading title="Workers" description="See what is running and open a worker for details.">
        <Link href="/queues" className={linkClass}>
          Queues
        </Link>
        <Refresh onClick={refresh} loading={loading || refreshing} />
      </PageHeading>
      ...
      {data && (
        <>
          ...
          />
          {data.ai_cooldown_seconds > 0 && (
            <p role="status" className="rounded-lg border p-4 text-sm">
              New AI work is waiting for a provider cooldown: {duration(data.ai_cooldown_seconds)}{" "}
              remaining. Work already running can finish.
            </p>
          )}
          <div className="flex flex-wrap gap-3">
            <Input
              aria-label="Search workers"
              placeholder="Search worker, host, or current subject…"
              className="max-w-md"
              value={query}
            ...
            <Select
              ...
              options={["busy", "idle", "suspended", "started", "offline", "unknown"].map(
                (value) => ({ value, label: humanize(value) }),
              )}
            />
          </div>
          {visible.length ? (
            <WorkerTable workers={visible} />
          ) : (
            <p className="rounded-lg border p-6 text-sm">
              {workers.length
                ? "No workers match these filters."
                : "No workers are registered. Check that the worker services are running."}
            </p>
          )}
          <p className="text-xs text-muted-foreground">
            Counts cover this worker session. Open a run to check its result.
          </p>
          <p className="text-xs text-muted-foreground" role="status">
            Updated <DateTime value={data.generated_at} /> ·{" "}
            {seconds ? `Refreshes every ${seconds}s` : "Automatic refresh is off"}
          </p>
        </>
      )}
    </section>
...
export function WorkerDetails({ name }: { name: string }) {
  ...
  const load = useCallback(
    (signal: AbortSignal) => {
      void revision;
      return adminWorkerGet(name, { signal });
    },
    [name, revision],
  );
  const {
    ...
    error,
    refreshing,
  } = useRequest(`worker/${name}`, load, seconds * 1000);
  ...
  return (
    <section className="space-y-6" aria-busy={loading || refreshing}>
      <PageHeading
        title={worker ? workerLabel(worker) : name.length > 28 ? "Worker" : name}
        browserTitle={`${worker ? workerLabel(worker) : "Worker"} · Workers`}
        trail={[{ label: "Workers", href: "/workers" }]}
      >
        <Refresh
      ...
      {worker && (
        <>
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <StatusBadge value={worker.state} />
            <span className="text-muted-foreground">
              {worker.role === "ai" ? "AI" : humanize(worker.role)} ·{" "}
              {duration(worker.uptime_seconds)} uptime
            </span>
          </div>
          <Card>
            <CardHeader>
              ...
              {job ? (
                <>
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    {job.outcome && <StatusBadge value={job.outcome} />}
                    {href && (
                      <Link href={href} className={linkClass}>
                        Run details and saved results
                      </Link>
                    )}
                    {target && (
                      <Link href={target} className={linkClass}>
                        Open {job.proposal_id ? "topic proposal" : "subject"}
                      </Link>
                    )}
                    {jobHref(job, true) && (
                      <Link href={jobHref(job, true)!} className={linkClass}>
                        Job logs
                      </Link>
                    )}
                  </div>
                  {job.kind === "research-verification" && (
                    <p className="text-sm text-muted-foreground">
                      Checking evidence before approval.
                    </p>
                  )}
                </>
              ) : (
                <p className="text-sm text-muted-foreground">
                  {worker.state === "suspended"
                    ? "This worker is suspended. Check AI connection status and the queue cooldown before expecting new work."
          ...
          {job?.id && href && (
            <details className="rounded-lg border bg-card">
              <summary className="cursor-pointer px-5 py-4 text-sm font-medium">Live logs</summary>
              <div className="border-t p-4">
                <JobLogs
                  kind={
                    (job.kind === "research-verification"
                      ? "topic-analysis"
                      : job.kind) as Parameters<typeof JobLogs>[0]["kind"]
                  }
                  id={job.id}
                  followExecution={
                    job.kind === "research-verification" &&
          ...
          <details className="rounded-lg border bg-card">
            ...
            <div className="space-y-4 border-t px-5 py-4">
              <p className="break-all font-mono text-xs text-muted-foreground">{worker.name}</p>{" "}
              <p className="mb-4 text-xs text-muted-foreground">
                Queues: {worker.queues.map(humanize).join(", ")}
              </p>
              <dl className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ["Host", worker.hostname ?? "Unknown"],
                  ["Process", worker.pid ?? "Unknown"],
                  ["Uptime", duration(worker.uptime_seconds)],
                  ["Heartbeat age", duration(worker.heartbeat_age_seconds)],
                  ["Completed executions", worker.completed_executions],
                  ["Failed executions", worker.failed_executions],
                  ["Time processing", duration(worker.working_seconds)],
                  [
                    "Registration expires in",
                    duration(Math.max(0, worker.registration_ttl_seconds)),
                  ],
                ].map(([label, value]) => (
                  <div key={label}>
              ...
              {worker.last_heartbeat && (
                <p className="mt-4 text-xs text-muted-foreground">
                  Last heartbeat <DateTime value={worker.last_heartbeat} />
                </p>
              )}
              <p className="text-xs text-muted-foreground">
                Execution counters reset with each registration. Logs have limited retention.
              </p>
            </div>
          </details>
        </>
...
export function QueuesOverview() {
  ...
  return (
    <section className="space-y-6" aria-busy={loading || refreshing}>
      <PageHeading
        title="Queues"
        description="Work waiting to be picked up, and what is running now."
      >
      ...
      <ReadError error={error} stale={!!data} />
      {loading && <p role="status">Loading queues…</p>}
      {data && (
        <>
          {data.ai_cooldown_seconds > 0 && (
            <p role="status" className="rounded-lg border p-4 text-sm">
              AI provider cooldown: {duration(data.ai_cooldown_seconds)} remaining.
            </p>
          )}
            ...
            {data.queues.map((queue) => (
              <Card key={queue.name} id={queue.name} className="scroll-mt-6">
                ...
                <CardContent className="space-y-5">
                  {!!queue.queued && !queue.registered_workers && (
                    <p role="status" className="text-sm text-destructive">
                      Waiting work has no registered workers.
                    </p>
                  )}
                  ...
                  <details className="border-t pt-3">
                    ...
                    <dl className="mt-3 grid grid-cols-3 gap-4">
                      {[
                        ...
                        ["Review required", queue.review_required],
                      ].map(([label, value]) => (
                        <div key={label}>
                          <dt className="text-xs text-muted-foreground">{label}</dt>
                    ...
                    <p className="mt-3 text-xs text-muted-foreground">
                      ...
                      {queue.oldest_queued_at ? (
                        <DateTime value={queue.oldest_queued_at} />
                      ) : (
                        "None"
                      )}
                    </p>
                  ...
                  <div className="flex flex-wrap gap-4 text-sm">
                    {queueLinks[queue.name]?.map((link) => (
