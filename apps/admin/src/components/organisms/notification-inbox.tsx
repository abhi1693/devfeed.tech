...
export function NotificationInbox({ csrfToken }: { csrfToken: string }) {
  ...
  useEffect(() => {
    ...
    return runWhenPageActive((signal) => {
      if (complete) return;
      void adminNotificationConfig({ signal })
        .then((value) => {
          if (!signal.aborted) {
            setConfig(value);
            setError(false);
  ...
  if (config?.enabled && config.environment && config.subscriber_id) {
    return (
      <ConnectedInbox
        key={`${config.environment}:${config.subscriber_id}:${csrfToken}`}
        config={config}
        csrfToken={csrfToken}
      />
    );
...
function ConnectedInbox({ config, csrfToken }: { config: NotificationConfig; csrfToken: string }) {
  ...
  return (
    <SharedInbox
      client={client}
      prepare={inheritNotificationPreferences}
      showBadge={settings.notifications.show_badge}
      ...
      tabs={[
        ...
        {
          label: "Attention",
          filter: (item) => ["warning", "error"].includes(String(item.payload.severity)),
        },
      ]}
      actionUrl={inboxAction}
