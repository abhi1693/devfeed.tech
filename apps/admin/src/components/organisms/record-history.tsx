...
export function RecordHistory({ resource, id }: { resource: "articles" | "sources"; id: string }) {
  ...
  const load = useCallback(
    (signal: AbortSignal) =>
      resource === "articles"
        ? adminArticleReviews(id, { offset, limit: 20 }, { signal })
        : adminSourceReviews(id, { offset, limit: 20 }, { signal }),
    [resource, id, offset],
  );
  const result = useRequest<PageArticleReviewOut | PageSourceReviewOut>(
    `${resource}/${id}/${offset}`,
    load,
  );
  return (
      ...
      {result.data && (
        <>
          <p className="text-xs text-muted-foreground">
            {result.data.total} decisions. History is read-only.
          </p>
              ...
              <li key={item.id} className="space-y-2 py-4 text-sm">
                <div className="flex flex-wrap justify-between gap-2">
                  <strong className="capitalize">
                    {"action" in item ? item.action : item.decision}
                  </strong>
                  <DateTime value={item.created_at} />
                </div>
                <p className="break-all text-xs text-muted-foreground">
                  Actor: {item.actor ?? "Not recorded"}
                  {"revision" in item ? ` · Revision ${item.revision}` : ""}
                </p>
                {item.note && <DataValue value={item.note} />}
              </li>
            ))}
          </ol>
