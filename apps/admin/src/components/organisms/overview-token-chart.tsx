    ...
    <section aria-label="AI token usage" className="min-w-0 border-t pt-5">
      ...
      <ul aria-label="Token totals by job type" className="mb-5 flex flex-wrap gap-x-8 gap-y-3">
        {totals.map((item) => (
          <li key={item.key} className="flex items-center gap-2 text-sm">
            ...
            <strong className="ml-1 font-medium tabular-nums">
              {formatCompactCount(item.total)}
            </strong>
            <InfoTooltip label={`${item.label} token total`}>
              {number(item.total)} reported tokens in this period.
            </InfoTooltip>
          </li>
        ))}
      </ul>
      {totals.some((item) => item.total > 0) ? (
              ...
              content={(props) => {
                ...
                return (
                  <div>
                    ...
                    />
                    <div className="rounded-b-lg border border-t-0 bg-popover px-3 py-2 text-xs text-popover-foreground">
                      <p className="font-medium">Total: {formatCompactCount(total)} tokens</p>
                      <p className="mt-1 text-muted-foreground">
                        {formatCompactCount(day.reported_runs ?? 0)} jobs with usage
                        {day.unreported_runs
                          ? ` · ${formatCompactCount(day.unreported_runs)} without usage`
                          : ""}
                      </p>
                    </div>
                  </div>
        ...
        <p className="flex h-48 items-center justify-center text-sm text-muted-foreground">
          {missing
            ? "Token usage is unavailable for finished jobs in this period."
            : "No reported AI token usage in this period."}
        </p>
      )}
    </section>
  );
