      ...
      <CardContent className="space-y-5">
        ...
        {enabled ? (
          <Button asChild className="w-full">
            ...
            <a
              href={
                freshSignIn
                  ? "/api/v1/admin/auth/login?reauthenticate=true"
                  : "/api/v1/admin/auth/login"
              }
            >
              {error ? "Sign in again" : "Sign in with your organization"}
            </a>
          </Button>
        ) : (
        ...
        {enabled && error && (
          <p className="text-xs text-muted-foreground">
            Try again after your access is updated, or use another account. A fresh sign-in will be
            requested.
          </p>
        )}
        <p className="text-xs text-muted-foreground">
