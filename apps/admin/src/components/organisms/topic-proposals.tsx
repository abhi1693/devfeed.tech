...
import {
  TopicAnalysisControl,
  analysisActive,
} from "@/components/molecules/topic-analysis-control";
...
import {
  TopicProposalFilters,
  readProposalFilters,
  clearedProposalFilters,
} from "@/components/organisms/topic-proposal-filters";
...
export function TopicProposalsPanel({
  query,
  refreshRevision = 0,
}: {
  query: string;
  ...
  const limit = [10, 25, 50, 100].includes(Number(search.get("limit")))
    ...
    : 25;
  const sort = ["slug", "-slug", "created_at", "-created_at"].includes(search.get("sort") ?? "")
    ? search.get("sort")!
    : "-created_at";
  ...
  const load = useCallback(
    ...
    [status, batchId, q, kind, source, action, analysis, missing, sort, offset, limit],
  );
  ...
  return (
    <TopicProposalsTable
      toolbar={
        <TopicProposalFilters
          filters={{ kind, source, action, analysis, missing }}
          q={q}
          batchId={batchId}
      ...
      onChange={change}
      onRetry={refresh}
      onClearFilters={() => change(clearedProposalFilters)}
    />
  );
}
...
export function TopicProposalReview({ id }: { id: string }) {
  ...
  return (
    <>
      ...
      {result.data && <Review key={result.data.id} initial={result.data} />}
    </>
  );
}
    ...
    <div className="space-y-3">
      {proposal.evidence.map((item, index) => (
        <div key={index} className="rounded-md border p-3 text-sm">
          {item.provider === "ai_topic_research" ? (
            <TopicResearchEvidence evidence={item} />
          ) : "row" in item ? (
            <p>
              Imported row {String(item.row)} from {proposal.source_name}
            </p>
          ) : "source_url" in item &&
            ...
            >
              GitHub curated source
            </a>
          ) : "quote" in item ? (
            <>
              <p className="whitespace-pre-wrap">{String(item.quote)}</p>
              <Link
                className="text-primary hover:underline"
                href={`/content/articles/${encodeURIComponent(String(item.article_id))}`}
              >
                View source article
              </Link>
            </>
          ) : "migration" in item ? (
            <p>Migrated proposal; original audit record retained.</p>
          ) : (
            <>
              <p className="font-medium">
                {String(item.keyword)} · {String(item.article_count)} supporting articles
              </p>
                  ...
                  item.articles.map((article, articleIndex) => {
                    ...
                    return (
                      <li key={articleIndex}>
                        <Link
                          className="text-primary hover:underline"
                          href={`/content/articles/${encodeURIComponent(String(article.id))}`}
                        >
...
function Review({ initial }: { initial: TopicProposalOut }) {
  ...
  const dirty =
    JSON.stringify(draft) !== JSON.stringify(proposal.proposed) ||
    aliases !== (proposal.proposed.aliases?.join("\n") ?? "") ||
  ...
  async function review(decision: "approved" | "rejected") {
    if (reviewed || busy || analyzing) return;
    setBusy(true);
    setError(undefined);
    try {
      const topic =
        decision === "approved"
          ? {
              ...draft,
              aliases: lineValues(aliases),
              keywords: lineValues(keywords),
              facts: factsPayload(facts),
            }
          : undefined;
      const result = await adminTopicProposalReview(
        proposal.id,
        {
          decision,
          ...(proposal.content_hash ? { expected_input_hash: proposal.content_hash } : {}),
          note: note.trim() || null,
          ...(decision === "approved" ? { topic } : {}),
        },
        { headers: { "X-CSRF-Token": admin.csrf_token } },
      );
      setProposal(result);
      notify.success(
        decision === "approved" ? "Topic changes approved and applied" : "Topic proposal rejected",
      );
    } catch (error) {
  ...
  return (
    <section className="min-w-0 space-y-6">
      <PageHeading
        title={reviewed ? fields.name : proposal.proposed.name}
        browserTitle={adminRouteTitle(
          { view: "proposal", id: proposal.id },
          reviewed ? fields.name : proposal.proposed.name,
        )}
        trail={[
          ...resourceTrail("topics"),
          { label: "Topics", href: "/taxonomy/topics" },
          { label: "Proposals", href: "/taxonomy/topics?view=proposals&status=pending" },
        ]}
        description={`${proposal.action === "create" ? "New topic" : "Topic update"} · ${proposal.status}`}
      >
        {proposal.status === "approved" && proposal.topic_id && (
          <Button asChild>
            <Link href={`/taxonomy/topics/${proposal.topic_id}`}>Open topic</Link>
          </Button>
        )}
      </PageHeading>
      <div className="grid min-w-0 grid-cols-1 items-start gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
        <div className="min-w-0">
          <form
            aria-label={reviewed ? "Topic details" : "Review and edit"}
            onSubmit={(event) => {
          ...
          >
            ...
            {!reviewed && (
              <p className="text-sm text-muted-foreground">
                Keywords affect future automatic classification. Approval applies the fields below
                immediately.
              </p>
            )}
            <ValidationErrors error={error} />
            <fieldset
              disabled={reviewed || busy || analyzing}
            ...
            >
              {proposalFields.map((field) => {
                ...
                return (
                  <div
                    key={key}
                    className={
                      field.type === "textarea" || field.type === "lines" ? "sm:col-span-2" : ""
                    }
                  >
                    <FormField
                      field={field}
              ...
              {(!reviewed || !!fields.facts?.length) && (
                <div className="sm:col-span-2">
                  <FactsEditor
                    facts={reviewed ? initialFacts(fields.facts) : facts}
                    onChange={setFacts}
              ...
              <div className="flex flex-wrap justify-end gap-2">
                {dirty && (
                  <Button
                    variant="outline"
                    disabled={busy || analyzing}
                    onClick={() => analysisUpdated(proposal)}
                  >
                    Discard edits
                  </Button>
                )}
        ...
        <aside id="proposal-evidence" className="min-w-0 space-y-5">
          <TopicAnalysisControl
            proposal={proposal}
            disabled={!reviewed && (busy || dirty)}
            onUpdated={analysisUpdated}
          ...
          <section className="space-y-2 rounded-lg border bg-card p-5 text-sm">
            ...
            <p className="break-words">{proposal.source_name}</p>
            <p className="break-words text-muted-foreground">
              Submitted by {actorLabel(proposal.created_by, admin)} ·{" "}
            ...
            {proposal.review_note && <p className="whitespace-pre-wrap">{proposal.review_note}</p>}
          </section>
