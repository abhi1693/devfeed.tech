...
function LogViewer({ kind, id, followExecution = false }: Props) {
  ...
  const poll = useCallback(
    async (signal: AbortSignal, automatic = false) => {
      const current = progress.current;
      if (
        automatic &&
        (current.blocked ||
          (!followExecution && current.terminalPolls >= 3) ||
          (pending.current && !pending.current.aborted))
      )
        return;
      ...
      try {
        ...
        while (more && !signal.aborted) {
          const page = await adminJobLogs(
            kind,
            id,
            { after: current.cursor, limit: 200 },
            { signal },
          );
          ...
          current.terminalPolls =
            !page.has_more && ["succeeded", "failed"].includes(page.job_status)
        ...
        if (!failed.current)
          notifyFailure(error, "Could not load runtime logs", `job-logs-${kind}-${id}`);
        failed.current = true;
        setState((previous) => ({
          ...previous,
          error:
  ...
  function download() {
    ...
    anchor.href = url;
    anchor.download = `${kind}-${id}.log`;
    anchor.click();
    setTimeout(() => URL.revokeObjectURL(url), 0);
  }
  return (
    <InfoPanel title="Runtime logs">
      <div className="space-y-4" aria-busy={loading}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-xs text-muted-foreground" role="status">
            {page ? (
              <>
                {followExecution && <span>Research run </span>}
                <StatusBadge value={page.job_status} />
                {` · ${page.attempts} attempt${page.attempts === 1 ? "" : "s"} · ${state.items.length} log entries`}
              </>
            ) : state.error ? (
              "Logs unavailable"
            ) : (
          ...
          <Button variant="outline" size="sm" disabled={!state.items.length} onClick={download}>
            Download logs
          </Button>
        </div>
        {state.error && (
          <div role="alert" className="flex flex-wrap items-center gap-2 text-sm text-destructive">
            <p>{state.error}</p>
            <RetryButton size="sm" onRetry={() => setRefresh((value) => value + 1)} />
          </div>
        )}
        {page && (
          <p className="text-xs text-muted-foreground">
            Up to {page.max_entries.toLocaleString()} entries per run, kept for{" "}
            {Math.round(page.retention_seconds / 3600)} hours after the last log.
          </p>
        )}
        {(state.trimmed || state.unreadable > 0) && (
          <p className="text-xs text-muted-foreground">
            {state.trimmed ? "Older entries were removed by the retention limit. " : ""}
            {state.unreadable > 0 ? `${state.unreadable} unreadable entries were skipped.` : ""}
          </p>
        )}
        {!!state.items.length && (
          <>
            ...
            <div
              ...
              tabIndex={0}
              className="max-h-[36rem] overflow-auto rounded-md border"
            >
              {shown.length ? (
                <ol>
                  {shown.map((entry) => (
                    <JobLogLine key={entry.id} entry={entry} />
                  ))}
                </ol>
              ) : (
                <p className="p-4 text-sm text-muted-foreground">No entries match these filters.</p>
              )}
            </div>
          </>
        )}
