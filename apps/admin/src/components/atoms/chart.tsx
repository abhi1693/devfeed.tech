...
export function ChartTooltip({ active, payload, label, formatLabel, formatValue }: Pick<TooltipContentProps, "active" | "payload" | "label"> & { formatLabel?: (label: string) => string; formatValue?: (value: number) => string }) {
  ...
  return <div role="status" aria-live="polite" aria-atomic="true" className="max-w-64 rounded-lg border bg-popover px-3 py-2.5 text-xs text-popover-foreground shadow-md">
    ...
    <div className="space-y-1.5">{payload.map(item => <div key={String(item.dataKey)} className="flex items-center gap-2">
      ...
      <span className="ml-auto pl-4 font-medium tabular-nums">{formatValue ? formatValue(Number(item.value)) : Number(item.value).toLocaleString("en")}</span>
    </div>)}</div>
  </div>;
}
