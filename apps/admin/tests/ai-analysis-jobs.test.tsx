...
vi.mock("@/lib/api/generated/admin", () => ({
  adminAiAnalysisJobsList: vi.fn(),
  adminJobGet: vi.fn(),
  adminJobLogs: vi.fn(),
  adminJobsList: vi.fn(),
}));
vi.mock("@/lib/notifications", () => ({ notifyFailure: vi.fn() }));
const topic: AdminJobOut = {
  id: "topic-job",
  kind: "topic-analysis",
  target_name: "React",
...
const article: AdminJobOut = {
  ...topic,
  id: "article-job",
  kind: "analysis",
  target_name: "React guide",
...
it("lists both pipelines with subject links and a type filter", async () => {
  ...
  expect(adminAiAnalysisJobsList).toHaveBeenCalled();
  expect(adminJobsList).not.toHaveBeenCalled();
  expect(within(table).getByText("Topic")).toBeDefined();
  expect(within(table).getByText("Article")).toBeDefined();
  expect(within(table).getByRole("link", { name: "React" }).getAttribute("href")).toBe(
    "/taxonomy/topics/proposals/proposal-1",
  );
  expect(within(table).getByRole("link", { name: "React guide" }).getAttribute("href")).toBe(
    "/content/articles/article-1",
  );
  expect(within(table).getByRole("link", { name: "topic-jo" }).getAttribute("href")).toBe(
    "/jobs/analysis/topics/topic-job",
  );
  expect(within(table).getByRole("link", { name: "article-" }).getAttribute("href")).toBe(
    "/jobs/analysis/articles/article-job",
  );
  fireEvent.click(screen.getByRole("combobox", { name: "Analysis type" }));
  fireEvent.click(screen.getByRole("option", { name: "Topics" }));
  expect(router.push).toHaveBeenCalledWith("/jobs/analysis/topics?limit=25&offset=0", {
    scroll: false,
  });
});
...
it("opens the correct topic run and retains its pipeline in logs navigation", async () => {
  ...
  expect(adminJobGet).toHaveBeenCalledWith("topic-analysis", "topic-job", {
    signal: expect.any(AbortSignal),
  });
  await waitFor(() =>
    expect(adminJobLogs).toHaveBeenCalledWith(
      "topic-analysis",
      "topic-job",
      expect.anything(),
      expect.anything(),
  ...
  expect(screen.getByRole("link", { name: "Logs" }).getAttribute("href")).toBe(
    "/jobs/analysis/topics/topic-job/logs",
  );
  expect(screen.getByRole("link", { name: "Review topic proposal" }).getAttribute("href")).toBe(
    "/taxonomy/topics/proposals/proposal-1",
  );
});
...
it("keeps article run URLs and related-article filtering compatible", async () => {
  ...
  expect(adminAiAnalysisJobsList).toHaveBeenCalledWith(
    { article_id: "article-1", offset: 25, limit: 25 },
    { signal: undefined },
  );
});
...
it("refreshes runs without clearing the table or search input and stops after unmount", async () => {
  ...
  await act(async () => {
    view = renderAdmin(<ResourceList resource="analysis-jobs" />);
  });
  fireEvent.change(screen.getByRole("textbox", { name: "Search ai analysis" }), {
    target: { value: "draft search" },
  });
  let resolve!: (value: typeof page) => void;
  vi.mocked(adminAiAnalysisJobsList).mockReturnValueOnce(
    new Promise((done) => {
      resolve = done;
    }),
  );
  ...
  expect(
    (screen.getByRole("textbox", { name: "Search ai analysis" }) as HTMLInputElement).value,
  ).toBe("draft search");
  await act(async () => {
    resolve({ ...page, items: [{ ...topic, status: "running" }] });
  });
  expect(screen.getByText("Running")).toBeDefined();
  view.unmount();
  const count = vi.mocked(adminAiAnalysisJobsList).mock.calls.length;
  await act(async () => {
...
it("derives analysis type from the path and keeps it during search and pagination", async () => {
  ...
  expect(adminAiAnalysisJobsList).toHaveBeenCalledWith(
    expect.objectContaining({ analysis_type: "topics", status: "running", offset: 25 }),
    expect.anything(),
  );
  await waitFor(() =>
  ...
  expect(router.push).toHaveBeenLastCalledWith(
    "/jobs/analysis/topics?status=running&offset=50&limit=25",
    { scroll: false },
  );
  fireEvent.change(screen.getByRole("textbox", { name: "Search ai analysis" }), {
    target: { value: "React" },
  ...
  await waitFor(() =>
    expect(router.replace).toHaveBeenLastCalledWith(
      "/jobs/analysis/topics?status=running&offset=0&limit=25&q=React",
      { scroll: false },
    ),
  );
  fireEvent.click(screen.getByRole("combobox", { name: "Analysis type" }));
  ...
  expect(router.push).toHaveBeenLastCalledWith(
    "/jobs/analysis/articles?status=running&offset=0&limit=25",
    { scroll: false },
  );
