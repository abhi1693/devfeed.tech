...
const job = (
  id: string,
  kind: AdminJobOut["kind"] = "analysis",
  status = "failed",
): RecordData & AdminJobOut => ({
  id,
  kind,
  status,
  ...
  error: null,
  details: {},
  target_name: `Subject ${id}`,
});
const page = (items: RecordData[], total = items.length, offset = 0) => ({
  items,
  total,
  offset,
  limit: 25,
...
beforeEach(() => {
  ...
  vi.mocked(adminJobRetry).mockResolvedValue({ ...job("new"), status: "queued" } as Awaited<
    ReturnType<typeof adminJobRetry>
  >);
});
afterEach(cleanup);
...
it.each(Object.keys(jobKinds) as Resource[])("offers retry all failed in %s", async (resource) => {
  ...
  await waitFor(() =>
    expect(adminJobRetry).toHaveBeenCalledWith(jobKinds[resource], "failed", {
...
it("loads all failed pages before mutation, preserving type, search and subject filters", async () => {
  ...
  vi.mocked(listRecords).mockImplementation(async (_, params = {}) =>
    params.status === "failed"
      ? page(
          failed.slice(Number(params.offset), Number(params.offset) + Number(params.limit)),
          103,
          Number(params.offset),
        )
      : page([job("running", "topic-analysis", "running")]),
  );
  ...
  expect(vi.mocked(adminJobRetry).mock.calls.every(([kind]) => kind === "topic-analysis")).toBe(
    true,
  );
});
...
it("keeps both AI pipelines when their job IDs collide", async () => {
  vi.mocked(listRecords).mockResolvedValue(
    page([job("shared", "analysis"), job("shared", "topic-analysis")]),
  );
  ...
  expect(
    vi
      .mocked(adminJobRetry)
      .mock.calls.map(([kind]) => kind)
      .sort(),
  ).toEqual(["analysis", "topic-analysis"]);
});
...
it("retries only failed selected rows and retains individual error messages", async () => {
  ...
  renderAdmin(
    <RecordTable
      ...
      page={page([job("failed", "ingestion"), job("running", "ingestion", "running")])}
      sort="-created_at"
      onChange={vi.fn()}
      onRefresh={vi.fn()}
    />,
...
it("keeps related operations scoped to their subject without retry all failed", async () => {
  ...
  expect(listRecords).toHaveBeenCalledWith(
    "analysis-jobs",
    expect.objectContaining({ topic_id: "react", offset: 0, limit: 25 }),
    expect.any(AbortSignal),
  );
  expect(screen.getByRole("link", { name: "View full list" }).getAttribute("href")).toBe(
    "/jobs/analysis?topic_id=react",
  );
});
...
it("does not retry a partial selection if a later page fails", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "Retry all failed" }));
  await waitFor(() =>
    expect(toast.error).toHaveBeenCalledWith("Could not load matching records", expect.anything()),
  );
  expect(adminJobRetry).not.toHaveBeenCalled();
...
it("reports no eligible jobs and does not mutate anything", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "Retry all failed" }));
  await waitFor(() =>
    expect(toast.info).toHaveBeenCalledWith("No eligible records match these filters"),
  );
  expect(adminJobRetry).not.toHaveBeenCalled();
});
...
it("ignores duplicate loads and cancels preparation when leaving the table", async () => {
  ...
  vi.mocked(listRecords).mockImplementation(async (_, params) =>
    params?.status === "failed" ? pending : page([job("failed")]),
  );
  ...
  const loads = vi
    .mocked(listRecords)
    .mock.calls.filter(([, params]) => params?.status === "failed");
  expect(loads).toHaveLength(1);
  view.unmount();
  expect(loads[0][2]?.aborted).toBe(true);
  await act(async () => {
    resolve(page([job("failed")]));
    await pending;
  });
  expect(adminJobRetry).not.toHaveBeenCalled();
...
it("does not select the old failure when retry all is used again after queuing", async () => {
  ...
  expect(adminJobRetry).toHaveBeenCalledTimes(1);
  fireEvent.click(screen.getByRole("checkbox", { name: "Select all on this page" }));
  const retry = within(screen.getByRole("region", { name: "Selected rows" })).getByRole("button", {
    name: "Retry (0)",
  });
  ...
  vi.mocked(listRecords).mockResolvedValue(
    page(
      Array.from({ length: 25 }, (_, i) => ({
        ...job(String(i), "ingestion", "queued"),
        source_id: `source-${i}`,
        target_name: `Publisher ${i}`,
      })),
    ),
  );
