...
vi.mock("@/lib/api/generated/admin", () => ({ adminAutomationRecover: vi.fn(), adminSourcePublicationPolicy: vi.fn(), adminPublicationDecisions: vi.fn(), adminPublicationPolicyHistory: vi.fn() }));
vi.mock("@/lib/notifications", () => ({ notify: { success: vi.fn(), warning: vi.fn() }, notifyFailure: vi.fn() }));
beforeEach(() => vi.clearAllMocks());
afterEach(cleanup);

const data: AutomationData = {
  analysis_duration_ms: 1200, analysis_tokens: 850, automatic_publication_percent: 50,
  median_ingestion_to_publication_seconds: 120, published_in_window: 4, published_without_intervention: 2, usage_reported_runs: 3,
  blockers: [{ code: "missing_primary_topic", label: "Missing primary topic", count: 1, action: "analyze", targets: [{ id: "article-1", title: "Routing guide", kind: "article", revision: 7 }] }],
};
...
it("recovers the selected article with its displayed revision and session token", async () => {
  ...
  expect(api.adminAutomationRecover).toHaveBeenCalledWith("article-1", "analyze", { expected_revision: 7 }, { headers: { "X-CSRF-Token": "test-csrf" } });
...
it("requires a saved preview before auto mode and uses the returned revision", async () => {
  vi.mocked(api.adminSourcePublicationPolicy).mockResolvedValue({ publication_policy: "preview", publication_policy_revision: 4 } as SourceOut);
  ...
  const automatic = screen.getByRole("option", { name: "Publish eligible articles automatically" }) as HTMLOptionElement;
  ...
  expect(api.adminSourcePublicationPolicy).toHaveBeenLastCalledWith("source-1", { mode: "preview", expected_revision: 3 }, { headers: { "X-CSRF-Token": "test-csrf" } });
  vi.mocked(api.adminSourcePublicationPolicy).mockResolvedValue({ publication_policy: "auto", publication_policy_revision: 5 } as SourceOut);
  fireEvent.change(select, { target: { value: "auto" } });
  fireEvent.click(screen.getByRole("button", { name: "Save publication mode" }));
  await waitFor(() => expect(api.adminSourcePublicationPolicy).toHaveBeenLastCalledWith("source-1", { mode: "auto", expected_revision: 4 }, { headers: { "X-CSRF-Token": "test-csrf" } }));
});
...
it("does not unlock automatic publication when saving preview fails", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "Save publication mode" }));
  await waitFor(() => expect(notifyFailure).toHaveBeenCalledOnce());
  expect((screen.getByRole("option", { name: "Publish eligible articles automatically" }) as HTMLOptionElement).disabled).toBe(true);
});

it("disables publication policy changes for unapproved sources", () => {
  renderAdmin(<SourcePublicationPolicy id="source-1" mode="manual" revision={0} approved={false} />);
  expect((screen.getByRole("combobox") as HTMLSelectElement).disabled).toBe(true);
  expect((screen.getByRole("button", { name: "Save publication mode" }) as HTMLButtonElement).disabled).toBe(true);
});

it("paginates retained publication decisions", async () => {
  vi.mocked(api.adminPublicationDecisions).mockResolvedValue({ items: [{ id: "decision-1", created_at: "2026-09-10T00:00:00Z", decision: { status: "would_publish", policy_version: "trusted-source-v1" } }], total: 21, limit: 20, offset: 0 });
