...
vi.mock("@/lib/api/generated/admin", async (original) => ({
  ...
  adminTopicProposalDelete: vi.fn(),
...
const proposal: TopicProposalOut = {
  ...
  proposed: { name: "React", slug: "react", kind: "technology", aliases: [], keywords: [] },
...
function proposals(
  items: TopicProposalOut[] = [
    proposal,
    {
      ...proposal,
      id: "proposal-2",
      proposed: { ...proposal.proposed, name: "Vue", slug: "vue" },
  ...
  return renderAdmin(
    <TopicProposalsTable
      page={{ items, total: 1264, limit: 25, offset: 0 }}
      loading={false}
      status="pending"
      filtered={false}
      sort="slug"
...
function action(name: string) {
  fireEvent.click(
    within(screen.getByRole("region", { name: "Selected rows" })).getByRole("button", { name }),
  );
}

it.each(["Approve", "Reject"])(
  "bulk %s reviews only selected proposals using the displayed hash and CSRF",
  async (label) => {
    proposals();
...
it("bulk deletes reviewed proposals with their current content and review status", async () => {
  proposals([{ ...proposal, status: "approved", applied: proposal.proposed }]);
  selectAll();
  expect(
    (
  ...
  await waitFor(() =>
    expect(api.adminTopicProposalDelete).toHaveBeenCalledWith(
      proposal.id,
      { expected_input_hash: proposal.content_hash, expected_status: "approved" },
      options,
    ),
  );
});
...
it.each(["topics", "tags", "topic-relations", "articles", "sources"] as const)(
  "offers guarded bulk deletion for %s",
  async (resource) => {
    const record = {
      id: "record-1",
      name: "Example",
    ...
    renderAdmin(
      <RecordTable
        resource={resource}
        page={{ items: [record], total: 1, limit: 25, offset: 0 }}
        sort="name"
        onChange={vi.fn()}
      />,
    );
    ...
    fireEvent.click(screen.getByRole("button", { name: "Delete permanently" }));
    await waitFor(() =>
      expect(deleteRecord).toHaveBeenCalledWith(resource, record.id, "test-csrf"),
    );
  },
);
...
it("preserves the article editorial revision for bulk review", async () => {
  renderAdmin(
    <RecordTable
      ...
      page={{
        items: [
          { id: "article-1", title: "Guide", review_status: "pending", editorial_revision: 7 },
        ],
        total: 1,
        limit: 25,
        offset: 0,
  ...
  await waitFor(() =>
    expect(api.adminArticleReview).toHaveBeenCalledWith(
      "article-1",
      { action: "approve", expected_revision: 7 },
      options,
    ),
  );
});
...
it("uses source approval status to enable bulk review", async () => {
  renderAdmin(
    <RecordTable
      resource="sources"
      page={{
        items: [{ id: "source-1", name: "Feed", approval_status: "pending" }],
        total: 1,
        limit: 25,
        offset: 0,
      }}
  ...
  await waitFor(() =>
    expect(api.adminSourceReview).toHaveBeenCalledWith(
      "source-1",
      { decision: "rejected" },
      options,
    ),
...
it("allows job selection for retries without edit, delete or export controls", () => {
  renderAdmin(
    <RecordTable
      ...
      page={{
        items: [
          { id: "same-id", kind: "analysis" },
          { id: "same-id", kind: "topic-analysis" },
        ],
        total: 2,
        limit: 25,
        offset: 0,
