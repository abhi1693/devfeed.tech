...
const item = {
  id: "bcast_" + "a".repeat(26),
  source: "broadcast",
  category: "jobs.ingestion",
  payload: {
    title: "Feed ingestion failed",
    body: "Open the run for logs.",
    severity: "error",
    action_url: "/ingestion-jobs/run-1",
  },
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input, init) => {
      ...
      if (url.pathname.endsWith("/counts"))
        return Response.json({ unread: items.filter((row) => !row.read).length, unseen });
      if (url.pathname.endsWith("/items")) return Response.json({ items, next_cursor: null });
      if (url.pathname.endsWith("/seen-all")) {
        unseen = 0;
...
describe("persistent Chimely inbox (separate from toasts)", () => {
  it("loads unseen count, opens on click only, marks seen with CSRF, and links to run", async () => {
    ...
    expect(screen.queryByText("Feed ingestion failed")).toBeNull();
    await user.click(bell);
    await screen.findByText("Feed ingestion failed");
    expect(screen.getByText("Open the run for logs.")).toBeTruthy();
    expect(
      requests.some(
        (r) => r.path.endsWith("/seen-all") && r.headers.get("x-csrf-token") === "csrf",
      ),
    ).toBe(true);
  ...
  it("opens topic research notifications at the topic run URL", async () => {
    items = [
      {
        ...item,
        category: "jobs.topic-analysis",
        payload: {
          ...item.payload,
          title: "Topic research completed",
          severity: "success",
          action_url: "/jobs/analysis/topics/topic-run",
        },
      },
    ];
    const user = userEvent.setup();
  ...
  it.each([
    ...
    undefined,
    123,
  ])("blocks unsafe notification actions %s", (value) => {
