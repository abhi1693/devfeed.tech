...
} from "@/components/organisms/relationship-proposals";
...
vi.mock("@/lib/api/generated/admin", async (original) => ({
  ...
  adminTopicGet: vi.fn(),
  adminTopicRelationshipsAnalyze: vi.fn(),
  adminRelationshipsList: vi.fn(),
  adminRelationshipProposalsList: vi.fn(),
  adminRelationshipProposalGet: vi.fn(),
  adminRelationshipProposalReview: vi.fn(),
  adminRelationshipProposalDelete: vi.fn(),
}));
vi.mock("@/lib/resource-api", async (original) => ({
  ...(await original<typeof records>()),
  listRecords: vi.fn(),
  getRecord: vi.fn(),
}));
...
const topic = {
  ...
  keywords: [],
  facts: [],
} as unknown as AdminTopicOut;
const proposal: RelationshipProposalOut = {
  id: "proposal",
  job_id: "job",
  topic_id: "react",
  related_topic_id: "javascript",
  topic_name: "React",
  related_topic_name: "JavaScript",
...
beforeEach(() => {
  ...
  vi.mocked(api.adminTopicRelationshipsAnalyze).mockResolvedValue({
    id: "job",
    kind: "topic-analysis",
    topic_id: "react",
    status: "queued",
  } as AdminJobOut);
  vi.mocked(api.adminRelationshipProposalsList).mockResolvedValue({
    items: [proposal],
    total: 1,
    offset: 0,
    limit: 25,
  ...
  vi.mocked(api.adminRelationshipProposalReview).mockResolvedValue({
    ...proposal,
    status: "approved",
    can_approve: false,
  });
...
const pendingRow = () => ({
  ...
  related_topic_name: proposal.related_topic_name,
...
const approvedRow = () => ({
  ...pendingRow(),
  id: "react~typescript~related_to",
  related_topic_id: "typescript",
  related_topic_name: "TypeScript",
...
it("loads combined relationships through the typed adapter with distinct row identities", async () => {
  ...
  vi.mocked(api.adminRelationshipsList).mockResolvedValue({
    items: [pendingRow(), approvedRow()],
    total: 2,
    offset: 0,
    limit: 10,
  });
  ...
  const page = await listRecords(
    "topic-relations",
    { topic_id: "react", limit: 10, offset: 0, sort: "relation" },
    signal,
  );
  expect(api.adminRelationshipsList).toHaveBeenCalledWith(
    { topic_id: "react", limit: 10, offset: 0, sort: "relation" },
    { signal },
  );
  expect(page.items.map((row) => row.id)).toEqual([
    "proposal~proposal",
    "react~typescript~related_to",
  ]);
});

it.each(["approved", "rejected"] as const)(
  "reviews pending relationships as %s directly in the topic table",
  async (decision) => {
    vi.mocked(records.listRecords).mockResolvedValue({
      items: [pendingRow(), approvedRow()],
      total: 2,
    ...
    expect(screen.getByRole("link", { name: proposal.evidence_title }).getAttribute("href")).toBe(
    ...
    const reviewed = {
      ...pendingRow(),
      id: "react~javascript~uses_language",
      status: "approved",
      proposal: null,
    };
    vi.mocked(records.listRecords).mockResolvedValue({
      items: decision === "approved" ? [reviewed, approvedRow()] : [approvedRow()],
      total: decision === "approved" ? 2 : 1,
      offset: 0,
      limit: 10,
    });
    fireEvent.click(
      screen.getByRole("button", {
        name: `${decision === "approved" ? "Approve" : "Reject"} React → JavaScript (Uses language)`,
      }),
    );
    await waitFor(() =>
      expect(api.adminRelationshipProposalReview).toHaveBeenCalledWith(
        "proposal",
        { decision, expected_input_hash: proposal.content_hash },
        csrf,
      ),
    );
...
it.each([
  ...
  const row = {
    ...
    related_topic_id: "react",
    related_topic_name: "React",
    relation,
  };
  renderAdmin(
    <RecordTable
      ...
      topicId="react"
      page={{ items: [row], total: 1, offset: 0, limit: 10 }}
      sort="relation"
      onChange={vi.fn()}
    />,
  );
  expect(screen.getByText(expected)).toBeDefined();
  expect(screen.getByRole("link", { name: "TypeScript" }).getAttribute("href")).toBe(
    "/taxonomy/topics/typescript",
  );
  expect(screen.queryByRole("link", { name: "React" })).toBeNull();
});
...
it("retains both endpoints in the unfiltered relationships table", () => {
  renderAdmin(
    <RecordTable
      resource="topic-relations"
      page={{ items: [approvedRow()], total: 1, offset: 0, limit: 10 }}
      sort="relation"
      onChange={vi.fn()}
    />,
  );
...
it("keeps stale proposals visible with rejection available and approval disabled", async () => {
  const row = {
    ...
    proposal: {
      ...proposal,
      can_approve: false,
      approval_blocker: "A topic changed after research",
    },
  };
  renderAdmin(
    <RecordTable
  ...
  expect(
    (
      screen.getByRole("button", {
        name: "Approve React → JavaScript (Uses language)",
  ...
  expect(
    (
      screen.getByRole("button", {
        name: "Reject React → JavaScript (Uses language)",
      }) as HTMLButtonElement
    ).disabled,
  ).toBe(false);
});
...
it("bulk approval in the combined table only applies pending eligible suggestions", async () => {
  renderAdmin(
    <RecordTable
      resource="topic-relations"
      page={{ items: [pendingRow(), approvedRow()], total: 2, offset: 0, limit: 10 }}
      sort="relation"
      onChange={vi.fn()}
    />,
  );
  ...
  fireEvent.click(
    within(screen.getByRole("region", { name: "Selected rows" })).getByRole("button", {
      name: /Approve/,
    }),
  );
  ...
  expect(api.adminRelationshipProposalReview).toHaveBeenCalledWith(
    "proposal",
    { decision: "approved", expected_input_hash: proposal.content_hash },
    csrf,
  );
});

it("uses natural discovery and review routes before interpreting relationship IDs", () => {
  expect(resolveAdminRoute(["taxonomy", "relationships", "discover"])).toEqual({
    view: "relationship-discover",
  });
  expect(resolveAdminRoute(["taxonomy", "relationships", "proposals"])).toEqual({
    view: "relationship-proposals",
  });
  expect(resolveAdminRoute(["taxonomy", "relationships", "proposals", "proposal"])).toEqual({
    view: "relationship-proposal",
    id: "proposal",
  });
  expect(
    resolveAdminRoute(["taxonomy", "relationships", "react", "uses_language", "javascript"]),
  ).toEqual({
    view: "detail",
    resource: "topic-relations",
    id: "react~javascript~uses_language",
...
it("queues research from an active topic and opens the existing analysis run", async () => {
  ...
  await waitFor(() =>
    expect(
      (screen.getByRole("button", { name: "Discover with AI" }) as HTMLButtonElement).disabled,
    ).toBe(false),
  );
  fireEvent.click(screen.getByRole("button", { name: "Discover with AI" }));
  await waitFor(() =>
    expect(api.adminTopicRelationshipsAnalyze).toHaveBeenCalledWith(
      "react",
...
it("requests only active topics through the shared entity picker", async () => {
  renderAdmin(
    <EntityPicker resource="topics" label="Topic" status="active" value="" onChange={vi.fn()} />,
  );
  ...
  renderAdmin(
    <RecordTable
      ...
      page={{
        items: [
          { ...topic },
          { ...topic, id: "pending", status: "proposed" },
          { ...topic, id: "rejected", status: "rejected" },
        ],
        total: 3,
        offset: 0,
        limit: 25,
...
it("loads all matching relationship proposals and approves only eligible snapshots", async () => {
  ...
  const stale = {
    ...proposal,
    id: "stale",
    related_topic_name: "Changed",
    can_approve: false,
    approval_blocker: "Both topics must still be active before approval",
  };
  vi.mocked(api.adminRelationshipProposalsList)
  ...
  fireEvent.click(
    await screen.findByRole("checkbox", { name: "Select React → JavaScript (Uses language)" }),
  );
  ...
  expect(api.adminRelationshipProposalsList).toHaveBeenCalledWith(
...
it("shows evidence and preserves the displayed hash when reviewing", async () => {
  ...
  expect(
    (await screen.findByRole("link", { name: "React documentation" })).getAttribute("href"),
  ).toBe("https://react.dev");
  fireEvent.change(screen.getByRole("textbox", { name: "Review note" }), {
    target: { value: "Verified the official documentation" },
  });
  ...
  await waitFor(() =>
    expect(api.adminRelationshipProposalReview).toHaveBeenCalledWith(
      "proposal",
      {
        ...
        note: "Verified the official documentation",
      },
      csrf,
    ),
  );
  expect(
    (await screen.findByRole("link", { name: "View relationships" })).getAttribute("href"),
  ).toBe("/taxonomy/relationships?topic_id=react");
});

it("allows rejection when changed or inactive topics block approval", async () => {
  vi.mocked(api.adminRelationshipProposalGet).mockResolvedValue({
    ...proposal,
    can_approve: false,
    approval_blocker: "Both topics must still be active before approval",
  });
  ...
  expect(
    (screen.getByRole("button", { name: "Approve relationship" }) as HTMLButtonElement).disabled,
  ).toBe(true);
  fireEvent.click(screen.getByRole("button", { name: "Reject" }));
  await waitFor(() =>
    expect(api.adminRelationshipProposalReview).toHaveBeenCalledWith(
      "proposal",
      { decision: "rejected", expected_input_hash: proposal.content_hash, note: null },
      csrf,
    ),
  );
});
...
it("keeps manual creation and AI discovery in the shared add menu", async () => {
  ...
  for (const [name, href] of [
    ["Create relationship", "/taxonomy/relationships/new"],
    ["Discover with AI", "/taxonomy/relationships/discover"],
    ["Review proposals", "/taxonomy/relationships/proposals"],
  ])
    expect(screen.getByRole("menuitem", { name }).getAttribute("href")).toBe(href);
});
