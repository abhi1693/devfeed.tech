...
vi.mock("@/lib/api/generated/admin", () => ({ adminWorkerGet: vi.fn(), adminWorkersSnapshot: vi.fn() }));
vi.mock("@/lib/notifications", () => ({ notifyFailure: vi.fn() }));
vi.mock("@/components/organisms/job-logs", () => ({ JobLogs: ({ kind, id }: { kind: string; id: string }) => <div data-testid="logs">{kind}/{id}</div> }));
const worker: WorkerOut = {
  name: "ai-worker-1", role: "ai", queues: ["analysis", "relationships"], state: "busy", registered: true,
  hostname: "host-1", pid: 12, started_at: "2026-09-10T18:00:00Z", last_heartbeat: "2026-09-10T18:01:00Z",
  uptime_seconds: 60, heartbeat_age_seconds: 1, registration_ttl_seconds: 90, completed_executions: 42,
  failed_executions: 2, working_seconds: 30,
  current_job: { rq_id: "rq-1", id: "job-1", kind: "research-verification", proposal_id: "proposal-1", target_name: "React", status: "running", elapsed_seconds: 5 },
};
const snapshot: WorkersSnapshot = { generated_at: "2026-09-10T18:01:01Z", workers: [worker], ai_cooldown_seconds: 0, queues: [
  { name: "analysis", registered_workers: 1, busy_workers: 1, idle_workers: 0, suspended_workers: 0, dispatched: 5, queued: 6, running: 1, failed: 0, succeeded: 20, review_required: 8, oldest_queued_at: "2026-09-10T17:00:00Z" },
] };

beforeEach(() => { vi.clearAllMocks(); vi.mocked(adminWorkerGet).mockResolvedValue(worker); vi.mocked(adminWorkersSnapshot).mockResolvedValue(snapshot); });
...
it("shows worker activity and links verification to its topic run", async () => {
  ...
  expect(within(table).getByRole("link", { name: "ai-worker-1" }).getAttribute("href")).toBe("/workers/ai-worker-1");
  expect(within(table).getByRole("link", { name: "React" }).getAttribute("href")).toBe("/jobs/analysis/topics/job-1");
  ...
  fireEvent.change(screen.getByRole("textbox", { name: "Search workers" }), { target: { value: "missing" } });
...
it("keeps successful data visible with an explicit stale warning on polling failure", async () => {
  ...
  await act(async () => { view = render(<WorkersOverview />); });
...
it("opens the verification logs and proposal without implying approval", async () => {
  ...
  expect(screen.getByRole("link", { name: "Open topic proposal" }).getAttribute("href")).toBe("/taxonomy/topics/proposals/proposal-1");
  expect(screen.getByRole("link", { name: "Job logs" }).getAttribute("href")).toBe("/jobs/analysis/topics/job-1/logs");
});

it("explains a worker disappearing after a restart", async () => {
  vi.mocked(adminWorkerGet).mockRejectedValue(new ApiError(404, "Worker is no longer registered; it may have stopped or restarted"));
...
it("shows review-required completions separately from failures and warns about missing capacity", async () => {
  vi.mocked(adminWorkersSnapshot).mockResolvedValue({ ...snapshot, ai_cooldown_seconds: 120, queues: [{ ...snapshot.queues[0], registered_workers: 0 }] });
