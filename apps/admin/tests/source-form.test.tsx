...
vi.mock("@/lib/resource-api", async original => ({ ...await original<typeof import("@/lib/resource-api")>(), getRecord: vi.fn(), saveRecord: vi.fn() }));
vi.mock("@/lib/api/generated/admin", async original => ({ ...await original<typeof import("@/lib/api/generated/admin")>(), adminSourcePreview: vi.fn() }));

const details: SourcePreviewOut = { name: "Developer News", description: "Engineering articles", website_url: "https://publication.example/", logo_url: "https://publication.example/logo.png", image_url: "https://publication.example/cover.png", language: "en-us", entries_seen: 10, entries_skipped: 0, warnings: [] };
...
const advance = (ms = 650) => act(async () => { await vi.advanceTimersByTimeAsync(ms); });
async function mount(id?: string) {
  await act(async () => { render(<AdminSession admin={{ subject: "admin", issuer: "https://identity.example", organization_id: "org", roles: ["superuser"], expires_at: 4102444800, csrf_token: "test-csrf" }}><ResourceForm resource="sources" id={id} /></AdminSession>); });
}
beforeEach(() => {
  vi.useFakeTimers();
  vi.clearAllMocks();
  vi.mocked(adminSourcePreview).mockResolvedValue(details);
  vi.mocked(getRecord).mockResolvedValue({ id: "source-1", ...details, feed_url: url, source_type: "publisher", enabled: true, poll_interval_seconds: 1800 });
...
describe("automatic source details", () => {
  it("waits for a valid URL and a typing pause, without a lookup button or success box", async () => {
    ...
    expect(adminSourcePreview).not.toHaveBeenCalled();
    await advance(1);
    expect(adminSourcePreview).toHaveBeenCalledWith({ feed_url: url, source_type: "publisher" }, { signal: expect.any(AbortSignal), headers: { "X-CSRF-Token": "test-csrf" } });
    expect(input("Name").value).toBe("Developer News");
    expect(input("Website").value).toBe(details.website_url);
    expect(screen.getByRole("combobox", { name: "Language" }).textContent).toBe("English (United States)");
  ...
  it("preserves edits made before or during lookup and disables creation while pending", async () => {
    ...
    vi.mocked(adminSourcePreview).mockImplementationOnce(() => new Promise(done => { resolve = done; }));
    await mount();
  ...
  it("shows duplicate URL errors inline, without retry loops or erasing them on unrelated edits", async () => {
    const message = "A source with this RSS / Atom URL already exists. Edit the existing source instead.";
    vi.mocked(adminSourcePreview).mockRejectedValueOnce(new ApiError(409, "Please correct the highlighted fields.", { feed_url: message }));
    await mount();
    fireEvent.change(input("RSS / Atom URL"), { target: { value: url } });
    await advance();
  ...
  it("toasts actionable partial-lookup warnings without adding a banner", async () => {
    vi.mocked(adminSourcePreview).mockResolvedValueOnce({ ...details, warnings: ["Website unavailable; fill missing details manually."] });
    await mount();
    fireEvent.change(input("RSS / Atom URL"), { target: { value: url } });
    await advance();
    expect(toast.warning).toHaveBeenCalledWith("Some source details could not be fetched", expect.objectContaining({ description: "Website unavailable; fill missing details manually." }));
...
describe("language selector", () => {
  it("shows readable names and preserves regional and three-letter codes", () => {
    const changed = vi.fn();
    const view = render(<LanguageSelect aria-label="Language" value="sr-latn-rs" onChange={changed} />);
    const select = screen.getByRole("combobox", { name: "Language" });
    expect(select.textContent).toContain("Serbian");
    fireEvent.click(select);
    fireEvent.change(screen.getByPlaceholderText("Search languages or codes…"), { target: { value: "Hindi" } });
