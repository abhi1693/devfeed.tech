...
describe("read-only language labels", () => {
  it.each([
    ...
    "shows %j as %j in list and related-object tables without changing stored codes",
    (language, label) => {
      const article = {
        id: "article-1",
      ...
      renderAdmin(
        <RecordTable
          resource="articles"
          page={{ items: [article], total: 1, limit: 25, offset: 0 }}
          sort="-discovered_at"
          onChange={vi.fn()}
        />,
      );
  ...
  it.each(["sources", "articles"] as const)(
    "uses the same regional language name on %s detail pages",
    async (resource) => {
