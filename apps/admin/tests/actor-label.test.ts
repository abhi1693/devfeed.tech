...
const identity = { subject: "389598389664220144", issuer: "https://identity.example", organization_id: "org" };
...
describe("proposal attribution", () => {
  it("prefers the recorded name, then email", () => {
    expect(actorLabel({ ...identity, name: " Original name ", email: "old@example.com" }, viewer)).toBe("Original name");
    expect(actorLabel({ ...identity, name: " ", email: "old@example.com" }, viewer)).toBe("old@example.com");
  });
  ...
  it.each(["subject", "issuer", "organization_id"] as const)("does not assign the viewer's name when %s differs", field => {
  ...
  it("labels the known system actor", () => {
    expect(actorLabel({ subject: "ai-analysis", issuer: "devfeed", organization_id: "system" })).toBe("AI analysis");
  });
});
