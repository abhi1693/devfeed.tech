...
vi.mock("next/navigation", () => ({
  useRouter: () => router,
  useSearchParams: () => new URLSearchParams("view=proposals&" + router.query),
}));
vi.mock("@/lib/api/generated/admin", () => ({
  adminTopicProposalFilterOptions: vi.fn(),
  adminTopicProposalAnalyze: vi.fn(),
  adminTopicImportPreview: vi.fn(),
  adminTopicImportSubmit: vi.fn(),
  adminTopicProposalGet: vi.fn(),
  adminTopicProposalReview: vi.fn(),
  adminTopicEnrichmentPreview: vi.fn(),
  adminTopicEnrichmentSubmit: vi.fn(),
  adminTopicProposalsList: vi.fn(),
...
const fact = {
  ...
  retrieved_at: "2026-09-07T12:34:56.789Z",
};
...
beforeEach(() => {
  vi.clearAllMocks();
  router.query = "";
  vi.mocked(api.adminTopicProposalFilterOptions).mockResolvedValue({
    kinds: ["discipline", "language", "technology"],
    sources: ["GitHub curated topics", "topics.json"],
  });
  vi.mocked(api.adminTopicProposalsList).mockResolvedValue({
    items: [],
    total: 0,
    offset: 0,
    limit: 25,
  });
  vi.mocked(api.adminTopicGithubPull).mockResolvedValue({
  ...
  vi.mocked(api.adminTopicImportPreview).mockResolvedValue({
    rows: [{ row: 1, action: "create", topic: draft, issues: [] }],
  ...
  vi.mocked(api.adminTopicProposalReview).mockResolvedValue({
    ...proposal,
    status: "approved",
    applied: draft,
    topic_id: "topic-1",
  ...
  vi.mocked(api.adminTopicEnrichmentPreview).mockResolvedValue({
    topic: draft,
    ...
    suggestions: [
      {
        ...
        article_count: 2,
        articles: [
          { id: "article-1", title: "Database design", url: "https://example.com/article" },
        ],
      },
    ],
  });
  vi.mocked(api.adminTopicEnrichmentSubmit).mockResolvedValue({
    ...proposal,
    action: "update",
...
it("selects all matching proposals using the same filters across every page", async () => {
  router.query =
    "kind=technology&source=GitHub+curated+topics&q=web&analysis=not_run&missing=description&sort=slug";
  const second = {
    ...proposal,
    id: "proposal-2",
    proposed: { ...proposal.proposed, name: "Frontend" },
  };
  vi.mocked(api.adminTopicProposalsList)
    .mockResolvedValueOnce({ items: [proposal], total: 2, offset: 0, limit: 25 })
  ...
  for (const offset of [0, 1])
    expect(api.adminTopicProposalsList).toHaveBeenCalledWith(
      expect.objectContaining({
        status: "pending",
        kind: "technology",
        source: "GitHub curated topics",
        q: "web",
        analysis: "not_run",
        missing: "description",
        sort: "slug",
...
describe("supervised topic workflow", () => {
  it("changes import format through the shared picker and invalidates the previous preview", async () => {
    mount(<TopicImport />);
    fireEvent.change(screen.getByRole("textbox", { name: "Topic data" }), {
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "Topic data" }), {
      target: { value: "name,slug,kind\nBackend,backend,discipline" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Preview import" }));
    await waitFor(() =>
      expect(api.adminTopicImportPreview).toHaveBeenLastCalledWith(
  ...
  it("previews a proposal logo and blocks approval until invalid URLs are corrected", async () => {
    ...
    expect((await screen.findByAltText("Logo preview")).getAttribute("src")).toBe(
    ...
    fireEvent.click(screen.getByRole("button", { name: "Approve and create topic" }));
    await waitFor(() =>
      expect(api.adminTopicProposalReview).toHaveBeenCalledWith(
        "proposal-1",
        expect.objectContaining({
  ...
  it("edits sourced facts with explicit approval, retaining the correct timestamps after removing a row", async () => {
    vi.mocked(api.adminTopicProposalGet).mockResolvedValue({
      ...proposal,
      content_hash: "c".repeat(64),
      proposed: {
        ...draft,
        facts: [{ ...fact, name: "Old fact", retrieved_at: "2026-09-01T05:00:12Z" }, fact],
      },
    });
    mount(<TopicProposalReview id="proposal-1" />);
    ...
    fireEvent.change(screen.getAllByRole("textbox", { name: "Fact name" })[1], {
      target: { value: "License" },
    });
    fireEvent.change(screen.getAllByRole("textbox", { name: "Value" })[1], {
      target: { value: "MIT" },
    });
    fireEvent.change(screen.getAllByRole("textbox", { name: "Source URL" })[1], {
      target: { value: "https://example.com/license" },
    });
    fireEvent.change(screen.getAllByLabelText(/Retrieved at/)[1], {
    ...
    await waitFor(() =>
      expect(api.adminTopicProposalReview).toHaveBeenCalledWith(
        "proposal-1",
        expect.objectContaining({
          expected_input_hash: "c".repeat(64),
          decision: "approved",
          topic: {
            ...draft,
            facts: [
              { ...fact, value: "2021" },
              {
                ...
                retrieved_at: new Date("2026-09-09T12:00").toISOString(),
              },
            ],
          },
        }),
  ...
  it("protects dirty facts from AI replacement and restores them when edits are discarded", async () => {
    vi.mocked(api.adminTopicProposalGet).mockResolvedValue({
      ...proposal,
      proposed: { ...draft, facts: [fact] },
    });
    mount(<TopicProposalReview id="proposal-1" />);
    const analysis = (await screen.findByRole("button", {
      name: "Run AI analysis for Backend",
  ...
  it("shows applied facts in disabled shared fields after review", async () => {
    vi.mocked(api.adminTopicProposalGet).mockResolvedValue({
      ...proposal,
      status: "approved",
      applied: { ...draft, facts: [fact], logo_url: "https://example.com/logo.svg" },
    });
    mount(<TopicProposalReview id="proposal-1" />);
    expect(
      ((await screen.findByRole("textbox", { name: "Value" })) as HTMLInputElement).value,
    ).toBe(fact.value);
    for (const name of ["Value", "Source URL", "Logo URL"])
      expect(screen.getByRole("textbox", { name }).matches(":disabled")).toBe(true);
  ...
  it("previews imports without creating proposals, then explicitly submits for review", async () => {
    ...
    await waitFor(() =>
      expect(api.adminTopicImportSubmit).toHaveBeenCalledWith(
  ...
  it("invalidates a preview when its input changes", async () => {
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "Source name" }), {
      target: { value: "edited.csv" },
    });
    expect(screen.queryByRole("button", { name: "Send proposals for review" })).toBeNull();
  });
  it("blocks submission of an invalid import and displays its row issue", async () => {
    vi.mocked(api.adminTopicImportPreview).mockResolvedValue({
      rows: [{ row: 1, action: "create", topic: draft, issues: ["Duplicate slug in this import"] }],
  ...
  it("shows the evidence and submits the administrator's edited fields only on approval", async () => {
    ...
    await waitFor(() =>
      expect(api.adminTopicProposalReview).toHaveBeenCalledWith(
        ...
        {
          decision: "approved",
          topic: { ...draft, name: "Backend engineering", facts: [] },
          note: "Checked scope",
        },
        { headers: { "X-CSRF-Token": "test-csrf" } },
      ),
  ...
  it("requires selecting evidence-backed enrichment terms before proposing them", async () => {
    ...
    expect((submit as HTMLButtonElement).disabled).toBe(true);
    expect(screen.getByRole("link", { name: "Database design" }).getAttribute("href")).toBe(
      "/content/articles/article-1",
    );
    fireEvent.click(screen.getByRole("checkbox"));
    fireEvent.click(submit);
    await waitFor(() =>
      expect(api.adminTopicEnrichmentSubmit).toHaveBeenCalledWith(
        "topic-1",
        { preview_token: "evidence-token", keywords: ["postgres"] },
        { headers: { "X-CSRF-Token": "test-csrf" } },
      ),
    );
    expect(api.adminTopicProposalReview).not.toHaveBeenCalled();
...
it("pulls the GitHub repository with one button and continues batches automatically", async () => {
  vi.mocked(api.adminTopicGithubPull)
  ...
  expect(api.adminTopicGithubPull).toHaveBeenNthCalledWith(
    1,
    {},
    { headers: { "X-CSRF-Token": "test-csrf" } },
  );
  expect(api.adminTopicGithubPull).toHaveBeenNthCalledWith(
    2,
    { revision: "a".repeat(40), offset: 100 },
    { headers: { "X-CSRF-Token": "test-csrf" } },
  );
...
it("can continue after a GitHub pull fails without asking for search parameters", async () => {
  vi.mocked(api.adminTopicGithubPull).mockRejectedValueOnce(
...
describe("proposal table", () => {
  it("shows a scannable table with review links and keeps discovery collapsed", async () => {
    ...
    expect(within(table).getByRole("link", { name: "Review Backend" }).getAttribute("href")).toBe(
  ...
  it("preserves batch, query, and page size when changing status, sort, or page", async () => {
    ...
    const approved = within(screen.getByRole("navigation", { name: "Topic views" })).getByRole(
      "link",
      { name: "Approved" },
    );
    expect(approved.getAttribute("href")).toBe(
      "/taxonomy/topics?view=proposals&status=approved&batch_id=batch-1&q=backend&offset=0&limit=25&sort=-created_at",
    );
    fireEvent.click(screen.getByRole("button", { name: "Next" }));
    expect(router.push).toHaveBeenLastCalledWith(
      "/taxonomy/topics?view=proposals&status=pending&batch_id=batch-1&q=backend&offset=50&limit=25&sort=-created_at",
      { scroll: false },
    );
    fireEvent.click(screen.getByRole("button", { name: "Sort by topic slug" }));
    expect(router.push).toHaveBeenLastCalledWith(
      "/taxonomy/topics?view=proposals&status=pending&batch_id=batch-1&q=backend&offset=0&limit=25&sort=slug",
      { scroll: false },
    );
    fireEvent.change(screen.getByRole("textbox", { name: "Search proposals" }), {
      target: { value: "github" },
    });
    fireEvent.submit(screen.getByRole("search"));
    expect(router.replace).toHaveBeenLastCalledWith(
      "/taxonomy/topics?view=proposals&status=pending&batch_id=batch-1&q=github&offset=0&limit=25&sort=-created_at",
      { scroll: false },
    );
  });
  ...
  it("restores combined filters from the URL and preserves them across navigation", async () => {
    router.query =
      "status=pending&kind=technology&source=GitHub+curated+topics&action=create&analysis=failed&missing=keywords&offset=25&limit=25";
    vi.mocked(api.adminTopicProposalsList).mockResolvedValue({
      items: [proposal],
      total: 80,
      offset: 25,
    ...
    expect(next.get("analysis")).toBe("failed");
    expect(next.get("missing")).toBe("keywords");
    ...
    expect(removed.has("source")).toBe(false);
    expect(removed.get("kind")).toBe("technology");
    expect(removed.get("offset")).toBe("0");
    fireEvent.click(screen.getByRole("button", { name: "Clear filters" }));
    expect(router.push).toHaveBeenLastCalledWith(
      "/taxonomy/topics?view=proposals&status=pending&offset=0&limit=25",
      { scroll: false },
    );
  });
  ...
  it("offers catalog-wide choices and resets pagination when adding a filter", async () => {
    ...
    expect(router.push).toHaveBeenLastCalledWith(
      "/taxonomy/topics?view=proposals&q=backend&offset=0&limit=25&kind=language",
      { scroll: false },
    );
    ...
    expect(router.push).toHaveBeenLastCalledWith(
      "/taxonomy/topics?view=proposals&q=backend&offset=0&limit=25&analysis=enriched",
      { scroll: false },
    );
  });
  ...
  it("uses view actions for reviewed proposals and recovers from a failed table request", async () => {
    ...
    vi.mocked(api.adminTopicProposalsList)
      ...
      .mockResolvedValue({
        items: [{ ...proposal, status: "rejected" }],
        total: 1,
        offset: 0,
        limit: 25,
      });
  ...
  it("explains an empty search and clears its filters without changing the status", async () => {
    ...
    expect(router.push).toHaveBeenCalledWith(
      "/taxonomy/topics?view=proposals&status=approved&offset=0&limit=25",
      { scroll: false },
    );
  });
});
...
it("keeps a GitHub pull running when discovery is closed and restores progress on reopening", async () => {
  ...
  vi.mocked(api.adminTopicGithubPull).mockReturnValueOnce(
    new Promise((resolve) => {
      finish = resolve;
    }),
  );
  ...
  await waitFor(() =>
    expect(screen.getByRole("button", { name: "Discover topics" }).textContent).toBe(
      "Discover topics",
    ),
  );
  fireEvent.click(screen.getByRole("button", { name: "Discover topics" }));
...
it("shows review context and lets admins choose columns without losing choices on refresh", async () => {
  ...
  expect(within(table).getByRole("link", { name: "1 article" }).getAttribute("href")).toBe(
    "/content/articles/article-1",
  );
  fireEvent.click(screen.getByRole("button", { name: "Columns" }));
  expect((screen.getByRole("checkbox", { name: "Topic" }) as HTMLInputElement).disabled).toBe(true);
  expect((screen.getByRole("checkbox", { name: "Actions" }) as HTMLInputElement).disabled).toBe(
    true,
  );
...
describe("proposal row actions and AI research", () => {
  it("approves the displayed version with CSRF and prevents repeated clicks", async () => {
    ...
    vi.mocked(api.adminTopicProposalReview).mockReturnValue(
      new Promise((resolve) => {
        finish = resolve;
      }),
    );
    ...
    expect(api.adminTopicProposalReview).toHaveBeenCalledWith(
      "proposal-1",
      { decision: "approved", topic: draft, expected_input_hash: "a".repeat(64) },
      { headers: { "X-CSRF-Token": "test-csrf" } },
    );
    expect(
      (screen.getByRole("button", { name: "Reject Backend" }) as HTMLButtonElement).disabled,
    ).toBe(true);
  ...
  it("rejects a row without applying fields and leaves retry available on failure", async () => {
    ...
    vi.mocked(api.adminTopicProposalReview).mockRejectedValue(
      new Error("Proposal changed. Reload it before reviewing"),
    );
    mount(<ResourceList resource="topics" />);
  ...
  it("runs web research from the review page, refreshes fields and shows sources without approving", async () => {
    const enriched: TopicProposalOut = {
      ...proposal,
      content_hash: "b".repeat(64),
      proposed: { ...draft, aliases: ["Server development"], facts: [fact] },
      evidence: [
        {
          ...
          sources: [
            {
              ...
              quote: "Server development is its alternate name.",
            },
          ],
        },
      ],
    ...
    vi.mocked(api.adminTopicProposalGet)
      .mockResolvedValueOnce(proposal)
      .mockResolvedValue(enriched);
    vi.mocked(api.adminTopicProposalAnalyze).mockResolvedValue({
      id: "analysis-1",
      kind: "topic-analysis",
      status: "queued",
      attempts: 0,
    ...
    await act(async () => {
      fireEvent.click(screen.getByRole("button", { name: "Run AI analysis for Backend" }));
    });
    expect(api.adminTopicProposalAnalyze).toHaveBeenCalledWith("proposal-1", {
      headers: { "X-CSRF-Token": "test-csrf" },
    });
    expect(
      (screen.getByRole("button", { name: "Approve and create topic" }) as HTMLButtonElement)
        .disabled,
    ).toBe(true);
    await act(async () => {
    ...
    expect((screen.getByRole("textbox", { name: /^Aliases/ }) as HTMLTextAreaElement).value).toBe(
      "Server development",
    );
    expect((screen.getByRole("textbox", { name: "Fact name" }) as HTMLInputElement).value).toBe(
      fact.name,
    );
    expect(screen.queryByRole("button", { name: "Discard edits" })).toBeNull();
    expect(api.adminTopicProposalReview).not.toHaveBeenCalled();
  ...
  it("preserves unsaved edits by disabling analysis until they are resolved", async () => {
    ...
    fireEvent.change(await screen.findByRole("textbox", { name: "Name" }), {
      target: { value: "Human change" },
    });
    expect(
      (screen.getByRole("button", { name: "Run AI analysis for Backend" }) as HTMLButtonElement)
        .disabled,
    ).toBe(true);
    expect(api.adminTopicProposalAnalyze).not.toHaveBeenCalled();
  });
...
it("keeps a failed AI status check inside one icon and retries without rerunning analysis", async () => {
  const running: TopicProposalOut = {
    ...proposal,
    analysis: {
      id: "analysis-1",
      status: "running",
  ...
  vi.mocked(api.adminTopicProposalGet)
    .mockRejectedValueOnce(new Error("The admin service is unavailable. Try again."))
    .mockResolvedValue({
      ...running,
      analysis: { ...running.analysis!, status: "succeeded", outcome: "enriched" },
    });
  vi.useFakeTimers();
  await act(async () => {
    mount(<ResourceList resource="topics" />);
...
it("shows human attribution in provenance and table columns without raw IDs", async () => {
  const value: TopicProposalOut = {
    ...proposal,
    created_by: { subject: "389598389664220144", name: "Alex Morgan" },
    status: "approved",
    reviewed_at: proposal.created_at,
    reviewed_by: { subject: "99334455", email: "reviewer@example.com" },
  };
