...
describe("notification policy", () => {
  it("supports all severities, with more reading time for warnings and errors", () => {
    notify.success("Saved"); notify.info("Information"); notify.warning("Warning"); notify.error("Error");
  ...
  it("shows actionable API errors, but not arbitrary network or exception details", () => {
    notifyFailure(new ApiError(409, "Another editor changed this record. Reload and try again."), "Could not save");
    expect(toast.error).toHaveBeenLastCalledWith("Could not save", expect.objectContaining({ description: "Another editor changed this record. Reload and try again." }));
  ...
  it("deduplicates failed reads until recovery, without hiding later session expiry", async () => {
    ...
    load.mockRejectedValueOnce(new ApiError(401)); view.rerender({ id: "3" });
    await waitFor(() => expect(returnToLogin).toHaveBeenCalledOnce());
    load.mockResolvedValueOnce("loaded"); view.rerender({ id: "4" });
  ...
  it("ignores stale failures after unmount", async () => {
    ...
    const load = vi.fn(() => new Promise((_, fail) => { reject = fail; }));
    const view = renderHook(() => useRequest("record", load));
    view.unmount();
    await act(async () => { reject(new ApiError(503)); });
    expect(toast.error).not.toHaveBeenCalled();
  });
});
...
describe("authentication feedback", () => {
  it("confirms logout once after document navigation, even in Strict Mode", async () => {
    render(<StrictMode><LoginPanel enabled signedOut /></StrictMode>);
    await waitFor(() => expect(toast.info).toHaveBeenCalledTimes(1));
    expect(toast.info).toHaveBeenCalledWith("You have signed out of DevFeed.", { id: "auth-result" });
  });
  it("keeps role-denial recovery visible as well as notifying, with no false logout success", async () => {
    render(<StrictMode><LoginPanel enabled signedOut error="Required role missing." /></StrictMode>);
...
describe("workflow feedback", () => {
  it.each(["approve", "reject", "publish", "unpublish"])("confirms article %s after the API succeeds", async decision => {
    vi.mocked(adminArticleReview).mockResolvedValueOnce({} as Awaited<ReturnType<typeof adminArticleReview>>);
    ...
    fireEvent.change(screen.getByLabelText(/Reason/), { target: { value: "Reviewed the article." } });
  ...
  it("reports queued work as requested, not completed", async () => {
    vi.mocked(adminSourceFetch).mockResolvedValueOnce({ id: "job-1" } as Awaited<ReturnType<typeof adminSourceFetch>>);
  ...
  it("retains failed decisions and validation without a success or redirect", async () => {
    ...
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith("Could not apply decision", expect.any(Object)));
