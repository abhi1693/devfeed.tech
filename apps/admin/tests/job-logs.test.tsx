...
const entry = (id: string, message = "Fetching feed"): JobLogEntry => ({
  id,
  message,
  timestamp: "2026-09-07T10:00:00Z",
  level: "INFO",
  fields: { attempt: 1, event: "ingestion_started" },
});
const page = (overrides: Partial<AdminJobLogs> = {}): AdminJobLogs => ({
  items: [],
  next_cursor: null,
  has_more: false,
  truncated: false,
...
const advance = (ms = 10000) =>
  act(async () => {
    await vi.advanceTimersByTimeAsync(ms);
  });
const mount = () =>
  act(async () => {
    render(<JobLogs kind="ingestion" id="job-1" />);
  });
...
describe("runtime log viewer", () => {
  it("polls using the cursor and appends entries without duplicates", async () => {
    vi.mocked(adminJobLogs).mockResolvedValueOnce(
      page({ items: [entry("1-0")], next_cursor: "1-0" }),
    );
    await mount();
    expect(screen.getByText("Fetching feed")).toBeDefined();
    vi.mocked(adminJobLogs).mockResolvedValue(
      page({ items: [entry("1-0"), entry("2-0", "Ingested 10 entries")], next_cursor: "2-0" }),
    );
    await advance();
    expect(adminJobLogs).toHaveBeenLastCalledWith(
      "ingestion",
      "job-1",
      { after: "1-0", limit: 200 },
  ...
  it("drains pages even for completed jobs then stops after settling polls", async () => {
    vi.mocked(adminJobLogs)
      ...
      .mockResolvedValueOnce(
        page({ items: [entry("2-0", "Finished")], next_cursor: "2-0", job_status: "succeeded" }),
      )
  ...
  it("keeps existing logs during an outage and recovers on retry", async () => {
    vi.mocked(adminJobLogs)
      .mockResolvedValueOnce(page({ items: [entry("1-0")], next_cursor: "1-0" }))
    ...
    vi.mocked(adminJobLogs).mockResolvedValue(
      page({
        items: [
          entry("1-0"),
          {
            ...entry("2-0", '<img src=x onerror="alert(1)">'),
            level: "ERROR",
            fields: { attempt: 2, error_type: "ValueError" },
          },
        ],
      }),
    );
  ...
  it("bounds loaded entries and reports retention trimming and corrupt records", async () => {
    vi.mocked(adminJobLogs).mockResolvedValue(
      page({
        items: [entry("1-0", "Old"), entry("2-0", "Recent")],
        max_entries: 1,
        truncated: true,
    ...
    expect(screen.getByText(/Older entries were removed/).textContent).toContain(
      "1 unreadable entries",
    );
  });
  ...
  it("aborts requests and discards old logs on navigation", async () => {
    ...
    await act(async () => {
      view.rerender(<JobLogs kind="images" id="job-2" />);
    });
  ...
  it("pauses updates from the saved setting without a page refresh control", async () => {
    await act(async () => {
      render(
        <RefreshSettings>
          <JobLogs kind="ingestion" id="job-1" />
        </RefreshSettings>,
      );
    });
...
it("continues following an active verifier after its parent research run succeeded", async () => {
  ...
  await act(async () => {
    view = render(<JobLogs kind="topic-analysis" id="research-1" followExecution />);
  });
  await advance(50000);
  ...
  vi.mocked(adminJobLogs).mockResolvedValue(
    page({
      job_status: "succeeded",
      items: [entry("5-0", "Verification finished")],
      next_cursor: "5-0",
    }),
  );
  await advance();
  ...
  await act(async () => {
    view.rerender(<JobLogs kind="topic-analysis" id="research-1" followExecution={false} />);
  });
  await advance(30000);
  const calls = vi.mocked(adminJobLogs).mock.calls.length;
  await advance(30000);
