...
const advance = (ms = 400) =>
  act(async () => {
    await vi.advanceTimersByTimeAsync(ms);
  });
function Example(props: Omit<FieldProps, "label" | "children">) {
  return (
    <Field label="Title" {...props}>
      {(control) => <Input {...control} defaultValue="Developer news" />}
    </Field>
  );
}
...
describe("shared field composition", () => {
  ...
  it("connects subtext, tooltip information, and live validation errors, removing resolved errors", () => {
    const { rerender } = render(
      <Example
        id="title"
        subtext="Use the original title."
        tooltip="Do not replace this with an AI headline."
        error="Title is too long."
      />,
    );
    const input = screen.getByRole("textbox", { name: "Title" });
    expect(descriptions(input).map((element) => element?.textContent)).toEqual([
      "Use the original title.",
      "Do not replace this with an AI headline.",
      "Title is too long.",
    ]);
  ...
  it("preserves explicit IDs, names, native required/disabled behavior and form values", () => {
    const { rerender } = render(
      <form aria-label="Article">
        <Example id="article-title" name="title" required />
      </form>,
    );
    ...
    expect(new FormData(screen.getByRole("form") as HTMLFormElement).get("title")).toBe(
      "Developer news",
    );
    ...
    rerender(
      <form aria-label="Article">
        <Example id="article-title" name="title" required disabled />
      </form>,
    );
  ...
  it("supports rich subtext and does not render empty optional content", () => {
    const { rerender } = render(
      <Example
        subtext={
          <>
            Read the <Link href="/content/sources">source details</Link>.
          </>
        }
      />,
    );
    expect(screen.getByRole("link", { name: "source details" }).getAttribute("href")).toBe(
      "/content/sources",
    );
    expect(descriptions(screen.getByRole("textbox"))[0]?.textContent).toBe(
  ...
  it("exposes reusable label, subtext and error atoms with caller styling", () => {
    render(
      <>
        ...
        <FieldDescription className="custom">Helpful text</FieldDescription>
  ...
  it.each([
    ...
    "language",
    "reference",
  ] as const)("applies the same accessible wrapper to %s controls", (type) => {
    ...
    const role =
      type === "boolean"
        ? "checkbox"
        : ["select", "language", "reference"].includes(type)
          ? "combobox"
          : type === "number"
            ? "spinbutton"
            : null;
    const control = role
      ? screen.getByRole(role, { name: "Detail" })
      : document.getElementById("detail")!;
    expect(control.getAttribute("aria-invalid")).toBe("true");
    expect(descriptions(control).map((element) => element?.textContent)).toEqual([
      "Required metadata",
...
describe("field help tooltip", () => {
  ...
  it("opens after a hover delay and never opens the associated combobox", async () => {
    ...
    fireEvent.pointerMove(screen.getByRole("button", { name: "About Type" }), {
      pointerType: "mouse",
    });
    await advance(349);
    expect(screen.queryByRole("tooltip")).toBeNull();
