...
describe("shared button", () => {
  ...
  it.each([
    ...
    "destructive-ghost",
    "link",
  ] as const)("shares cursor, focus and reduced-motion behavior for %s", (variant) => {
  ...
  it("shows loading feedback, prevents repeat activation and restores the original state", () => {
    ...
    const { rerender } = render(
      <Button onClick={click} loading loadingText="Saving…">
        Save
      </Button>,
    );
    ...
    expect(button.querySelector('[data-slot="button-spinner"]')?.getAttribute("aria-hidden")).toBe(
      "true",
    );
  ...
  it.each([{ disabled: true }, { "aria-disabled": true }, { "aria-disabled": "true" as const }])(
    "blocks disabled button handlers: %j",
    (props) => {
      const click = vi.fn(),
        capture = vi.fn();
      render(
        <Button {...props} onClick={click} onClickCapture={capture}>
          Delete
        </Button>,
      );
  ...
  it("preserves Next.js link semantics with asChild and adds no nested button", () => {
    render(
      <Button asChild variant="outline">
        <Link href="/content/sources" prefetch={false}>
          Sources
        </Link>
      </Button>,
    );
  ...
  it.each([{ disabled: true }, { loading: true }, { "aria-disabled": true }])(
    "blocks normal child link activation when unavailable: %j",
    (props) => {
      const childClick = vi.fn((event) => event.preventDefault()),
        parentClick = vi.fn();
      render(
        <Button asChild {...props} onClick={parentClick}>
          <a href="https://example.test/" onClick={childClick}>
            Open
          </a>
        </Button>,
      );
...
it.each(["icon", "icon-xs", "icon-sm", "icon-lg"] as const)(
  "replaces the %s action icon with one spinner without changing its label",
  (size) => {
    const { rerender } = render(
      <Button size={size} aria-label="Run AI analysis">
    ...
    rerender(
      <Button size={size} loading aria-label="Run AI analysis">
        <svg data-testid="action-icon" />
      </Button>,
    );
