...
it("cancels unsent searches on browser navigation, filter changes and unmount", () => {
  const search = vi.fn();
  const view = render(
    <SearchField label="Search" value="React" scopeKey="pending" onSearch={search} />,
  );
  fireEvent.change(screen.getByRole("textbox"), { target: { value: "Vue" } });
  view.rerender(
    <SearchField label="Search" value="Angular" scopeKey="pending" onSearch={search} />,
  );
  tick();
  expect((screen.getByRole("textbox") as HTMLInputElement).value).toBe("Angular");
  fireEvent.change(screen.getByRole("textbox"), { target: { value: "Vue" } });
  view.rerender(
    <SearchField label="Search" value="Angular" scopeKey="approved" onSearch={search} />,
  );
  tick();
  expect((screen.getByRole("textbox") as HTMLInputElement).value).toBe("Angular");
  fireEvent.change(screen.getByRole("textbox"), { target: { value: "Vue" } });
