...
beforeEach(() => {
  vi.useFakeTimers();
  vi.clearAllMocks();
  load.mockResolvedValue("Loaded data");
});
afterEach(() => {
  cleanup();
  vi.useRealTimers();
...
it("uses 10 seconds by default without page controls and preserves drafts through settings changes", async () => {
  ...
  fireEvent.change(screen.getByRole("textbox", { name: "articles draft" }), {
    target: { value: "Unsubmitted filter" },
  });
  await advance(10000);
  ...
  await act(async () => {
    render(
      <SettingsProvider
        ...
        initial={{ defaults: { refresh_seconds: 60 } }}
      >
        <Reader />
      </SettingsProvider>,
    );
...
it("uses the same saved interval for every panel and after page navigation", async () => {
  function Pages() {
    ...
    return (
      <RefreshSettings>
        <button onClick={() => setPage("topics")}>Next page</button>
        <Reader key={page} id={page} />
        <Reader id="logs" />
      </RefreshSettings>
    );
  }
