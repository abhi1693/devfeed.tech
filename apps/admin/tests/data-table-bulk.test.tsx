...
const props = {
  ...
  getRowLabel: (row: (typeof rows)[number]) => row.name,
  bulkActions: [{ id: "approve", label: "Approve", description: "Apply changes", run: vi.fn() }],
};
...
it("clears selection and open confirmation when filters change, including when returning to the old filter", () => {
  ...
  expect((screen.getByRole("checkbox", { name: "Select Alpha" }) as HTMLInputElement).checked).toBe(
    false,
  );
...
it("requires typed deletion confirmation and retains failed rows after partial success", async () => {
  const run = vi
    ...
    .mockRejectedValueOnce(new ApiError(409, "Linked content prevents deletion"));
  const refresh = vi.fn();
  render(
    <DataTable
      {...props}
      ...
      bulkActions={[
        {
          id: "delete",
          label: "Delete",
          description: "Permanently delete these records.",
          destructive: true,
          run,
        },
      ]}
  ...
  expect((screen.getByRole("checkbox", { name: "Select Beta" }) as HTMLInputElement).checked).toBe(
    true,
  );
});
...
it("keeps selection through refreshed rows and submits the snapshot shown in the confirmation", async () => {
  ...
  expect(
    (screen.getByRole("checkbox", { name: "Select Updated Alpha" }) as HTMLInputElement).checked,
  ).toBe(true);
  fireEvent.click(screen.getByRole("button", { name: "Approve" }));
...
it("explicitly expands page selection to all matching records and acts on off-page rows", async () => {
  ...
  render(
    <DataTable
      ...
      pagination={{ offset: 0, limit: 25, total: 3, onChange: vi.fn() }}
  ...
  expect(
    (screen.getByRole("checkbox", { name: "Select all on this page" }) as HTMLInputElement)
      .indeterminate,
  ).toBe(false);
  fireEvent.click(screen.getByRole("button", { name: "Approve" }));
...
it("cancels selecting all on filter changes and ignores a late response", async () => {
  ...
  const loadAllRows = vi.fn().mockReturnValue(
    new Promise((resolve) => {
      complete = resolve;
    }),
  );
  ...
  const view = render(
    <DataTable {...props} loadAllRows={loadAllRows} pagination={page} selectionKey="pending" />,
  );
  fireEvent.click(screen.getByRole("checkbox", { name: "Select all on this page" }));
  fireEvent.click(screen.getByRole("button", { name: "Select all 3 matching records" }));
  ...
  view.rerender(
    <DataTable {...props} loadAllRows={loadAllRows} pagination={page} selectionKey="approved" />,
  );
...
it("reads every matching page and deduplicates records moved by concurrent edits", async () => {
  const load = vi
    .fn()
    .mockResolvedValueOnce({ items: rows, total: 4 })
    .mockResolvedValueOnce({ items: [rows[1], { id: "c", name: "Gamma" }], total: 4 });
