...
import { resources, resourceKeys } from "@/lib/resources";
import { toast } from "sonner";
vi.mock("sonner", () => ({ toast: { success: vi.fn(), error: vi.fn(), warning: vi.fn(), info: vi.fn() } }));
const router = vi.hoisted(() => ({ push: vi.fn(), replace: vi.fn(), refresh: vi.fn() }));
vi.mock("next/navigation", () => ({ useRouter: () => router, useSearchParams: () => new URLSearchParams("q=React&offset=25&limit=25"), usePathname: () => "/topics/test/edit" }));
...
const category = { id: "category-1", name: "Languages", slug: "languages", keywords: ["code"], parent_id: null, topic_id: null };
...
beforeEach(() => {
  ...
  vi.mocked(getRecord).mockResolvedValue(category);
  vi.mocked(saveRecord).mockResolvedValue(category);
...
describe("object navigation and table conventions", () => {
  it("groups all supported objects and marks the current sidebar item", () => {
    ...
    for (const resource of resourceKeys) expect(screen.getByRole("link", { name: resources[resource].label }).getAttribute("href")).toBe(`/${resource}`);
  ...
  it("provides real object/action links and server-side pagination", () => {
    const change = vi.fn();
    render(<RecordTable resource="categories" page={{ items: [category], total: 26, limit: 25, offset: 0 }} sort="name" onChange={change} />);
    expect(screen.getByRole("link", { name: "Languages" }).getAttribute("href")).toBe("/categories/category-1");
    expect(screen.getByRole("link", { name: "Edit" }).getAttribute("href")).toBe("/categories/category-1/edit");
    expect(screen.getByRole("link", { name: "Delete" }).getAttribute("href")).toBe("/categories/category-1/delete");
  ...
  it("keeps navigation usable when a saved offset exceeds the remaining records", () => {
    const change = vi.fn();
    render(<RecordTable resource="categories" page={{ items: [], total: 1, limit: 25, offset: 25 }} sort="name" onChange={change} />);
  ...
  it("omits the actions column on read-only job tables while keeping the run link", () => {
    ...
    expect(screen.queryByRole("link", { name: "View" })).toBeNull();
    expect(screen.getByRole("link", { name: "job-1234" }).getAttribute("href")).toBe("/ingestion-jobs/job-123456");
  });
  it("searches and paginates at the API, not just within downloaded rows", async () => {
    ...
    expect(router.push).toHaveBeenCalledWith("/topics?q=Terraform&offset=0&limit=25", { scroll: false });
  });
  it("does not offer create/edit/delete for worker-owned jobs", async () => {
    render(<ResourceList resource="analysis-jobs" />);
    await screen.findByText("No ai analysis match these filters.");
...
describe("dedicated object forms", () => {
  it.each([
    { id: undefined, label: "Create category", cancelHref: "/categories" },
    { id: "category-1", label: "Save changes", cancelHref: "/categories/category-1" },
  ])("right-aligns form actions with $label after Cancel", async ({ id, label, cancelHref }) => {
    withAdmin(<ResourceForm resource="categories" id={id} />);
  ...
  it.each(["topics", "categories", "tags"] as const)("generates a slug while typing a new %s name", async resource => {
  ...
  it("submits a category form with parsed keywords and a session csrf token", async () => {
    withAdmin(<ResourceForm resource="categories" />);
    ...
    fireEvent.change(screen.getByLabelText("Keywords"), { target: { value: "code\n code\npython" } });
    fireEvent.click(screen.getByRole("button", { name: "Create category" }));
    await waitFor(() => expect(saveRecord).toHaveBeenCalledWith("categories", { name: "Languages", slug: "languages", keywords: ["code", "python"], parent_id: null, topic_id: null }, "test-csrf", undefined));
    expect(router.replace).toHaveBeenCalledWith("/categories/category-1");
  ...
  it("preserves a failed form and displays field-level validation", async () => {
    ...
    expect(toast.error).toHaveBeenCalledWith("Could not save category", expect.objectContaining({ description: "Please correct the highlighted fields." }));
    expect(toast.success).not.toHaveBeenCalled();
  });
  it("loads existing object values on the edit page", async () => {
    withAdmin(<ResourceForm resource="categories" id="category-1" />);
    const name = await screen.findByLabelText(/Name/) as HTMLInputElement;
    expect(name.value).toBe("Languages");
    fireEvent.change(name, { target: { value: "Programming languages" } });
    fireEvent.click(screen.getByRole("button", { name: "Save changes" }));
    expect((screen.getByLabelText(/Slug/) as HTMLInputElement).value).toBe("languages");
    await waitFor(() => expect(saveRecord).toHaveBeenCalledWith("categories", expect.objectContaining({ name: "Programming languages", slug: "languages" }), "test-csrf", "category-1"));
  ...
  it("requires explicit delete confirmation and keeps conflicts visible", async () => {
    ...
    withAdmin(<ResourceDelete resource="categories" id="category-1" />);
    ...
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith("Could not delete category", expect.objectContaining({ description: "Cannot delete: linked tags exist." })));
    expect(deleteRecord).toHaveBeenCalledWith("categories", "category-1", "test-csrf");
    expect(router.replace).not.toHaveBeenCalled();
