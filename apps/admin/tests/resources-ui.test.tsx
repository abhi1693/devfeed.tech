...
import {
  adminTopicDeletePreview,
  adminTopicReplacementsList,
  adminTopicProposalGet,
  adminTopicProposalsList,
  adminTopicProposalFilterOptions,
...
vi.mock("next/navigation", () => ({
  ...
  usePathname: () => "/taxonomy/topics/test/edit",
}));
vi.mock("@/lib/resource-api", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@/lib/resource-api")>()),
  listRecords: vi.fn(),
  getRecord: vi.fn(),
  saveRecord: vi.fn(),
  deleteRecord: vi.fn(),
...
vi.mock("@/lib/api/generated/admin", async (original) => ({
  ...
  adminTopicDeletePreview: vi.fn(),
  adminTopicReplacementsList: vi.fn(),
...
beforeEach(() => {
  ...
  vi.mocked(adminTopicDeletePreview).mockResolvedValue({
    articles: 3,
    published_articles: 2,
    tags: 1,
    relationships: 2,
    relationship_proposals: 1,
    research_jobs: 1,
    pending_topic_proposals: 1,
  });
  vi.mocked(adminTopicReplacementsList).mockResolvedValue({
    items: [],
    total: 0,
    limit: 25,
    offset: 0,
  });
  vi.mocked(adminTopicProposalsList).mockResolvedValue({
    items: [],
    total: 0,
    limit: 10,
    offset: 0,
...
describe("object navigation and table conventions", () => {
  it("groups all supported objects and marks the current sidebar item", () => {
    ...
    for (const resource of resourceKeys)
      expect(
        screen.getByRole("link", { name: resources[resource].label }).getAttribute("href"),
      ).toBe(resourceHref(resource));
  ...
  it("provides real object/action links and server-side pagination", () => {
    ...
    renderAdmin(
      <RecordTable
        resource="topics"
        page={{ items: [topic], total: 26, limit: 25, offset: 0 }}
        sort="name"
        onChange={change}
      />,
    );
    expect(screen.getByRole("link", { name: "Languages" }).getAttribute("href")).toBe(
      "/taxonomy/topics/topic-1",
    );
    expect(screen.getByRole("link", { name: "Edit" }).getAttribute("href")).toBe(
      "/taxonomy/topics/topic-1/edit",
    );
    expect(screen.getByRole("link", { name: "Delete" }).getAttribute("href")).toBe(
      "/taxonomy/topics/topic-1/delete",
    );
  ...
  it("keeps navigation usable when a saved offset exceeds the remaining records", () => {
    ...
    renderAdmin(
      <RecordTable
        resource="topics"
        page={{ items: [], total: 1, limit: 25, offset: 25 }}
        sort="name"
        onChange={change}
      />,
    );
  ...
  it("omits the actions column on read-only job tables while keeping the run link", () => {
    ...
    expect(screen.queryByRole("link", { name: "View" })).toBeNull();
    expect(screen.getByRole("link", { name: "job-1234" }).getAttribute("href")).toBe(
      "/jobs/ingestion/job-123456",
    );
  });
  it("searches and paginates at the API, not just within downloaded rows", async () => {
    ...
    await waitFor(() =>
      expect(listRecords).toHaveBeenCalledWith(
        "topics",
        { q: "React", offset: 25, limit: 25, sort: "name" },
    ...
    await waitFor(() =>
      expect(router.replace).toHaveBeenCalledWith(
        "/taxonomy/topics?q=Terraform&offset=0&limit=25",
        { scroll: false },
      ),
    );
  });
  it("opens pending proposals from the proposed filter and a visible shortcut", async () => {
    renderAdmin(<ResourceList resource="topics" />);
    expect(
      within(screen.getByRole("navigation", { name: "Topic views" }))
        .getByRole("link", { name: "Proposed" })
        .getAttribute("href"),
    ).toBe("/taxonomy/topics?q=React&offset=0&limit=25&view=proposals&status=pending");
    fireEvent.click(screen.getByRole("combobox", { name: "Status" }));
    fireEvent.click(await screen.findByRole("option", { name: "Proposed" }));
    expect(router.push).toHaveBeenCalledWith(
      "/taxonomy/topics?q=React&offset=0&limit=25&status=pending&view=proposals",
      { scroll: false },
    );
  });
  it("renders an explicit proposed filter on the same screen without querying catalog topics", async () => {
    ...
    renderAdmin(<ResourceList resource="topics" />);
    await waitFor(() =>
      expect(adminTopicProposalsList).toHaveBeenCalledWith(
  ...
  it("opens proposals when the proposed filter was restored from preferences", async () => {
    ...
    render(
      <AdminSession
        ...
        settings={normalizeSettings({
          tables: { topics: { query: { status: "proposed", limit: "10", sort: "name" } } },
        })}
      >
    ...
    await waitFor(() =>
      expect(adminTopicProposalsList).toHaveBeenCalledWith(
        expect.objectContaining({ status: "pending", limit: 10, sort: "slug" }),
        { signal: expect.any(AbortSignal) },
      ),
    );
    expect(router.replace).not.toHaveBeenCalled();
  ...
  it.each(["pending", "approved", "rejected"])(
    "shows %s proposals on the Topics screen without loading the catalog",
    async (status) => {
      ...
      expect(listRecords).not.toHaveBeenCalled();
      const catalog = within(screen.getByRole("navigation", { name: "Topic views" })).getByRole(
        "link",
        { name: "Topics" },
      );
  ...
  it("restores a saved proposal view directly on the Topics screen", async () => {
    ...
    render(
      <AdminSession
        ...
        settings={normalizeSettings({
          tables: { topics: { query: { view: "proposals", status: "rejected", limit: "10" } } },
        })}
      >
        <ResourceList resource="topics" />
      </AdminSession>,
    ...
    await waitFor(() =>
      expect(adminTopicProposalsList).toHaveBeenCalledWith(
        expect.objectContaining({ status: "rejected", limit: 10 }),
        { signal: expect.any(AbortSignal) },
      ),
    );
    expect(listRecords).not.toHaveBeenCalled();
...
describe("dedicated object forms", () => {
  it("preserves automatic tag discovery on edits and makes manual clearing explicit", async () => {
    ...
    await waitFor(() =>
      expect(saveRecord).toHaveBeenCalledWith(
        "tags",
        expect.objectContaining({ auto_link_topic: true, topic_id: "topic-1" }),
        "test-csrf",
        "tag-1",
      ),
    );
    ...
    expect((screen.getByRole("combobox", { name: "Topic" }) as HTMLButtonElement).disabled).toBe(
      false,
    );
    fireEvent.click(screen.getByRole("button", { name: "Save changes" }));
    await waitFor(() =>
      expect(saveRecord).toHaveBeenLastCalledWith(
        "tags",
        expect.objectContaining({ auto_link_topic: false, topic_id: "topic-1" }),
        "test-csrf",
        "tag-1",
      ),
    );
  ...
  it.each([
    ...
    expect(Array.from(submit.parentElement!.children)).toEqual(
      id
        ? [cancel, submit]
        : [cancel, submit, screen.getByRole("button", { name: "Create and add another" })],
    );
  ...
  it.each(["topics", "tags"] as const)(
    "generates a slug while typing a new %s name",
    async (resource) => {
  ...
  it("keeps manual slugs, including edits made before entering a name, until cleared", async () => {
    ...
    await waitFor(() =>
      expect(saveRecord).toHaveBeenCalledWith(
        "tags",
        expect.objectContaining({ name: "JavaScript Libraries", slug: "js-tools" }),
        "test-csrf",
        undefined,
      ),
    );
  ...
  it("submits a topic form with parsed keywords and a session csrf token", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Matching keywords"), {
      target: { value: "code\n code\npython" },
    });
  ...
  it("preserves a failed form and displays field-level validation", async () => {
    vi.mocked(saveRecord).mockRejectedValue(
      new ApiError(422, "Please correct the highlighted fields.", { slug: "Invalid slug" }),
    );
    withAdmin(<ResourceForm resource="topics" />);
    fireEvent.change(await screen.findByLabelText(/Name/), { target: { value: "Languages" } });
    ...
    expect(toast.error).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ description: "Please correct the highlighted fields." }),
    );
  ...
  it("creates another topic on a fresh form, clearing facts and manual slug state", async () => {
    ...
    expect(saveRecord).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        name: "First topic",
        slug: "manual-slug",
        facts: [expect.objectContaining({ name: "Release" })],
      }),
      "test-csrf",
      undefined,
    );
    ...
    expect(saveRecord).toHaveBeenLastCalledWith(
      "topics",
      expect.objectContaining({ name: "Second topic", slug: "second-topic", facts: [] }),
      "test-csrf",
      undefined,
    );
    expect(router.replace).toHaveBeenCalledWith("/taxonomy/topics/topic-1");
  ...
  it("keeps failed create-and-add-another input and prevents a second save while pending", async () => {
    ...
    vi.mocked(saveRecord).mockReturnValueOnce(
      new Promise((_, fail) => {
        reject = fail;
      }),
    );
    ...
    await waitFor(() =>
      expect(toast.error).toHaveBeenCalledWith(
        "Could not save tag",
        expect.objectContaining({ description: "This tag already exists" }),
      ),
    );
  ...
  it("loads existing object values on the edit page", async () => {
    ...
    await waitFor(() =>
      expect(saveRecord).toHaveBeenCalledWith(
        "topics",
        expect.objectContaining({ name: "Programming languages", slug: "languages" }),
        "test-csrf",
        "topic-1",
      ),
    );
  ...
  it("requires explicit delete confirmation and keeps conflicts visible", async () => {
    vi.mocked(deleteRecord).mockRejectedValue(
      new ApiError(409, "Linked records are being updated. Try again."),
    );
    ...
    await waitFor(() =>
      expect(toast.error).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ description: "Linked records are being updated. Try again." }),
      ),
    );
    expect(deleteRecord).toHaveBeenCalledWith("topics", "topic-1", "test-csrf");
    expect(router.replace).not.toHaveBeenCalled();
    expect(
      (screen.getByRole("textbox", { name: "Type DELETE to confirm" }) as HTMLInputElement).value,
    ).toBe("DELETE");
  ...
  it("confirms deletion only after the server succeeds", async () => {
    let resolve!: () => void;
    vi.mocked(deleteRecord).mockImplementationOnce(
      () =>
        new Promise((done) => {
          resolve = done;
        }),
    ...
    withAdmin(<ResourceDelete resource="topics" id="topic-1" />);
    fireEvent.change(await screen.findByRole("textbox", { name: "Type DELETE to confirm" }), {
      target: { value: "DELETE" },
    });
  ...
  it.each(["proposed", "rejected", "active"])(
    "shows deletion impact and can relink to a %s topic",
    async (status) => {
      const replacement = {
        ...topic,
        id: "replacement",
      ...
      expect(
        within(table).getByRole("row", { name: "Linked articles 3 Unpublish and unlink" }),
      ).toBeDefined();
      expect(
        within(table).getByRole("row", { name: "Currently published 2 Remove from public feed" }),
      ).toBeDefined();
      expect(adminTopicDeletePreview).toHaveBeenCalledWith("topic-1", {
        signal: expect.any(AbortSignal),
      });
      ...
      expect(adminTopicReplacementsList).toHaveBeenCalledWith(
        expect.objectContaining({ exclude_topic_id: "topic-1" }),
        { signal: expect.any(AbortSignal) },
      );
      fireEvent.click(option);
      expect(
        within(table).getByRole("row", { name: "Linked articles 3 Unpublish and relink" }),
      ).toBeDefined();
      fireEvent.change(screen.getByRole("textbox", { name: "Type DELETE to confirm" }), {
        target: { value: "DELETE" },
      });
      fireEvent.click(screen.getByRole("button", { name: "Delete permanently" }));
      await waitFor(() =>
        expect(deleteRecord).toHaveBeenCalledWith("topics", "topic-1", "test-csrf", "replacement"),
      );
    },
  );
  it("can relink to a pending proposal while keeping approval explicit", async () => {
    ...
    vi.mocked(adminTopicProposalGet).mockResolvedValue({
      id: "pending-1",
      status: "pending",
      proposed: { name: "Vercel", slug: "vercel", kind: "platform" },
    } as Awaited<ReturnType<typeof adminTopicProposalGet>>);
    withAdmin(<ResourceDelete resource="topics" id="topic-1" />);
  ...
  it("omits zero counts from the deletion summary", async () => {
    vi.mocked(adminTopicDeletePreview).mockResolvedValue({
      ...
      relationship_proposals: 1,
      research_jobs: 0,
      pending_topic_proposals: 0,
    });
    withAdmin(<ResourceDelete resource="topics" id="topic-1" />);
    const table = await screen.findByRole("table", { name: "Deletion impact" });
    expect(within(table).getAllByRole("row")).toHaveLength(3);
    expect(
      within(table).getByRole("row", { name: "Linked articles 2 Unpublish and unlink" }),
    ).toBeDefined();
    expect(
      within(table).getByRole("row", { name: "Relationship proposals 1 Delete" }),
    ).toBeDefined();
    for (const label of [
      "Currently published",
      "Tag links",
      ...
      expect(within(table).queryByText(label)).toBeNull();
  });
  ...
  it("preserves publication timestamp precision when an unrelated field is edited", () => {
    const record = {
      id: "article-1",
      title: "Article",
      published_at: "2026-01-01T00:00:12.123456Z",
