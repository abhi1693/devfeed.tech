...
describe("clickable read-only image previews", () => {
  it.each([true, false])(
    "settles an image that completed before hydration (loaded: %s)",
    (loaded) => {
      ...
      Object.defineProperties(img, {
        complete: { value: true },
        naturalWidth: { value: loaded ? 460 : 0 },
      });
  ...
  it.each(["logo", "image"] as const)(
    "renders a %s preview that opens the original URL safely in a new tab",
    (kind) => {
  ...
  it.each([
    ...
    "https://user:secret@publication.example/image.png",
    "not a url",
  ])("does not create unsafe links or image requests: %s", (value) => {
    render(<ImagePreviewLink value={value} />);
  ...
  it.each(["sources", "topics", "articles"] as const)(
    "uses previews for media fields on real %s detail screens, leaving ordinary URLs as links",
    async (resource) => {
      ...
      if (resource !== "topics")
        expect(
          screen.getByRole("link", { name: "Open image in a new tab" }).getAttribute("href"),
        ).toBe(cover);
      const ordinaryUrl =
        resource === "articles"
  ...
  it.each([null, "", "javascript:alert(1)"])(
    "omits absent or unsafe header logos without adding an empty details row: %j",
    async (value) => {
      vi.mocked(getRecord).mockResolvedValue({
        id: "record-1",
        name: "Cloudflare AI",
