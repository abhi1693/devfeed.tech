...
describe("shared toast host", () => {
  it("does not lose initial login feedback before the host subscribes", async () => {
    render(
      <StrictMode>
        <LoginFeedback error="Required role missing." signedOut={false} />
        <Toaster />
      </StrictMode>,
    );
  ...
  it.each(["success", "error", "warning", "info"] as const)(
    "renders accessible %s feedback with a dismiss button",
    async (severity) => {
      render(<Toaster />);
      act(() => {
        notify[severity](`${severity} result`, { description: "Helpful details." });
      });
      await screen.findByText(`${severity} result`);
      expect(document.querySelector(`[data-sonner-toast][data-type="${severity}"]`)).not.toBeNull();
      expect(screen.getByRole("region", { name: /Notifications/ }).getAttribute("aria-live")).toBe(
        "polite",
      );
  ...
  it("survives page content changes without duplicating a repeated notification ID", async () => {
    ...
    act(() => {
      notify.success("Source saved", { id: "save" });
    });
  ...
  it("renders API text as text, never HTML", async () => {
    render(<Toaster />);
    act(() => {
      notify.error("Could not save", { description: '<img src=x onerror="alert(1)">' });
    });
