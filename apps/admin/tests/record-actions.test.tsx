...
afterEach(cleanup);
const labels = () =>
  within(screen.getByRole("group", { name: "Record actions" }))
    .getAllByRole("link")
...
describe("record action ordering and icons", () => {
  it("puts source operations before Edit and Delete, with unchanged workflow links", () => {
    ...
    for (const [label, route] of [
      ...
      expect(screen.getByRole("link", { name: label }).getAttribute("href")).toBe(
        `/content/sources/source-1/${route}`,
      );
    }
  });
  ...
  it("puts article classification and review before Edit and Delete", () => {
    ...
    expect(screen.getByRole("link", { name: "Classify" }).getAttribute("href")).toBe(
      "/content/articles/article-1/classify",
    );
    expect(screen.getByRole("link", { name: "Review / publish" }).getAttribute("href")).toBe(
      "/content/articles/article-1/review",
    );
  });

  it.each(resourceKeys.filter((resource) => !resources[resource].readonly))(
    "keeps Delete last and Edit before it in %s row and detail actions",
    (resource) => {
      ...
      expect(screen.getByRole("link", { name: "Delete" }).getAttribute("data-variant")).toBe(
        "destructive-ghost",
      );
    },
  );
  ...
  it.each(resourceKeys.filter((resource) => resources[resource].readonly))(
    "does not offer editing, deletion, or unrelated operations for %s",
    (resource) => {
  ...
  it("keeps record IDs encoded in action URLs", () => {
    ...
    expect(screen.getByRole("link", { name: "Delete" }).getAttribute("href")).toBe(
      "/content/sources/source%20%2F1/delete",
    );
    expect(screen.getByRole("link", { name: "Fetch feed" }).getAttribute("href")).toBe(
      "/content/sources/source%20%2F1/fetch",
    );
  });
