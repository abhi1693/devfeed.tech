...
vi.mock("next/navigation", () => ({
  useSearchParams: () => new URLSearchParams(navigation.raw),
...
vi.mock("@/lib/api/generated/admin", () => ({
  ...
  adminSettingsProfile: vi.fn(),
  adminSettingsAppearance: vi.fn(),
  adminSettingsDefaults: vi.fn(),
  adminSettingsNotifications: vi.fn(),
  adminSettingsTable: vi.fn(),
  adminSettingsTablesReset: vi.fn(),
  adminNotificationConfig: vi.fn(),
}));
const admin = {
  name: "Alex",
...
beforeEach(() => {
  ...
  vi.mocked(adminSettingsProfile).mockImplementation(async (value) => {
    settings = normalizeSettings({ ...settings, profile: value });
    return settings;
  });
  vi.mocked(adminSettingsAppearance).mockImplementation(async (value) => {
    settings = normalizeSettings({ ...settings, appearance: value });
    return settings;
  });
  vi.mocked(adminSettingsTable).mockImplementation(async (key, value) => {
    settings = normalizeSettings({
      ...settings,
      tables: {
        ...settings.tables,
        [key]: {
          ...settings.tables[key],
          ...value,
          columns: value.columns ?? settings.tables[key]?.columns,
          query: value.query ?? settings.tables[key]?.query,
        },
      },
    });
    return settings;
...
function page(section: SettingsSection) {
  return render(
...
it("saves profile with CSRF, updates the shared menu and restores it after remount", async () => {
  ...
  expect(adminSettingsProfile).toHaveBeenCalledWith(
...
it("scopes preferences to an account even when the surrounding layout survives", () => {
  const view = render(
    <AdminSession admin={admin} settings={{ profile: { display_name: "One" } }}>
      <UserMenu admin={admin} />
    </AdminSession>,
  );
  expect(screen.getByRole("button", { name: "User menu: One" })).toBeTruthy();
  view.rerender(
    <AdminSession admin={{ ...admin, subject: "two" }} settings={{}}>
      <UserMenu admin={admin} />
    </AdminSession>,
  );
...
it("restores filters, lets explicit URLs and clearing win, and persists column choices", async () => {
  settings = normalizeSettings({
    defaults: { page_size: 50 },
    tables: { topics: { query: { q: "AI", sort: "name" }, columns: { kind: false } } },
  });
  function Reader() {
    const query = useTableQuery("topics");
    return <output>{query.toString()}</output>;
  }
  const view = render(
    <AdminSession admin={admin} settings={settings}>
      <Reader />
      <DataTable
        label="Topics"
        preferenceKey="topics"
        columnChoices
        data={[{ id: "one", kind: "Technology" }]}
        getRowId={(row) => row.id}
        columns={[
...
it("cannot let an old focus response overwrite a newer save", async () => {
  ...
  vi.mocked(adminSettingsGet).mockImplementationOnce(
    () =>
      new Promise((resolve) => {
        complete = resolve;
      }),
  ...
  function Editor() {
    ...
    return (
      <button onClick={() => void save("profile", { display_name: "New", avatar_url: null })}>
        Change
      </button>
    );
  }
  render(
    <AdminSession admin={admin} settings={settings}>
      <Editor />
      <UserMenu admin={admin} />
      <DateTime value="2026-09-10T00:00:00Z" />
    </AdminSession>,
  );
  fireEvent(window, new Event("blur"));
  fireEvent(window, new Event("focus"));
  await waitFor(() => expect(adminSettingsGet).toHaveBeenCalled());
  fireEvent.click(screen.getByText("Change"));
  await screen.findByRole("button", { name: "User menu: New" });
...
it("preserves earlier category opt-outs and separates failure, retry and completion preferences", () => {
  const choices = notificationChoices([
    { category: "jobs.analysis", channel: "in_app", enabled: false },
  ]);
  ...
  expect(payload.find((row) => row.category === "jobs.relationship-research.error")?.enabled).toBe(
    true,
  );
});

it("loads and saves actual Chimely preferences through the session proxy with CSRF", async () => {
  vi.mocked(adminNotificationConfig).mockResolvedValue({
    enabled: true,
    environment: "test",
    subscriber_id: "admin-fixture",
  });
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input, init) => {
      ...
      if (url.pathname.endsWith("/preferences"))
        return Response.json(payload ?? { preferences: [] });
  ...
  vi.mocked(adminSettingsNotifications).mockImplementation(async (value) => ({
    ...settings,
    notifications: value,
  }));
  page("notifications");
  ...
  await waitFor(() =>
    expect(
      payload?.preferences.find((row) => row.category === "jobs.analysis.success")?.enabled,
    ).toBe(false),
  );
  await waitFor(() =>
    expect(
...
it("inherits earlier category opt-outs into missing event choices without overwriting explicit choices", async () => {
  ...
  const existing = [
    { category: "jobs.topic-analysis", channel: "in_app" as const, enabled: false },
    { category: "jobs.topic-analysis.error", channel: "in_app" as const, enabled: true },
  ];
  const client = {
    getPreferences: vi.fn().mockResolvedValue(existing),
    setPreferences: vi.fn().mockResolvedValue(existing),
  };
