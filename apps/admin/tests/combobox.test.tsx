...
const options = [
  { value: "react", label: "React", description: "UI library", keywords: ["reactjs"] },
...
beforeEach(() => {
  ...
  vi.mocked(listRecords).mockResolvedValue({
    items: [{ id: "react", name: "React", slug: "react", kind: "framework", status: "active" }],
    total: 1,
    offset: 0,
    limit: 25,
  });
...
describe("searchable combobox", () => {
  ...
  it("reports no matches and resets the query when reopened", async () => {
    ...
    const trigger = screen.getByRole("combobox", { name: "Technology" });
    await user.click(trigger);
    await user.type(screen.getByPlaceholderText("Search technology…"), "nonexistent");
  ...
  it("blocks required empty values and exposes a focusable, labelled validation error", async () => {
    const user = userEvent.setup();
    const saved = vi.fn();
    render(<Example required onSubmit={saved} />);
    await user.click(screen.getByRole("button", { name: "Save" }));
    expect(saved).not.toHaveBeenCalled();
    const trigger = screen.getByRole("combobox", { name: "Technology" });
  ...
  it("retains selected values that are absent from the current choices", () => {
    render(
      <Combobox
        ...
        onChange={vi.fn()}
        options={[]}
        selectedOption={{ value: "old", label: "Current topic" }}
      />,
    );
  ...
  it("searches language names and codes without dropping an uncommon selected tag", async () => {
    ...
    await user.click(trigger);
    await user.type(screen.getByPlaceholderText("Search languages or codes…"), "en-us");
...
describe("API-backed choices", () => {
  it("loads only on open, keeps selected labels and searches the API after a typing pause", async () => {
    ...
    vi.mocked(listRecords).mockResolvedValue({
      items: [{ id: "match", name: "Matched by an alias" }],
      total: 1,
      limit: 25,
      offset: 0,
    });
  ...
  it("excludes self-links, explains empty matches and can retry failed lookups", async () => {
    const user = userEvent.setup();
    vi.mocked(listRecords).mockRejectedValueOnce(new ApiError(503));
    render(
      <EntityPicker
        id="topic"
        label="Topic"
  ...
  it("discards late results after closing the dropdown", async () => {
    const user = userEvent.setup();
    let resolve!: (value: Awaited<ReturnType<typeof listRecords>>) => void;
    vi.mocked(listRecords).mockImplementationOnce(
      () =>
        new Promise((done) => {
          resolve = done;
        }),
    ...
    await act(async () =>
      resolve({ items: [{ id: "late", name: "Late topic" }], total: 1, offset: 0, limit: 25 }),
    );
