...
describe("shared colored status pills", () => {
  it.each([
    ...
    ["failed", "danger", "rose"],
    ["unpublished", "neutral", "slate"],
    ["disabled", "neutral", "slate"],
    ["enabled", "success", "emerald"],
    ["future_state", "neutral", "slate"],
    ["constructor", "neutral", "slate"],
  ...
  it.each([
    [true, "Yes", "check", "emerald"],
    [false, "No", "x", "rose"],
  ] as const)("shows boolean %s as an accessible %s icon", (value, label, icon, color) => {
    render(<StatusBadge value={value} />);
  ...
  it("supports a descriptive screen-reader and hover label", () => {
    ...
    expect(screen.getByRole("img", { name: "Polling disabled" }).getAttribute("title")).toBe(
      "Polling disabled",
    );
  });

  it.each([true, false, null])(
    "uses boolean icons in Enabled cells, preserving unknown values: %j",
    (enabled) => {
      renderAdmin(
        <RecordTable
          resource="sources"
  ...
  it.each([
    ...
    "analysis-jobs",
    "notification-jobs",
  ] as const)("uses the same lifecycle colors in %s tables", (resource) => {
    const states = { queued: "warning", running: "info", succeeded: "success", failed: "danger" };
    renderAdmin(
      <RecordTable
        resource={resource}
        page={{
          items: Object.keys(states).map((status) => ({
            id: `job-${status}`,
            status,
            attempts: 1,
          })),
    ...
    for (const [status, tone] of Object.entries(states))
      expect(screen.getByText(humanize(status)).getAttribute("data-variant")).toBe(tone);
  });

  it.each(["sources", "topics", "articles"] as const)(
    "uses colored statuses on %s detail pages",
    async (resource) => {
      ...
      for (const [status, tone] of Object.entries({
        ...
        expect(
          screen
            .getAllByText(humanize(status))
            .every((badge) => badge.getAttribute("data-variant") === tone),
        ).toBe(true);
      }
      if (resource === "sources")
        expect(
          screen.getByRole("img", { name: "No" }).querySelector("svg")!.getAttribute("class"),
  ...
  it.each([
    ...
    ["ERROR", "danger"],
    ["CRITICAL", "danger"],
  ] as const)("colors %s runtime log pills without changing severity labels", (level, tone) => {
    render(
      <ol>
        <JobLogLine
          entry={{
