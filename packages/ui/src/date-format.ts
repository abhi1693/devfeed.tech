...
export const defaultDateTimePreferences: DateTimePreferences = { timezone: "local", time_format: "system", date_format: "locale" };

export function formatDate(value: string | number | Date, appearance: DateTimePreferences, dateOnly = false): string {
...
export function timezoneOptions(current: string) {
  ...
  return [{ value: "local", label: "Device timezone" }, { value: "UTC", label: "UTC" },
    ...Array.from(zones).sort().map(value => ({ value, label: value.replaceAll("_", " ") }))];
}
